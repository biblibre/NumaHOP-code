...
/**
 * Aspect for logging execution of service and repository Spring components. By default,
 * it only runs with the "dev" profile.
 */
@Aspect
public class LoggingAspect {
	...
	public LoggingAspect(Long warnDuration) {
		this.warnDuration = warnDuration != null ? warnDuration : 1500l;
	}
	...
	public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			if (elapsedTime > warnDuration) {
				LOG.warn("Execution time : {} ms for method: {}.{}()", elapsedTime,
						joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
			}
			else if (LOG.isTraceEnabled()) {
				LOG.trace("Execution time : {} ms for method: {}.{}()", elapsedTime,
						joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
			}
		}
	}
