package org.numahop.numahop.service.administration.digitallibrary;
...
import org.numahop.numahop.repository.administration.digitallibrary.DigitalLibraryConfigurationRepository;
import org.numahop.numahop.service.administration.mapper.DigitalLibraryConfigurationMapper;
...
public class DigitalLibraryConfigurationService {
	...
	public DigitalLibraryConfigurationService(final DigitalLibraryConfigurationRepository repository,
			final LibraryService libraryService, final CryptoService cryptoService) {
	...
	public Set<DigitalLibraryConfigurationDTO> findByLibraryAndActiveDTO(final Library library, final boolean active) {
		return (Set<DigitalLibraryConfigurationDTO>) mapper
			.configurationsToDtos(findByLibraryAndActive(library.getIdentifier(), active));
	 ...
	 */
	@Transactional
	public DigitalLibraryConfiguration save(final DigitalLibraryConfiguration conf)
	...
	private void validate(final DigitalLibraryConfiguration conf) throws PgcnValidationException {
		...
		if (StringUtils.isEmpty(conf.getLabel())) {
			errors.add(builder.reinit()
				.setCode(PgcnErrorCode.CONF_DIGITAL_LIBRARY_LABEL_MANDATORY)
				.setField("label")
				.build());
		}
		...
		if (conf.getLibrary() == null) {
			errors.add(builder.reinit()
				.setCode(PgcnErrorCode.CONF_DIGITAL_LIBRARY_LIBRARY_MANDATORY)
				.setField("library")
				.build());
		}
		// Retour
	...
	public DigitalLibraryConfiguration create(final DigitalLibraryConfiguration configuration)
			throws PgcnTechnicalException {
		return repository.save(configuration);
	}

	public Page<DigitalLibraryConfigurationDTO> search(final String search, final List<String> filteredLibraries,
