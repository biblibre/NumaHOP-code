...
public class Z3950ServerService {
	 ...
	 * @return une liste de <code>Z3950Server</code> correspondant à l'ensemble des
	 * bibliothèques.
	 */
	@Transactional(readOnly = true)
	 ...
	 * @return liste de <code>Z3950ServerDTO</code> contenant l'ensemble des serveurs
	 * Z39.50 (identifiant + nom + version)
	 */
	@Transactional(readOnly = true)
		...
		// le nom est unique
			...
			if (dup != null) {
				errors.add(builder.reinit()
					.setCode(PgcnErrorCode.Z3950SERVER_UNIQUE_NAME_VIOLATION)
					.setField("name")
					.build());
			}
		}
