...
import org.numahop.numahop.domain.dto.document.PreDeliveryDocumentFileDTO.FileRoleEnum;
...
import org.numahop.numahop.repository.check.AutomaticCheckResultRepository;
import org.numahop.numahop.repository.check.AutomaticCheckTypeRepository;
...
import org.numahop.numahop.service.util.AutomaticCheckResultConvertUtil;
...
public class AutomaticCheckService {
	...
	public AutomaticCheckService(final AutomaticCheckTypeRepository checkTypeRepository,
			final AutomaticCheckResultRepository checkResultRepository, final FacileCinesService facileService,
			final MetaDatasCheckService metaCheckService, final DigitalDocumentService digitalDocumentService,
			final BinaryStorageManager bm, final WebsocketService websocketService,
		...
		if (CollectionUtils.isNotEmpty(oldRes.keySet())) {
			...
			results.stream().filter(res -> res.getPage() != null).forEach(res -> {
				...
				if (oldRes.get(res.getPage().getIdentifier()) != null) {
					...
					BeanUtils.copyProperties(res, old, "identifier", "createdBy", "createdDate", "lastModifiedBy",
	...
	public Map<String, AutomaticCheckResult> findByDocUnitAndDigitalDocument(final String docUnit,
			final String digitalDocument) {

		final List<AutomaticCheckResult> results = checkResultRepository
			.findAllByDocUnitAndDigitalDocumentAndCheckType(docUnit, digitalDocument, AutoCheckType.FACILE);
		return results.stream()
			.map(r -> Pair.of(r.getPage().getIdentifier(), r))
			.collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
	}

	/**
	 * Lance un contrôle sur une unité documentaire de façon asynchrone L'unité
	 * documentaire doit être complètement initialisée
	 * @param type
	 * @param doc
	 */
	@Async
	...
	/**
	 * Lance le traitement Gère la sauvegarde
	 * @param checkList
	 * @param doc
	 * @return
	 */
	public List<AutomaticCheckResult> check(final List<AutomaticCheckType> checkList, final DocUnit doc,
	...
	/**
	 * Check against FACILE Vérifie les masters des Documents Numérisés
	 * @param checkType
	 * @param results
	 * @param doc
	 */
			...
			digitalDoc.getOrderedPages()
				...
				.forEach(page -> {
					page.getMaster().ifPresent(master -> {
						...
						final AutomaticCheckResult result = AutomaticCheckResultConvertUtil
							.convert(facileService.checkFileAgainstFacile(file));
	...
	/**
	 * Vérification automatique du format de fichier Contrôle uniquement ceux qui ont le
	 * bon préfixe Persiste le résultat
	 * @param files
	 * @param format
	 * @param fileNames la liste à remplir des fichiers qui correspondent
	 * @param fileFormatRule
	 * @return
	 */
	@Transactional
	public AutomaticCheckResult checkFileNamesAgainstFormat(final AutomaticCheckResult result,
			final Collection<File> files, final String format, final List<String> fileNames,
	...
	public AutomaticCheckResult checkFileAgainstBibPrefix(final AutomaticCheckResult result,
			final Collection<File> files, final String format, final List<String> fileNames,
			final AutomaticCheckRule bibPrefixRule, final String bibPrefix, final String seqSeparator) {
		result.setResult(AutoCheckResult.OK);
		if (bibPrefixRule.isActive()) {
			fileNames.stream()
				.filter(name -> !StringUtils.startsWithIgnoreCase(name, bibPrefix.concat(seqSeparator)))
				.forEach(result::addErrorFile);
		}
	...
	public AutomaticCheckResult checkFileCase(final AutomaticCheckResult result,
			final Map<String, Optional<SplitFilename>> splitNames, final AutomaticCheckRule caseRule,
			final AutomaticCheckRule bibPrefixRule, final String bibPrefix, final String seqSeparator,
			final String prefix) {
		result.setResult(AutoCheckResult.OK);
				...
				.forEach((name) -> {
					...
					if (splitNames.get(name).isPresent()) { // à priori toujours vrai....
						...
						final String toTest = prefix.startsWith(bibPrefix)
								? split.getLibrary().concat(seqSeparator).concat(split.getPrefix()) : split.getPrefix();
	...
	/**
	 * Vérification des métadonnées / aux prérequis : - Type compression - Taux
	 * compression - Résolution - Profil colorimétrique
	 * @param results
	 * @param checkingRules
	 * @param files
	 * @param format
	...
	public List<AutomaticCheckResult> checkMetadataOfFiles(final Map<AutoCheckType, AutomaticCheckResult> results,
			final Map<AutoCheckType, AutomaticCheckRule> checkingRules, final Collection<File> files,
			final String format, final Delivery delivery, final Map<File, Optional<Metadatas>> fileMetadatasForCheck) {
		...
		final String ERR_TYPE_COMP_MSG = "Détection de types de compression ne correspondant pas au prérequis ["
			.concat(compType)
			.concat("]");
		...
		final String ERR_COLORSPACE_MSG = "Détection de profils colorimétriques différents du prérequis ["
			.concat(colorspace != null ? colorspace : "Non renseigné")
			.concat("]");
		...
		final Map<String, List<ImageMetadataValue>> cacheMetadataValuesByDocUnit = Collections
			.synchronizedMap(new HashMap<>());
		fileMetadatasForCheck.forEach((file, metasOpt) -> {

			// Integrity : metas present && File size control
			if (!metasOpt.isPresent() || (!StringUtils.equalsIgnoreCase(format, "PDF")
					&& checkFileSizeError(metasOpt.get().getFilesize(), file.length()))) {
				...
				dd.setStatus(DigitalDocument.DigitalDocumentStatus.REJECTED); // Trying to
																				// reject
																				// the
																				// document
				...
				metasOpt.ifPresent(metas -> {
					...
					if (checkCompressionTypeError(format, compress, compType)) {
						if (AutoCheckResult.OK.equals(resultCompType.getResult())) {
							resultCompType
								.setResult(ruleTypeComp.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
			...
			if (ruleTauxComp != null && ruleTauxComp.isActive()) {
				...
				if (compRate == 0 || qual == 0 || qual > compRate) {
					if (AutoCheckResult.OK.equals(resultCompRate.getResult())) {
						resultCompRate
							.setResult(ruleTauxComp.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
						if (compRate == 0) {
							...
							sbCompRate.append("Détection de valeurs de taux de compression supérieures au prérequis [")
								.append(compRate)
								.append("]");
						}
			...
			if (ruleResolution != null && ruleResolution.isActive()) {
				final String[] vals = metasOpt
					.map(metas -> StringUtils.split(StringUtils.trimToEmpty(metas.getResolution()), "x", 2))
					.orElse(null);
				...
				if (resolution == 0 || fileRes == 0 || resolution > fileRes) {
					if (AutoCheckResult.OK.equals(resultResolution.getResult())) {
						resultResolution
							.setResult(ruleResolution.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
						if (resolution == 0) {
			...
			if (ruleDefinition != null && ruleDefinition.isActive()) {
				...
				metasOpt.ifPresent(metas -> {
					if (docUnit.getImageHeight() != null && docUnit.getImageWidth() != null) {
						...
						final int threshold = height > width ? docUnit.getImageHeight() : docUnit.getImageWidth();
						...
						if (!checkDefinitionError(defErrorRate, Math.max(height, width), threshold)) {
							if (AutoCheckResult.OK.equals(resultDefinition.getResult())) {
								resultDefinition.setResult(
										ruleDefinition.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
						...
						if (AutoCheckResult.OK.equals(resultDefinition.getResult())) {
							resultDefinition
								.setResult(ruleDefinition.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
			...
			if (ruleColor != null && ruleColor.isActive() && (StringUtils.isBlank(colorspace)
					|| (metasOpt.isPresent() && !StringUtils.equals(colorspace, metasOpt.get().getColorSpace())))) {
			...
			if (ruleImageMetadata != null && ruleImageMetadata.isActive()) {
				final List<ImageMetadataValue> metadataValues = cacheMetadataValuesByDocUnit.computeIfAbsent(
						resultImageMetadata.getDigitalDocument().getDocUnit().getIdentifier(),
						imageMetadataValuesRepository::findAllByDocUnitIdentifierWithDependencies);
				final boolean ok = (!metasOpt.isPresent() && metadataValues.isEmpty())
						|| (metasOpt.isPresent() && metadataValues.stream()
							.collect(Collectors.groupingBy(ImageMetadataValue::getMetadata))
							.entrySet()
							.stream()
							.allMatch(e -> isTagValuesValid(e.getKey().getIptcTag(), e.getValue(), metasOpt.get())
									|| isTagValuesValid(e.getKey().getXmpTag(), e.getValue(), metasOpt.get())));
				if (!ok) {
					if (AutoCheckResult.OK.equals(resultImageMetadata.getResult())) {
						resultImageMetadata
							.setResult(ruleImageMetadata.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
	...
	private boolean isTagValuesValid(final String tag, final List<ImageMetadataValue> values, final Metadatas metas) {
		return StringUtils.isNotBlank(tag) && metas.getTags().containsKey(tag)
				...
				&& metas.getTags()
					.get(tag)
					.stream()
					.allMatch(t -> values.stream()
						.map(ImageMetadataValue::getValue)
						.anyMatch(value -> value.replaceAll(" ", "").equalsIgnoreCase(t.replaceAll(" ", ""))));
	}
	...
	private boolean checkFileSizeError(final String metadataSize, final long fileSize) {
		...
		if (fileSize == longCalc || Math.abs(fileSize - longCalc) < margin || longCalc == 0) {
			// on se laisse une bonne marge car les ecarts sont consequents sur les gros
			// fichiers...
	 ...
	 * @param errorRate
	 * @param value : height or width
	 * @param threshold : threshold defined by the rule
	 * @return
	 */
	 ...
	 * @return
	 */
	private AutomaticCheckResult finalizeResult(final AutomaticCheckResult result, final StringBuilder msg,
			final String seqSeparator, final String format) {
		if (!AutoCheckResult.OK.equals(result.getResult())) {
			...
			final int concernedFiles = result.getErrorFiles()
				.stream()
				.filter(f -> StringUtils.contains(f, searchFile))
				.collect(Collectors.toList())
				.size();
			if (concernedFiles > 0) {
	 ...
	 */
	@Transactional
	public List<AutomaticCheckResult> checkMetaDataFilesFormat(final Delivery delivery, final String digitalIdDoc,
			final Set<PreDeliveryDocumentFileDTO> metaDatasDTO, final List<File> files, final boolean isTocBlocking,
			final boolean isPdfBlocking, final Map<String, List<MdSecType>> extractedDmdSec) {
		...
		if (!metaDatasDTO.isEmpty()) {
			for (final PreDeliveryDocumentFileDTO dto : metaDatasDTO) {
				switch (dto.getRole()) {
					case METS:
						// validation xml METS
						final Optional<File> metsToCheck = files.stream()
							.filter(file -> StringUtils.equalsIgnoreCase(file.getName(), dto.getName()))
							.findFirst();
						tocResult = metaCheckService.checkMetaDataFileFormat(tocResult, metsToCheck,
								MetaDatasCheckService.METS_FILE_FORMAT, MetaDatasCheckService.METS_MIME_TYPE,
								FileRoleEnum.METS, extractedDmdSec);

						break;
					case EXCEL:
						// validation table des matieres excel.
						final Optional<File> excelToCheck = files.stream()
							.filter(file -> StringUtils.equalsIgnoreCase(file.getName(), dto.getName()))
							.findFirst();

						if (excelToCheck.isPresent()) {
							if (excelToCheck.get().getName().endsWith(".xlsx")) {
								tocResult = metaCheckService.checkMetaDataFileFormat(tocResult, excelToCheck,
										MetaDatasCheckService.EXCEL_FILE_FORMAT, MetaDatasCheckService.EXCEL_MIME_TYPE,
										FileRoleEnum.EXCEL, extractedDmdSec);
							}
							else {
								tocResult = metaCheckService.checkMetaDataFileFormat(tocResult, excelToCheck,
										MetaDatasCheckService.EXCEL_FILE_FORMAT2,
										MetaDatasCheckService.EXCEL_MIME_TYPE2, FileRoleEnum.EXCEL, extractedDmdSec);
							}
						}
					...
					case PDF_MULTI:
						...
						final Optional<File> pdfToCheck = files.stream()
							.filter(file -> StringUtils.equalsIgnoreCase(file.getName(), dto.getName()))
							.findFirst();
						pdfResult = metaCheckService.checkMetaDataFileFormat(pdfResult, pdfToCheck,
								MetaDatasCheckService.PDF_FILE_FORMAT, MetaDatasCheckService.PDF_MIME_TYPE,
								FileRoleEnum.PDF_MULTI, extractedDmdSec);
	...
	public List<String> findNonMaster(final Collection<File> files, final String format) {
		return files.stream()
			.map(File::getName)
			.filter(name -> !checkIfNameIsCorrect(name, format))
			.collect(Collectors.toList());
	}
	...
	/**
	 * Vérifie que le nom est correct pour un master TODO FIXME : paramétrer la mire
	 * @param name
	 * @param format
	 * @return
	 */
	...
	/**
	 * Vérification automatique de la cohérence de la séquence des fichiers Persiste le
	 * résultat
	 * @param result
	 * @param fileNames
	 * @param splitNames la map à remplir à l'aide de split
	 * @param fileSeqRule
	 ...
	 */
	@Transactional
	public AutomaticCheckResult checkSequenceNumber(final AutomaticCheckResult result, final List<String> fileNames,
			final Map<String, Optional<SplitFilename>> splitNames, final AutomaticCheckRule fileSeqRule,
			final boolean bibPrefixMandatory, final String seqSeparator, final boolean isPdfDelivery,
			final boolean isJustOneEstampe, final String prefix) {
		result.setResult(AutoCheckResult.OK);
		...
		// par contre, on ne verifie la sequence que si le controle est actif et que ce
		// n'est pas 1 estampe unique non sequencée.
					...
					if (fileNumbers.get(0) != 0 && fileNumbers.get(0) != 1) {
						...
						for (int i = 1; i < fileNumbers.size(); i++) {
							if (fileNumbers.get(i) != fileNumbers.get(i - 1) + 1) {
								result.setResult(fileSeqRule.isBlocking() ? AutoCheckResult.KO : AutoCheckResult.OTHER);
								result.setMessage("Séquence erronée entre " + fileNumbers.get(i - 1) + " et "
										+ fileNumbers.get(i) + " pour la pièce " + entry.getKey());
								break;
							}
						}
					}
	...
	/**
	 * Vérification de la conformité entre le nombre de pages attendu (doc physique) et le
	 * nombre de pages reçues
	 */
	@Transactional
	public AutomaticCheckResult checkTotalFileNumber(final AutomaticCheckResult result, final List<String> fileNames,
			final PrefixedDocuments prefixedDoc, final AutomaticCheckRule nbFilesRule) {
		result.setResult(AutoCheckResult.OK);
		...
		else if (Integer.valueOf(fileNames.size())
			.compareTo(prefixedDoc.getPhysicalDocuments().get(0).getTotalPage()) < 0) {
	...
	public AutomaticCheckResult checkRadicalFile(final AutomaticCheckResult result, final Collection<File> files,
			final AutomaticCheckRule radicalRule, final String prefix) {
		result.setResult(AutoCheckResult.OK);
	...
	/**
	 * Crée un modèle vide de résultat de contrôle (Factory) Pas de sauvegarde
	 * @param type
	 * @return
	 */
	...
	private PgcnList<PgcnError> validate(final AutomaticCheckType type) throws PgcnValidationException {
		...
		if (StringUtils.isBlank(type.getLabel())) {
			errors
				.add(builder.reinit().setCode(PgcnErrorCode.AUTO_CHECK_TYPE_LABEL_MANDATORY).setField("label").build());
		}
		/** Retour **/
		if (!errors.isEmpty()) {
	 ...
	 * @param digitalIdDoc
	 */
	private void handleLinkResultMetaDatas(final AutomaticCheckResult result, final Delivery delivery,
