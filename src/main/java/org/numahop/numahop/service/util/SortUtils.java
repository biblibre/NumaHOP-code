...
public class SortUtils {
	...
	/**
	 * Construit et Retoune un objet Sort depuis une liste de propriétés à trier obéissant
	 * au format direction du tri + nom propriété exemple : name, -age etc
	 * @param sortProperties
	 * @return
	 */
	public static Sort getSort(final List<String> sortProperties) {
		if (CollectionUtils.isNotEmpty(sortProperties)) {
			final List<Sort.Order> orders = sortProperties.stream()
				.filter(property -> property != null && property.length() > 0)
				.map(property -> {
