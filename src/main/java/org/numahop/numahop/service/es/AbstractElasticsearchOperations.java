...
public abstract class AbstractElasticsearchOperations<T extends AbstractDomainObject, U> {
	...
	public AbstractElasticsearchOperations(final TransactionService transactionService,
			final ElasticsearchOperations elasticsearchOperations, final Integer bulkSize, final Class<?> classEs,
			final JpaRepository<T, String> crudRepository, final ElasticsearchRepository<U, String> esRepository) {
	...
	public enum SearchEntity {
		...
		CONDREPORT, DELIVERY, DOCUNIT, LOT, PROJECT, TRAIN
	...
	public void index(final List<String> identifiers) {
		final List<U> filteredDetails = crudRepository.findAllById(identifiers)
			.stream()
			.map(this::convertToEsObject)
	...
	public long reindex() {
		...
		new TransactionalJobRunner<>(idsToIdex, transactionService).autoSetMaxThreads()
			.setReadOnly(true)
			.setCommit(bulkSize)
			.forEachGroup(bulkSize, ids -> {
				esRepository.saveAll(crudRepository.findAllById(ids)
					.stream()
					.map(this::convertToEsObject)
					.collect(Collectors.toList()));
				return true;
