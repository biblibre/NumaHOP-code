package org.numahop.numahop.service.user.ui;
...
import org.numahop.numahop.service.user.mapper.UIUserMapper;
...
public class UIUserService {
	...
	@Autowired
	public UIUserService(final UserService userService, final UIUserMapper uiUserMapper,
	...
	public SimpleUserAccountDTO getCurrentUserWithAuthoritiesAndDashboard() {
		...
		if (user == null && SecurityUtils.getCurrentLogin().equals(adminLogin)) {
			...
			final SimpleUserAccountDTO.Builder builder = new SimpleUserAccountDTO.Builder().reinit()
				...
				.setDashboard(dashboard != null ? dashboard.getDashboard() : null);
			...
			if (currentUser != null) {
				...
				builder.setRoles(currentUser.getAuthorities()
					.stream()
	...
	@Transactional(readOnly = true)
	public Page<SimpleUserDTO> search(String search, String initiale, boolean active, boolean filterProviders,
			List<String> libraries, List<Category> categories, List<String> roles, Integer page, Integer size) {
		final Page<User> users = userService.search(search, initiale, active, filterProviders, libraries, categories,
				roles, page, size);
