...
import org.numahop.numahop.domain.user.Dashboard;
...
import org.numahop.numahop.repository.user.DashboardRepository;
...
import org.numahop.numahop.service.exchange.template.VelocityEngineService;
import org.numahop.numahop.service.storage.FileStorageManager;
import org.numahop.numahop.service.util.RandomUtil;
...
public class UserService {
	...
	public UserService(final DashboardRepository dashboardRepository, final FileStorageManager fm,
			final LotRepository lotRepository, final MailService mailService, final PasswordEncoder passwordEncoder,
			final PersistentTokenRepository persistentTokenRepository, final ProjectRepository projectRepository,
			final UserRepository userRepository, final WorkflowGroupRepository workflowGroupRepository,
			final UserValidationService userValidationService, final VelocityEngineService templateService) {
		this.fm = fm;
	...
	private void validateDelete(final User user) throws PgcnValidationException {
		...
		if (projCount > 0) {
			errors.add(builder.reinit()
				.setCode(PgcnErrorCode.USER_DEL_EXITS_PROJ)
		...
		if (lotCount > 0) {
			errors.add(builder.reinit()
				.setCode(PgcnErrorCode.USER_DEL_EXITS_LOT)
		...
		if (workflowCount > 0) {
			errors.add(builder.reinit()
				.setCode(PgcnErrorCode.USER_IN_WORKFLOW_GROUP)
				.setAdditionalComplement(lotCount)
				.build());
		}
	...
	/**
	 * Création d'un usager La vérification des champs requis se font en amont dans les
	 * DTO
	 */
	@Transactional
	...
	/**
	 * Sauvegarde un usager La vérification des champs requis se font en amont dans les
	 * DTO
	 */
	@Transactional
	public User update(final User user) throws PgcnValidationException {
		...
		// On ne modifie pas le mot de passe via cette méthode donc on va chercher
		// l'ancien dans la base pour le rétablir
	...
	public Page<User> search(final String search, final String initiale, final boolean active,
			final boolean filterProviders, final List<String> libraries, final List<Category> categories,
			final List<String> roles, final Integer page, final Integer size) {
		final Pageable pageRequest = PageRequest.of(page, size);
		return userRepository.search(search, initiale, active, filterProviders, libraries, categories, roles,
				pageRequest);
	}
	...
	public void removeOldPersistentTokens() {
		...
		final List<PersistentToken> tokens = persistentTokenRepository
			.findByTokenDateBefore(Date.from(now.minusMonths(1).atStartOfDay(ZoneId.systemDefault()).toInstant()));
	...
	/**
	 * Duplication d'un utilisateur. On ne recopie ni le login, ni le mot de passe.
	 * @param source
	 * @param destination
	 */
	 ...
	 * @throws IOException
	 */
	private boolean sendEmail(final User user, final Name templateName, final HashMap<String, Object> parameters)
			throws IOException {
		if (!user.isActive()) {
	...
	public File getUserSignature(final User user) {
		...
		if (user.getLibrary() != null) {
			logoFile = fm.getUploadFile(userDir, user.getLibrary().getIdentifier(), null,
					ViewsFormatConfiguration.FileFormat.MASTER.label() + "." + user.getIdentifier());
		}
		else {
			logoFile = fm.getUploadFile(userDir, null, null,
					ViewsFormatConfiguration.FileFormat.MASTER.label() + "." + user.getIdentifier());
		}
	...
	public File getUserThumbnail(final User user) {
		...
		if (user.getLibrary() != null) {
			thumbnailFile = fm.getUploadFile(userDir, user.getLibrary().getIdentifier(), null,
					ViewsFormatConfiguration.FileFormat.THUMB.label() + "." + user.getIdentifier());
		}
		else {
			thumbnailFile = fm.getUploadFile(userDir, null, null,
					ViewsFormatConfiguration.FileFormat.THUMB.label() + "." + user.getIdentifier());
		}
	...
	public void uploadSignature(final User user, final MultipartFile file) {
		if (file != null && file.getSize() > 0) {
			LOG.debug("Téléversement de la signature {} de l'utilisateur {}", file.getOriginalFilename(),
					user.getIdentifier());
	...
	private void uploadImage(final User user, final MultipartFile file,
			final ViewsFormatConfiguration.FileFormat format) {
		try (InputStream in = file.getInputStream()) {
			fm.createThumbnail(in, file.getContentType(), format, userDir, null,
					format.label() + "." + user.getIdentifier());
			LOG.debug("Le logo de l'utilisateur {} ({}) a été importé: {} (format {})", user.getLogin(),
					user.getIdentifier(), file.getOriginalFilename(), format);
