...
import org.numahop.numahop.repository.document.conditionreport.ConditionReportDetailRepository;
...
public class ConditionReportDetailService {
	...
	public ConditionReportDetailService(final ConditionReportDetailRepository conditionReportDetailRepository,
			final PropertyConfigurationService propertyConfigurationService, final UserService userService) {
	 ...
	 */
	@Transactional
	public ConditionReportDetail create(final ConditionReportDetail.Type type, final ConditionReport report)
	 ...
	 */
	@Transactional
	public ConditionReportDetail create(final ConditionReportDetail.Type type, final String fromDetailId)
			throws PgcnValidationException {
	 ...
	 * @param position position du détail parmi les détails du constat d'état
	 */
	public ConditionReportDetail getNewDetail(final ConditionReportDetail.Type type, final ConditionReport report,
			final int position) {
	 ...
	 * @param position position du détail parmi les détails du constat d'état
	 */
	public ConditionReportDetail getNewDetailWithoutWriters(final ConditionReportDetail.Type type,
			final ConditionReport report, final int position) {
		final ConditionReportDetail detail = new ConditionReportDetail();
		detail.setType(type);
		detail.setReport(report);
		detail.setDate(LocalDate.now());
	...
	public void delete(final String identifier) throws PgcnValidationException {
		conditionReportDetailRepository.findById(identifier).ifPresent(detail -> {
			...
			final List<ConditionReportDetail> details = conditionReportDetailRepository
				.findByConditionReportIdentifier(reportId)
				.stream()
	 ...
	 */
	@Transactional
	public ConditionReportDetail save(final ConditionReportDetail detail, final boolean validateDetail)
			throws PgcnValidationException {
	...
	public ConditionReportDetail updateProvWriter(final ConditionReportDetail value) {
		final ConditionReportDetail detail = conditionReportDetailRepository.findById(value.getIdentifier())
			.orElseThrow();
	...
	@Transactional(readOnly = true)
	public PgcnList<PgcnError> validateSave(final ConditionReportDetail detail,
		...
		if (detail.getDescriptions().isEmpty() && StringUtils.isBlank(detail.getAdditionnalDesc())
				&& StringUtils.isBlank(detail.getBindingDesc()) && StringUtils.isBlank(detail.getBodyDesc())
				&& detail.getDim1() == null && detail.getDim2() == null && detail.getDim3() == null
				&& detail.getInsurance() == null && detail.getNbViewTotal() == 0) {
		...
		if (CollectionUtils.isNotEmpty(confs)) {
			...
			confs.stream()
				...
				.filter(p -> detail.getDescriptions()
					.stream()
					.noneMatch(desc -> StringUtils.equals(desc.getProperty().getIdentifier(),
							p.getDescProperty().getIdentifier()) && !desc.isEmpty()))
				.map(PropertyConfiguration::getDescProperty)
				.forEach(p -> errors.add(builder.reinit()
					.setCode(PgcnErrorCode.CONDREPORT_DETAIL_DESC_EMPTY)
					.setField(p.getLabel())
					.build()));
	...
	private int getNextPosition(final ConditionReport report) {
		final Integer pos = conditionReportDetailRepository
			.getMaxPositionByConditionReportIdentifier(report.getIdentifier());
