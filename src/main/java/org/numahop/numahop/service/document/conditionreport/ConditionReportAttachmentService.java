...
public class ConditionReportAttachmentService {
	...
	@Autowired
	public ConditionReportAttachmentService(
			final ConditionReportAttachmentRepository conditionReportAttachmentRepository,
			final ConditionReportRepository conditionReportRepository, final FileStorageManager fm) {
		...
		for (final MultipartFile file : files) {
			...
			if (attachmentFile == null) {
				LOG.error("Une erreur s'est produite lors de la sauvegarde de la pièce jointe {} (ConditionReport {})",
						file.getOriginalFilename(), reportId);
				continue;
			}
			LOG.debug("Fichier {} créé", attachmentFile.getAbsolutePath());

			// Création de l'aperçu
			final File thumbnail = fm.createThumbnail(attachmentFile, file.getContentType(),
					ViewsFormatConfiguration.FileFormat.THUMB, reportDir, attachment.getReport(),
	...
	public File downloadAttachmentThumbnail(final ConditionReportAttachment attachment) {
		return fm.retrieveFile(reportDir, attachment.getReport(),
				ViewsFormatConfiguration.FileFormat.THUMB.label().concat(".").concat(attachment.getOriginalFilename()));
	}
