...
public abstract class ConditionReportDetailMapper {
	...
	@AfterMapping
	protected void updateDTO(final ConditionReportDetail detail,
			@MappingTarget final ConditionReportDetailVelocityDTO dto) {
	...
	private String concat(final String a, final String b) {
		return StringUtils.isNotBlank(a) ? StringUtils.isNotBlank(b) ? a + ", " + b : a
				: StringUtils.isNotBlank(b) ? b : "";
	}
	 ...
	 * @return
	 */
	private List<ConditionReportValueDTO> getDescriptionList(final List<ConditionReportValueDTO> descriptions,
			final DescriptionProperty.Type type) {
		return descriptions.stream()
			.filter(d -> DescriptionProperty.Type.valueOf(d.getPropertyType()) == type)
			.collect(Collectors.toList());
	}
