...
import org.numahop.numahop.domain.document.DocPage.PageStatus;
import org.numahop.numahop.domain.document.GlobalCheck;
...
import org.numahop.numahop.repository.document.CheckRepository;
...
import org.numahop.numahop.repository.document.GlobalCheckRepository;
...
public class CheckService {
	...
	@Autowired
	public CheckService(final CheckRepository checkRepository, final GlobalCheckRepository globalCheckRepository,
			final DigitalDocumentRepository digitalDocumentRepository, final DocPageRepository docPageRepository,
			final LotService lotService, final SampleService sampleService,
			final DigitalDocumentService digitalDocumentService, final DeliveryRepository deliveryRepository,
			final TransactionService transactionService, final ConditionReportService condReportService) {
	...
	public synchronized DocErrorReport setChecks(final String identifier, final DocPageErrorsDTO errors,
			final Integer pageNumber, final boolean sampling, final String deliveryId) {
	...
	private DocErrorReport generateErrorReportForSample(final String identifier) {
		...
		final DigitalDocument digitalDocument = digitalDocumentRepository
			.getOneWithCheckConfiguration(sample.getDocuments().iterator().next().getIdentifier());
	...
	private DigitalDocument cleanChecks(final String identifier) {
		...
		final List<Check.ErrorLabel> errLabels = digitalDocument.getChecks()
			.stream()
			.map(GlobalCheck::getErrorLabel)
	...
	public Set<String> getCondReportSummaryForSamplePage(final String idPage) {
		...
		try {
			...
			ConditionReport report = condReportService
				.findByDocUnit(docPage.getDigitalDocument().getDocUnit().getIdentifier());
