 ...
 *
 * @author jbrunet Créé le 7 mars 2017
 */
@Service
public class FileStorageManager {
	...
	public FileStorageManager(final ImageDispatcherService imageDispatcherService,
			final DefaultFileFormats defaultFileFormats, final UserRepository userRepository) {
	...
	/**
	 * Store a file, no questions asked. idem ci-dessus, mais sans chercher la library
	 * (réservé à ADMIN!).
	 * @param input
	 * @param directory
	 * @param filename
	...
	/**
	 * Store a file, no questions asked. idem ci-dessus, mais avec possiblilité
	 * d'intercaler des dossiers supplémentaires.
	 * @param input
	 * @param directory
	 * @param filename
	 * @param createDir
	 */
	public File copyInputStreamToFileWithOtherDirs(final InputStream input, final File directory,
			final List<String> dirsToAdd, final String filename, final boolean createDir, final boolean addLibInPath) {
		if (input != null && directory != null && filename != null) {
	...
	/**
	 * Upload d'un fichier storageDir/obj.getIdentifier()
	 * @param source
	 * @param storageDir
	 * @param obj
	 * @return
	 */
	public File copyInputStreamToFile(final InputStream source, final String storageDir,
			final AbstractDomainObject obj) {
		final File destinationDir = Paths.get(storageDir).toFile();
		final String destination = Base64.getEncoder()
			.encodeToString(obj.getIdentifier().getBytes(StandardCharsets.UTF_8));
		return copyInputStreamToFile(source, destinationDir, destination, true, true);
	}

	/**
	 * Upload d'un fichier storageDir/library/obj.getIdentifier()/name
	 * @param source
	 * @param storageDir
	 * @param obj
	 * @return
	...
	/**
	 * Retrieve a file, no questions asked storageDir/libraryId/obj.getIdentifier()/name
	 * @param directory
	 * @param dirsToAdd
	 * @param filename
	 * @return
	...
	public File retrieveFile(final String storageDir, final AbstractDomainObject obj) {
		...
		final String filename = Base64.getEncoder()
			.encodeToString(obj.getIdentifier().getBytes(StandardCharsets.UTF_8));
	...
	/**
	 * Récupération d'un fichier lié à un {@link AbstractDomainObject}
	 * storageDir/obj.getIdentifier() Pas de controle du user / library !!!! A reserver au
	 * reset password !!!!
	 * @param storageDir
	 * @param obj
	 * @return
	 ...
	 * @return
	 */
	public File createThumbnail(final File source, final String mimeType,
			final ViewsFormatConfiguration.FileFormat format, final String storageDir, final AbstractDomainObject obj,
			final String name) {
		...
		final boolean generationResult = imageDispatcherService.createThumbnailDerived(mimeType, source, destination,
				format, null,
				new Long[] { defautFileFormats.getHeightByFormat(format), defautFileFormats.getWidthByFormat(format) });
		if (!generationResult || destination.length() == 0L) {

			LOG.debug("La génération de l'aperçu pour le fichier {} a échoué ({})", source.getAbsolutePath(),
	...
	public File createThumbnail(final InputStream source, final String mimeType,
			final ViewsFormatConfiguration.FileFormat format, final String storageDir, final AbstractDomainObject obj,
			final String name) {
		final File destination = getUploadFile(storageDir, null, obj, name);
		if (destination == null) {
		...
		switch (mimeType) {
			...
			case MediaType.IMAGE_PNG_VALUE:
				generationResult = ImageUtils.createThumbnail(source, destination,
	 ...
	 * @return
	 */
	public File getUploadFile(final String storageDir, final String libraryId, final AbstractDomainObject obj,
			final String name) {
		final String destination = Base64.getEncoder().encodeToString(name.getBytes(StandardCharsets.UTF_8));
	...
	/**
	 * Génération du fichier temporaire utilisé pour le traitement asynchrone d'un
	 * MultipartFile
	 * @param multipartFile
	 * @return
	 */
	public File getImportFile(final MultipartFile multipartFile, final String targetDir) {
		return new File(targetDir, String.format("%s-%s-%s", SecurityUtils.getCurrentLogin(),
