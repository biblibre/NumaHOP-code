...
public class ExifToolService {
	...
	private static final String DEFAULT_ARGS = "$FileSize" + META_DATAS_SEPARATOR + "$FileType" + META_DATAS_SEPARATOR
			+ "$ProfileDescription" + META_DATAS_SEPARATOR + "$BitsPerComponent" + META_DATAS_SEPARATOR + "$XResolution"
			+ META_DATAS_SEPARATOR + "$YResolution" + META_DATAS_SEPARATOR + "$DisplayXResolution"
			+ META_DATAS_SEPARATOR + "$DisplayYResolution" + META_DATAS_SEPARATOR + "$DisplayXResolutionUnit"
			+ META_DATAS_SEPARATOR + "$ImageHeight" + META_DATAS_SEPARATOR + "$ImageWidth";
	...
	/**
	 * Collecte les metadatas du fichier image. [Utilisé pour JPEG 2000 == JP2]
	 * @param img
	 * @return
	...
	public Optional<Metadatas> extractMetadatas(final File img, final List<String> tags) throws PgcnTechnicalException {
		...
		if (!this.isConfigured) {
			throw new PgcnTechnicalException(
					"Le service de conversion Exif Tool n'est pas configuré, impossible de collecter les metadatas.");
		}
		if (img == null) {
		...
		final ProcessBuilder builder = new ProcessBuilder(exifProcessPath, "-m", "-sep",
				quoteDelim + META_TAGS_SEPARATOR + quoteDelim, "-p", args.toString(), img.getAbsolutePath());
		...
		IOUtils.readLines(is, StandardCharsets.UTF_8).forEach(ligne -> {
			if (StringUtils.isNotBlank(ligne)) {
				...
				if (!StringUtils.trim(tab[4]).isEmpty() || !StringUtils.trim(tab[5]).isEmpty()) {
					metasBuilder.setResolution(StringUtils.trim(tab[4]) + "x" + StringUtils.trim(tab[5]));

					// Display Resolution / Display Resolution Unit
				}
				else if ((!StringUtils.trim(tab[6]).isEmpty() || !StringUtils.trim(tab[7]).isEmpty())
						&& !StringUtils.trim(tab[8]).isEmpty()) {
					metasBuilder.setResolution(getResolution(tab[6], tab[8]) + "x" + getResolution(tab[7], tab[8]));
				}
	...
	public void copyAllMetadatas(final File source, final File destination) throws PgcnTechnicalException {
		final ProcessBuilder builder = new ProcessBuilder(exifProcessPath, "-tagsFromFile", source.getAbsolutePath(),
				"-all:all", "-overwrite_original", destination.getAbsolutePath());
		try {
			final Process process = builder.start();
			if (process.waitFor() == 0) {
	...
	/**
	 * tableau de conversion pour la métadonnée Display Resolution en pouce L'unité est
	 * présente dans la métadonnée Display X Resolution Unit
	 */
	...
	/**
	 * Calcul de la resolution 2.54 conversion pouce / cm
	...
	private String getResolution(final String resolution, final String resolutionUnit) {
		...
		if (conversion != null) {
			return String.valueOf(conversion * 2.54 * Double.parseDouble(StringUtils.trim(resolution)));
		}
		return "";
	}
