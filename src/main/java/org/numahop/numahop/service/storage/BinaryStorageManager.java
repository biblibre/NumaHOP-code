...
import org.numahop.numahop.domain.storage.StoredFile.StoredFileType;
...
public class BinaryStorageManager {
	...
	public BinaryStorageManager(final BinaryRepository binaryRepository,
			final ImageDispatcherService imageDispatcherService, final ImageMagickService imageMagickService,
			final ExifToolService exifToolService, final DeliveryProgressService deliveryProgressService,
			final ViewsFormatConfigurationService formatConfigurationService, final UserRepository userRepository) {
	...
	public void initialize(final String binaries, final int depth, final String digest,
			final String[] instanceLibraries) throws IOException {
		...
		Arrays.asList(instanceLibraries).forEach(lib -> {
			...
			LOG.info(
					"LIBRARY {} : Binaries storage using {} for data and {} for temporary data. Binary global store: {}",
					lib, storageDir, tmpDir, libBase);
		});
	}
	...
	@Transactional
	public StoredFile createFromFileForPage(final DocPage page, final File file, final StoredFileType type,
			final ViewsFormatConfiguration.FileFormat format, final Optional<Map<Integer, String>> ocrByPage,
	...
	/**
	 * Permet d'extraire les métadonnées d'un fichier. Des métadonnées de base sont
	 * extraites et il est possible de passer une liste de tags exiftools supplémentaires
	 * à extraire
	...
	public Optional<Metadatas> getMetadatas(final File file, final List<String> tags) throws PgcnTechnicalException {
		...
		switch (format.toUpperCase()) {
			...
			case "PNG":
				if (CollectionUtils.isEmpty(tags)) {
					...
					metas = mergeMetadatas(imageMagickService.getMetadatasOfFile(file, false),
							exifToolService.extractMetadatas(file, tags));
				}
				break;
			case "PDF":
		...
		return metas.isPresent() && !metas.get().isEmpty() ? metas : Optional.empty();
	}
	...
	public StoredFile createFromFileForPage(final DocPage page, final File file, final StoredFileType type,
			final ViewsFormatConfiguration.FileFormat format, final Optional<Map<Integer, String>> ocrByPage,
			final Map<File, Optional<Metadatas>> fileMetadatas, final String libraryId) throws PgcnException {
		...
		final ViewsFormatConfiguration conf = formatConfigurationService
			.getOneByLot(page.getDigitalDocument().getDocUnit().getLot().getIdentifier());
		...
		try {
			...
			if (!ViewsFormatConfiguration.FileFormat.MASTER.equals(format)) {
				...
				if (fileMetadatas == null) {
					try {
						// On passe une collection vide pour les tags car on n'a pas
						// besoin de
						// métadonnées supplémentaires ici
	...
	private void setMetadatas(final Metadatas metas, final StoredFile storedFile) {
		...
		if (NumberUtils.isParsable(metas.getQuality())) {
			storedFile.setCompressionRate(Integer.valueOf(metas.getQuality())); // en
																				// fait,
																				// bit
																				// depth
																				// (8)
	...
	private Long[] getDimensionsFromMeta(final Metadatas metas) {
		...
		if (StringUtils.isNotBlank(metas.getWidth()) && NumberUtils.isParsable(metas.getWidth())
				&& StringUtils.isNotBlank(metas.getHeight()) && NumberUtils.isParsable(metas.getHeight())) {
	...
	@Transactional
	@Async
	public void generateDerivedThumbnailForMaster(final StoredFile master, final DocPage page,
			final ViewsFormatConfiguration.FileFormat format, final Map<File, Optional<Metadatas>> fileMetadatas,
		...
		try {
			...
			final boolean generationResult = imageDispatcherService.createThumbnailDerived(master.getMimetype(),
					masterFile, thumbnailTmp, format, master.getFormatConfiguration(), masterDims);
			if (generationResult && thumbnailTmp.length() > 0L) {
				...
				if (progression > 0d && StringUtils.equalsIgnoreCase("Zoom", format.label())) {
					deliveryProgressService.deliveryProgress(delivery, page.getDigitalDocument().getDigitalId(), "INFO",
							"DELIVERING", progression, "", true);
				}
	...
	@Transactional
	public void generateDerivedThumbnailForPage(final DocPage page, final StoredFile master,
			final ViewsFormatConfiguration.FileFormat format, final Map<File, Optional<Metadatas>> fileMetadatas,
			final double progress, final Delivery delivery, final String libraryId) throws PgcnException {
		final PgcnList<PgcnError> errors = new PgcnList<>();
		final PgcnError.Builder builder = new PgcnError.Builder();
	...
	protected void storeWithDigest(final InputStream in, final StoredFile storedFile, final String libraryId)
			throws IOException {
	...
	public void deleteOrphanFiles(final String libraryId) {
		...
		try (final Stream<Path> stream = Files.walk(dest, FileVisitOption.FOLLOW_LINKS)) {
			...
			LOG.error("Erreur lors de la suppression des fichiers binaires dans {}", dest.toAbsolutePath().toString(),
					e);
		}
	...
	public static class Metadatas {
		...
		public Metadatas(final String width, final String height, final String filesize, final String format,
				final String colorSpace, final String compression, final String quality, final String resolution,
		...
		public static class Builder {
			...
			public Metadatas build() {
				return new Metadatas(StringUtils.trim(width), StringUtils.trim(height), StringUtils.trim(filesize),
						StringUtils.trim(format), StringUtils.trim(colorSpace), StringUtils.trim(compression),
						StringUtils.trim(quality), StringUtils.trim(resolution), tags);
			}

		}

		public boolean isEmpty() {
			return StringUtils.isBlank(width) && StringUtils.isBlank(height) && StringUtils.isBlank(filesize)
					&& StringUtils.isBlank(format) && StringUtils.isBlank(colorSpace)
					&& StringUtils.isBlank(compression) && StringUtils.isBlank(quality)
					&& StringUtils.isBlank(resolution) && tags.isEmpty();
		}
