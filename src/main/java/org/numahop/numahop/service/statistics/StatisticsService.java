...
import org.numahop.numahop.domain.dto.statistics.StatisticsDocUnitCheckDelayDTO;
import org.numahop.numahop.domain.dto.statistics.StatisticsDocUnitCountDTO;
import org.numahop.numahop.domain.dto.statistics.StatisticsDocUnitStatusRatioDTO;
...
import org.numahop.numahop.service.workflow.DocUnitWorkflowService;
...
public class StatisticsService {
	...
	public Page<StatisticsProjectDTO> searchProject(final String search, final List<String> libraries,
			final List<String> projects, final LocalDate fromDate, final LocalDate toDate, final Integer page,
			final Integer size, final List<String> sorts) {
		final Page<Project> results = projectService.findAllForStatistics(search, libraries, projects, fromDate, toDate,
				page, size, sorts);
	...
	@Transactional(readOnly = true)
	public List<Map<String, Object>> getDeliveryGroupByStatus(final List<String> libraries, final List<String> projects,
	...
	@Transactional(readOnly = true)
	public Page<StatisticsDocUnitCountDTO> getDocUnits(final List<String> libraries, final List<String> projects,
			final List<String> lots, final Integer page, final Integer size) {

		final Page<DocUnit> pageOfDocs = docUnitService.search(null, false, false, false, false, false, false, false,
				false, false, false, libraries, projects, lots, Collections.emptyList(), Collections.emptyList(), null,
				null, null, null, null, page, size, null);
		final List<String> docUnitIds = pageOfDocs.getContent()
			.stream()
		...
		return pageOfDocs.map(doc -> {
			...
			final boolean isIa = iaReports.stream()
				.anyMatch(report -> StringUtils.equals(report.getDocUnit().getIdentifier(), doc.getIdentifier())
			...
			final int totalPages = doc.getPhysicalDocuments()
				.stream()
			...
			final long totalLength = doc.getDigitalDocuments()
				.stream()
				.filter(dg -> dg.getTotalLength() != null)
	...
	public List<StatisticsDocUnitAverageDTO> getDocUnitAverages(final List<String> libraries,
			...
			final LocalDate fromDate, final LocalDate toDate, final GroupBy groupBy) {

		final List<DocUnitWorkflow> workflows = docUnitWorkflowService.findAll(libraries, projects, lots, deliveries,
				fromDate, toDate);
		...
		switch (groupBy) {
			case PROJECT:
				return groupDocUnitWorkflowByProject(workflows).entrySet()
					.stream()
					// Pour chaque projet + liste de workflow, calcul d'un
			...
			case LOT:
				return groupDocUnitWorkflowByLot(workflows).entrySet()
					.stream()
					// Pour chaque lot + liste de workflow, calcul d'un
					// StatisticsDocUnitAverageDTO
					.map(e -> {
	...
	public List<StatisticsDocUnitCheckDelayDTO> getCheckDelayStatisitics(final List<String> libraries,
		...
		final List<DocUnitWorkflow> workflows = docUnitWorkflowService.findDocUnitWorkflowsInControl(libraries,
				projects, lots, deliveries);
	...
	@Transactional(readOnly = true)
	public StatisticsDocUnitStatusRatioDTO getDocUnitStatusRatio(final List<String> libraries, final String project,
			final String lot, final WorkflowStateKey state) {
		final Page<DocUnit> pageOfDocs = docUnitService.search(null, false, false, false, false, false, false, false,
				false, false, false, libraries, project != null ? Collections.singletonList(project) : null,
				lot != null ? Collections.singletonList(lot) : null, Collections.emptyList(), Collections.emptyList(),
				null, null, null, null, null, 0, Integer.MAX_VALUE, null);
		...
		final List<DocUnitWorkflow> docWorkflows = docUnitWorkflowService.findAll(libraries,
				...
				lot != null ? Collections.singletonList(lot) : null, null,
	 ...
	 * @param workflows
	 * @param deliveries filtrer par livraison
	 * @return
	 */
		...
		return workflows.stream()
			...
			.flatMap(w -> w.getDocUnit()
				...
				.filter(delivery -> delivery != null
						&& (CollectionUtils.isEmpty(deliveries) || deliveries.contains(delivery.getIdentifier())))
				// on renvoie la livraison + le doc workflow pour construire le
				// regroupement
	...
	private void setDocUnitAverages(final StatisticsDocUnitAverageDTO dto, final List<DocUnitWorkflow> workflows) {
		...
		final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit = workflows.stream()
			.collect(Collectors.groupingBy(DocUnitWorkflow::getDocUnit));
		...
		byDocUnit.entrySet()
			...
			.map(e -> {
				...
				return docUnitWorkflows.stream()
					...
					// nombre de jours restant jusque: date de (re)livraison + délai de
					// contrôle
	 ...
	 * @param byDocUnit
	 */
	private void setAvgTotalPages(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
	 ...
	 * @param byDocUnit
	 */
	private void setRejectRatio(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
		...
		if (CollectionUtils.isEmpty(digs)) {
			...
			final long nbRejected = digs.stream()
				...
				.filter(dd -> (dd.getDeliveries().size() == 1
						...
						|| (dd.getDeliveries().size() > 1 && dd.getDeliveries()
							.stream()
	 ...
	 * @param byDocUnit
	 */
	private void setTotalLengthDocs(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
	 ...
	 * @param byDocUnit
	 */
	private void setAvgDurControl(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
		byDocUnit.values()
			.stream()
			// workflows
			.flatMap(Collection::stream)
			// calcul de la durée des étapes de contrôles (achevées) pour chaque workflow
			...
			.map(workflow -> {
				...
				// pas d'étapes => on n'en tient pas compte dans le calcul de la durée
				// moyenne
				return null;
	 ...
	 * @param byDocUnit
	 */
	private void setAvgDurDelivery(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
		byDocUnit.values()
			.stream()
			// workflow
	 ...
	 * @param byDocUnit
	 */
	private void setAvgDurWorkflow(final StatisticsDocUnitAverageDTO dto,
			final Map<DocUnit, List<DocUnitWorkflow>> byDocUnit) {
		byDocUnit.values()
			.stream()
			// workflows
			...
			// calcul de la durée du workflow, ie entre le début de l'initialisation et la
			// fin de la clôture
