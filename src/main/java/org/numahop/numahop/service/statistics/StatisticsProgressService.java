...
import org.numahop.numahop.domain.dto.statistics.StatisticsProgressDTO;
...
public class StatisticsProgressService {
	...
	public StatisticsProgressService(final CinesReportService cinesReportService, final DeliveryService deliveryService,
			final DocUnitWorkflowRepository docUnitWorkflowRepository,
			final InternetArchiveReportService iaReportService, final ProjectService projectService,
	...
	/**
	 * #186: Tableau de bord des bibliothèques et institutions par projet
	 */
	@Transactional(readOnly = true)
	public Page<StatisticsProgressDTO> getProjectProgress(final List<String> libraries, final List<String> projects,
			final LocalDate fromDate, final LocalDate toDate, final Integer page, final Integer size) {
		// Projets
		final Page<Project> pageOfProjects = projectService.findAllForStatistics(null, libraries, projects, fromDate,
				toDate, page, size, null);
		...
		// Pct
		setDocUnitsPct(pageOfDtos,
				pageOfProjects.getContent()
					.stream()
					.flatMap(pj -> pj.getDocUnits().stream())
					.collect(Collectors.toList()));
	...
	/**
	 * #186: Tableau de bord des bibliothèques et institutions par lot
	 */
	@Transactional(readOnly = true)
	public Page<StatisticsProgressDTO> getLotProgress(final List<String> libraries, final List<String> projects,
			final LocalDate fromDate, final LocalDate toDate, final Integer page, final Integer size) {
		// Lots
		final Page<Lot> pageOfLots = lotRepository.search(new LotSearchBuilder().setLibraries(libraries)
		...
		final List<String> lotIds = pageOfLots.getContent()
			.stream()
			.map(AbstractDomainObject::getIdentifier)
			.collect(Collectors.toList());
	...
	private void setDlvCounts(final StatisticsProgressDTO dto, final Set<DocUnit> docUnits) {
		...
		final int nbDlvControlled = Arrays.stream(DigitalDocumentStatus.values())
			...
			.sum();
		final int nbDlvRejected = countByStatus.getOrDefault(DigitalDocumentStatus.PRE_REJECTED, 0)
				+ countByStatus.getOrDefault(DigitalDocumentStatus.REJECTED, 0);
	 ...
	 * @param lotIds
	 */
	private void setDeliveryCounts(final Page<StatisticsProgressDTO> pageOfDtos, final List<String> projects,
		...
		pageOfDtos.forEach(dto -> {
			final long nbDlv = deliveries.stream()
				.filter(d -> match(dto, d.getLot().getIdentifier(), d.getLot().getProject().getIdentifier()))
				.count();
			dto.setNbDlv(nbDlv);
		});
	}
	 ...
	 * @param lotIds
	 */
	private void setWorkflowCounts(final Page<StatisticsProgressDTO> pageOfDtos, final List<String> libraries,
			final List<String> projects, final List<String> lotIds) {
		final List<DocUnitWorkflow> validatedUdw = docUnitWorkflowRepository.findDocUnitWorkflows(
				new DocUnitWorkflowSearchBuilder().setLibraries(libraries).setProjects(projects).setLots(lotIds));
		pageOfDtos.forEach(dto -> {
			final long nbWorkflowValidated = validatedUdw.stream()
				.filter(d -> match(dto,
						d.getDocUnit().getLot() != null ? d.getDocUnit().getLot().getIdentifier() : null,
						d.getDocUnit().getProject() != null ? d.getDocUnit().getProject().getIdentifier() : null))
	...
	private void setCinesCounts(final Page<StatisticsProgressDTO> pageOfDtos, final List<String> docUnitIds) {
		...
		pageOfDtos.forEach(dto -> {
			final long nbDocArchived = cinesReports.stream()
				.filter(report -> match(dto, report.getDocUnit().getLot().getIdentifier(),
						report.getDocUnit().getProject().getIdentifier()))
				.map(d -> d.getDocUnit().getIdentifier())
				.distinct()
				.count();
