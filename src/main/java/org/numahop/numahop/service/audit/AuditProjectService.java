...
import org.numahop.numahop.repository.audit.AuditProjectRepository;
...
public class AuditProjectService {
	...
	@Autowired
	public AuditProjectService(final AuditProjectRepository auditProjectRepository,
	...
	public List<AuditProjectRevisionDTO> getRevisions(final LocalDate fromDate, final List<String> libraries,
	 ...
	 */
	private List<AuditProjectRevisionDTO> updateRevisions(final List<AuditProjectRevisionDTO> revisions,
			final List<String> libraries) {
		final List<AuditProjectRevisionDTO> updatedRevs = new ArrayList<>();
		final List<String> projectIds = revisions.stream()
			.map(AuditProjectRevisionDTO::getIdentifier)
		...
		for (final AuditProjectRevisionDTO revision : revisions) {
			projects.stream()
				...
				.filter(pj -> CollectionUtils.isEmpty(libraries)
						|| (pj.getLibrary() != null && libraries.contains(pj.getLibrary().getIdentifier())))
				.findAny()
				.ifPresent(pj -> {
