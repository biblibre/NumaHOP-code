...
public class ImageMetadataValuesService {
	...
	@Autowired
	public ImageMetadataValuesService(final ImageMetadataValuesRepository imageMetadataValuesRepository,
			final ImageMetadataValuesMapper imageMetadataValuesMapper) {
	...
	public List<ImageMetadataValue> saveList(List<ImageMetadataValuesDTO> valuesDto) throws PgcnValidationException {
		...
		List<ImageMetadataValue> metadataToRemove = getValuesByDocUnit(valuesDto.get(0).getDocUnitId()).stream()
			.filter(meta -> !valuesDto.stream()
				.anyMatch(valDto -> StringUtils.equals(valDto.getIdentifier(), meta.getIdentifier())))
		...
		return savedValues.stream()
			.map(value -> imageMetadataValuesRepository.findOneWithDependencies(value.getIdentifier()))
			.collect(Collectors.toList());
	}
