...
public class TemplateResourceLoader implements ResourceLoader {
	...
	public InputStream getResourceStream(final ResourceName resourceName) throws ResourceNotFoundException {
		...
		if (contentStream != null) {
			...
			throw new ResourceNotFoundException("Le template " + resourceName.getName() + " pour la bibliothèque "
					+ resourceName.getLibraryId() + " n'a pas pu être chargé");
		}
	}
	...
	public Template findTemplateByName(final ResourceName resourceName) {
		...
		// Recherche du template par nom
		final List<Template> templates = templateService.findTemplate(resourceName.getName(),
		...
		if (CollectionUtils.isEmpty(templates)) {
			throw new ResourceNotFoundException("TemplateResourceLoader: la ressource " + resourceName.getName()
					+ " pour la bibliothèque " + resourceName.getLibraryId() + " n'existe pas.");
		}
		else if (templates.size() > 1) {
			LOG.warn("Plusieurs templates existent avec le nom {} pour la bibliothèque {}", resourceName.getName(),
					resourceName.getLibraryId());
		}
