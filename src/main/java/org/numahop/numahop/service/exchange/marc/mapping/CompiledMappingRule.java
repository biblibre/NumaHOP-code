...
public final class CompiledMappingRule {
	...
	/**
	 * Filtrage des champs MARC: - les champs présents dans les expressions sont repris
	 * tels quels - les champs présents dans les conditions sont repris: -> s'ils ne sont
	 * pas trouvés exactement dans les expressions -> si une expression n'utilise pas une
	 * notation plus générique (par ex: expression 6xx et condition 606, la condition 606
	 * est ignorée)
	 * @return
	 */
			...
			conditionStatement.getMarcKeys()
				...
				.filter(key -> expressionStatement == null || expressionStatement.getMarcKeys()
					...
					.noneMatch(exprKey -> Objects.equals(exprKey, key)
							...
							// condition
							|| (exprKey.isXX() && exprKey.getTag().charAt(0) == key.getTag().charAt(0))))
	...
	public boolean isConstant() {
		return (conditionStatement == null || conditionStatement.getMarcKeys().isEmpty())
				&& (expressionStatement == null || expressionStatement.getMarcKeys().isEmpty());
	}
