 ...
 *
 * @author jbrunet Créé le 20 févr. 2017
 */
public class DatePublicationFormatter extends CustomScript {
	...
	public String getConfigScript() {
		return "def " + SCRIPT_NAME + "Filter = {\n" + "      String code -> script." + getCode()
				+ ".setSubfield((char)code)\n" + "}\n" + "def " + SCRIPT_NAME + "PositionDebut = {\n"
				+ "      int start -> script." + getCode() + ".setDateStartPosition(start)\n" + "}\n" + "def "
				+ SCRIPT_NAME + "PositionFin = {\n" + "      int end -> script." + getCode()
				+ ".setDateEndPosition(end)\n" + "}\n" + "def " + SCRIPT_NAME + "PositionTypeDate = {\n"
				+ "      int position -> script." + getCode() + ".setDateTypePosition(position)\n" + "}\n";
	}
	...
	/**
	 * Formattage du field au format ISO 8601 approché La seule différence avec le format
	 * ISO est la possibilité de XX pour les données inconnues Ex: 198X pour une année
	 * entre 1980 et 1989
	 * @param field
	 * @return
	 */
	public String format(final DataField field) {
		...
		Optional<Subfield> subfieldForDate = field.getSubfields()
			.stream()
			.filter(sub -> subfield == sub.getCode())
			.findFirst();
		...
		// On vérifie que la longeur de la chaîne récupérée est au moins égale à la
		// position de fin attendue
		...
		switch (typeDate) {
			...
			case 'k':
				// monographie avec date édition/diffusion et production : choix de
				// l'édition
			case 'i':
				// monographie avec date publication et copyright : choix de la date de
				// publication
			case 'h':
