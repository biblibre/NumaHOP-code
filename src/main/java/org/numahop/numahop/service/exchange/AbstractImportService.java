...
import org.numahop.numahop.domain.AbstractDomainObject_;
...
import org.numahop.numahop.domain.exchange.ImportedDocUnit_;
...
public abstract class AbstractImportService {
	...
	protected AbstractImportService(final DeduplicationService deduplicationService,
			final DocUnitService docUnitService, final EsDocUnitService esDocUnitService,
	...
	/**
	 * Mise à disposition des notices pré-importées dans le catalogue Les notices déjà
	 * importées ne sont pas modifiées
	 * @param defaultProcess gestion par défaut des unité documentaires pré-importées,
	 * sans doublon
	 * @param defaultDedupProcess gestion par défaut des unité documentaires
	 * pré-importées, avec doublon
	 */

	@Async
	public void processPreimportedDocUnitsAsync(final ImportReport report, final ImportedDocUnit.Process defaultProcess,
			final ImportedDocUnit.Process defaultDedupProcess) {
		processPreimportedDocUnits(report, defaultProcess, defaultDedupProcess);
	}

	private ImportReport processPreimportedDocUnits(ImportReport report, final ImportedDocUnit.Process defaultProcess,
			final ImportedDocUnit.Process defaultDedupProcess) {
		try {
			...
			do {
				...
				try {
					final Pageable pageable = importedUnits == null
							? PageRequest.of(0, BULK_SIZE, Sort.by(ImportedDocUnit_.identifier.getName()))
					...
					for (final ImportedDocUnit imp : importedUnits) {
						...
						// Process à effectuer: celui défini sur l'unité documentaire, ou
						// celui par
						// défaut
						final ImportedDocUnit.Process process = imp.getProcess() != null
								// Une action est définie sur l'unité importée
								? imp.getProcess()
								// Sinon on effectue l'action par défaut avec / sans
								// doublon
						...
						if (process == ImportedDocUnit.Process.IGNORE) {
							...
							report.incrementNbImp(-1); // on décrémente le compteur quand
														// on n'importe pas l'UD
							...
							if (hasDuplicates) {
								...
								// Remplacement de la notice existante par celle importée,
								// sans toucher à l'UD
								// pour conserver les données liées
								// (workflows, constats, ...)
			...
			LOG.info("Fin de l'import de {} unités documentaires, pré-importées à partir du fichier {}", nbProcessed,
	 ...
	 * @param replace supprimer les notices existantes
	 */
	public void merge(final ImportedDocUnit udReport, final ImportReport report, final Set<DocUnit> deletedUnits,
	...
	/**
	 * Mets à jour les infos de l'unité documentaire par rapport aux paramètres d'import
	 * (projet, lot)
	 */
	...
	/**
	 * Passe le statut des unités documentaires importées à "validé" Logue les erreurs de
	 * validation
	...
	private int setAvailable(final Collection<ImportedDocUnit> importedDocUnits) {
		...
		for (final DocUnit docUnit : docUnits) {
			try {
				...
				transactionService.rollbackTransaction(status); // Annulation de la
																// sauvegarde de l'unité
																// documentaire

				status = transactionService.startTransaction(false); // nouvelle
																		// transaction
																		// pour mettre à
																		// jour le
																		// rapport
				...
				importedDocUnits.stream()
					.filter(imp -> imp.getDocUnit() != null
							&& StringUtils.equals(imp.getDocUnit().getIdentifier(), docUnit.getIdentifier()))
					.findAny()
					.ifPresent(imp -> {
	...
	/**
	 * Traitement de unité documentaires pré-importées: recherche de doublons, validation
	 * utilisateur, import
	 * @param report
	 * @param parentReportId identifiant de l'import parent, pour gérer la création des
	 * relations hiérarchiques avec des parents provenant d'un autre import
	 * @param stepValidation
	 * @param stepDeduplication
	 * @param defaultDedupProcess
	 * @return
	 */
	protected ImportReport importDocUnit(ImportReport report, final String parentReportId, final boolean stepValidation,
		...
		if (stepValidation) {
			LOG.debug(
					"Pré-import des unités documentaires terminé; l'import débutera après validation par l'utilisateur");
	...
	protected ImportReport lookupDuplicates(final ImportReport report) {
		LOG.info("Recherche de doublons pour les unités documentaires pré-importées à partir du fichier {}",
				report.getFilesAsString());
		Page<ImportedDocUnit> importedUnits = null;
		int nbProcessed = 0;
		do {
			...
			try {
				...
				for (final ImportedDocUnit imp : importedUnits) {
					...
					if (!duplicated.isEmpty()) {
						LOG.debug("{} doublon(s) trouvé(s) pour l'unité documentaire ({}) {}", duplicated.size(),
								imp.getDocUnit().getIdentifier(), imp.getDocUnit().getLabel());
	...
	/**
	 * Création des liens hiérarchiques parent / enfant, les UD parentes ayant été
	 * importées dans un autre import
	 * @param report
	 * @param parentReportId identifiant de l'import des UD parentes
	 * @return
	 */
	protected ImportReport linkToParent(final ImportReport report, final String parentReportId) {
		LOG.info("Création des liens hiérarchiques pour les unités documentaires pré-importées à partir du fichier {}",
				report.getFilesAsString());
		...
		do {
			...
			try {
				...
				final List<ImportedDocUnit> parentUnits = importDocUnitService
					.findByReportIdentifierAndParentKeyIn(parentReportId, parentKeys);

				// création des liens parent / enfant
				for (final ImportedDocUnit imp : importedUnits) {
					final DocUnit docUnit = imp.getDocUnit();
					// L'unité documentaire a déjà été importée, ou le lien parent/enfant
					// n'existe
					// pas
					...
					parentUnits.stream()
						.filter(u -> StringUtils.equals(u.getParentKey(), imp.getParentKey()))
						.findAny()
						.ifPresent(parent -> {
		...
		do {
			final int[] result = transactionService.executeInNewTransactionWithReturn(() -> {
				...
				final Pageable pageable = pageRef.get() == null
						? PageRequest.of(0, esBulkSize, Sort.Direction.ASC, AbstractDomainObject_.identifier.getName())
						: pageRef.get().nextPageable();
				final Page<DocUnit> pageOfDocs = importDocUnitService.findDocUnitByImportReport(report, State.AVAILABLE,
						pageable);
				final List<DocUnit> docUnits = pageOfDocs.getContent();
				final List<String> docUnitIds = docUnits.stream()
					.map(DocUnit::getIdentifier)
					.collect(Collectors.toList());
				final List<String> recordIds = docUnits.stream()
					.flatMap(doc -> doc.getRecords().stream())
					.map(BibliographicRecord::getIdentifier)
					.collect(Collectors.toList());
			...
			nbDocUnits += result[0];
			nbRecords += result[0];
			LOG.trace("{} / {} unités documentaires (et {} notices) indexées", nbDocUnits,
					pageRef.get().getTotalElements(), nbRecords);
