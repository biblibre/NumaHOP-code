...
import org.numahop.numahop.domain.jaxb.oaidc.ObjectFactory;
...
public class OAIDcEntityHandler {
	...
	/**
	 * Parse le fichier xml d'entrée à la recherche d'éléments TAG_DC. Chaque élément
	 * TAG_DC de niveau supérieur donne lieu à un appel de process
	 * @param file
	...
	public void parse(final File file) throws JAXBException, ParserConfigurationException, SAXException, IOException {
		...
		// filter.setFeature("http://apache.org/xml/features/nonvalidating/load-external-dtd",
		// false);
	 ...
	 * a "c" element, it creates a new unmarshaller and unmarshals one {@link OaiDcType}.
	 * <p>
			...
			if (depth != 0) {
				...
				if (depth == 0) {
					...
					try {
						...
						LOG.error("Erreur lors du traitement de l'élémment \"oai_dc:dc\" à la ligne {}",
	...
	/**
	 * XMLFilter qui corrige le format invalide des fichiers de Calames: -> les
	 * déclarations des namespaces sont incorrectes (dc) ou absentes (oai_dc) -> oai_dc
	 * (namespace) est utilisé au lieu de oai_dc:dc (élément dc du namespace oai_dc)
	 */
