...
public class MarcMappingEvaluationService {
	...
	@Autowired
	public MarcMappingEvaluationService(@Qualifier("groovy") final ScriptEngine engine,
	...
	public boolean evalCondition(CompiledMappingRule rule, Map<String, Object> bindings) {
		try {
			...
			LOG.error(
					...
					StringUtils.join(bindings.keySet(), ", "), rule.getRuleCondition());
	...
	public Object evalExpression(CompiledMappingRule rule, Map<String, Object> bindings) {
		try {
			...
			LOG.error(
					...
					StringUtils.join(bindings.keySet(), ", "), rule.getRuleExpression());
			LOG.error(e.getMessage(), e);
			return null;
		}
	}
