...
/**
 * Formatter gérant les séparateurs entre sous-champs, en distinguant les sous-champs
 * suivant qu'ils ont le même tag ou non
 * <p>
...
public class ConcatWithSepFormatter extends CustomScript {
	...
	public String getConfigScript() {
		return "def " + SCRIPT_NAME + "Codes = {\n" + "      String... codes -> script." + getCode()
				+ ".setCodes((char[])codes)\n" + "}\n" + "def " + SCRIPT_NAME + "Inner = {\n"
				+ "      String sep -> script." + getCode() + ".setSepInner(sep)\n" + "}\n" + "def " + SCRIPT_NAME
				+ "Outer = {\n" + "      String sep -> script." + getCode() + ".setSepOuter(sep)\n" + "}\n";
	}
	...
	public String format(final DataField field) {
		...
		final List<Subfield> subfields = field.getSubfields()
			.stream()
			.filter(sub -> ArrayUtils.contains(codes, sub.getCode()))
