...
public class UIWorkflowService {
	...
	@Autowired
	public UIWorkflowService(final WorkflowService service, final DocUnitService docUnitService,
			final DigitalDocumentService digitalDocumentService, final UIDocUnitService uiDocUnitService) {
	...
	/**
	 * Indique si le user peut ou non effectuer la tache. Attention : systematiquement
	 * bloqué pour 1 prestataire: cette méthode doit être réservée à la vue UD détail -
	 * onglet Workflow.
	 * @param docUnitId
	 * @param key
	 * @param isUserPresta
	 ...
	 */
	@Transactional(readOnly = true)
	public BooleanValueDTO canCurrentUserProcessState(final String docUnitId, final WorkflowStateKey key,
			final boolean isUserPresta) {
	...
	/**
	 * Reinitialise les workflows pour relivraison et s'assure que cela ne provoque pas de
	 * blocage des autres workflows de la livraison d'origine.
	 * @param docIds
	 */
	@Transactional
			...
			workflow.getStates()
				...
				.filter(state -> {
					if (WorkflowStateKey.VALIDATION_CONSTAT_ETAT == keyState) {
						return service.areStatesRunning(id, WorkflowStateKey.VALIDATION_BORDEREAU_CONSTAT_ETAT,
