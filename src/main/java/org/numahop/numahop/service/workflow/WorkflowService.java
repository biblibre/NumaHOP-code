...
import org.numahop.numahop.service.util.NumahopCollectors;
...
public class WorkflowService {
	...
	public WorkflowService(final DocUnitWorkflowService docUnitWorkflowService,
			final WorkflowGroupService workflowGroupService, final DocUnitService docUnitService,
			final BibliographicRecordService recordService, final UserService userService,
			final ConditionReportService conditionReportService, final LotService lotService,
			final ProjectService projectService, final DocCheckHistoryService docCheckHistoryService,
	...
	public DocUnitWorkflow initializeWorkflow(final DocUnit doc, final WorkflowModel model, LocalDateTime startDate) {
		...
		// Constats d'état
		final DocUnitState validateConstatState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.VALIDATION_CONSTAT_ETAT);
		validateConstatState.initializeState(startDate, null, null);

		// Génération du bordereau
		final DocUnitState generateBordereauState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.GENERATION_BORDEREAU);
		generateBordereauState.initializeState(startDate, null, null);

		// Validation notice
		final DocUnitState validateBibliographicRecord = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.VALIDATION_NOTICES);
	...
	/**
	 * Initialisation du workflow pour les lots numériques Les étapes jusqu'au contrôles
	 * sont ignorées
	 * @param doc
	 * @param model
	 */
	@Transactional
	public DocUnitWorkflow initializeWorkflowForDigitalLot(final DocUnit doc, final WorkflowModel model,
		...
		final DocUnitState valBordState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.VALIDATION_BORDEREAU_CONSTAT_ETAT);
		valBordState.initializeState(startDate, startDate, WorkflowStateStatus.CANCELED);
		// Constats d'états
		final DocUnitState constatAvState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.CONSTAT_ETAT_AVANT_NUMERISATION);
		constatAvState.initializeState(startDate, startDate, WorkflowStateStatus.CANCELED);
		final DocUnitState constatApState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.CONSTAT_ETAT_APRES_NUMERISATION);
		...
		final DocUnitState livraisonState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.LIVRAISON_DOCUMENT_EN_COURS);
		livraisonState.initializeState(startDate, startDate, WorkflowStateStatus.CANCELED);
		final DocUnitState controleAutoState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.CONTROLES_AUTOMATIQUES_EN_COURS);
		controleAutoState.initializeState(startDate, startDate, WorkflowStateStatus.CANCELED);

		// Démarrage des étapes en cours
		final DocUnitState controleQualiteState = workflow
			.getFutureOrRunningByKey(WorkflowStateKey.CONTROLE_QUALITE_EN_COURS);
	...
	/**
	 * Met fin à un workflow. Toutes les étapes en cours et à venir passe au statut
	 * {@link WorkflowStateStatus#CANCELED} sauf l'étape
	 * {@link WorkflowStateKey#CLOTURE_DOCUMENT} qui passe à l'état
	 * {@link WorkflowStateStatus#FINISHED}
	 * @param docUnitId
	 */
	@Transactional
	...
	/**
	 * Termine les workflows des docs du projet annulé, et pose les statuts correspondants
	 * sur les entites concernees.
	 * @param docUnits
	 */
	@Transactional
	public void endWorkflowForCancelingProject(final Set<DocUnit> docUnits) {
		...
		docUnits.forEach(du -> {
			...
			final DocUnitState state = docUnit.getWorkflow()
				.getByKey(WorkflowStateKey.CLOTURE_DOCUMENT)
				.stream()
	...
	public void processState(final DocUnit doc, final WorkflowStateKey key, final User user) {
		...
		if (workflow == null) {
			...
			// Cas particulier CONSTAT ETAT HORS WORKFLOW : on peut valider l'etape non
			// requise (skipped)
			if (currentState == null && (WorkflowStateKey.VALIDATION_CONSTAT_ETAT == key
					|| WorkflowStateKey.CONSTAT_ETAT_AVANT_NUMERISATION == key
					|| WorkflowStateKey.CONSTAT_ETAT_APRES_NUMERISATION == key
					|| WorkflowStateKey.VALIDATION_BORDEREAU_CONSTAT_ETAT == key)) {
				currentState = workflow.getStates()
					.stream()
					.filter(st -> key == st.getKey() && WorkflowStateStatus.SKIPPED.equals(st.getStatus()))
					.findFirst()
					.orElse(null);

				if (currentState != null && currentState.getModelState().getGroup() == null) {
					// on a une state mais sans group de workflow => on force l'excecution
					// de la
					// tache (shunt controles )
			...
			if (currentState != null
					&& (!canUserProcessTask(userId, currentState) || !canStateBeProcessed(doc, currentState))) {
	...
	public void processAutomaticState(final String docUnitId, final WorkflowStateKey key) {
		...
		final DocUnitWorkflow workflow = doc.getWorkflow();
		if (workflow == null) {
			LOG.warn("Aucun workflow sur le document {} : impossible de réaliser automatiquement l'étape {}",
					doc.getPgcnId(), key.name());
		}
		else {
			DocUnitState currentState = workflow.getCurrentStateByKey(key);
			if (currentState != null && currentState.isWaiting()) {
				...
				LOG.error("Impossible de valider automatiquement la tâche {} pour le document {}", key.name(),
						doc.getPgcnId());
			}
	...
	public void rejectAutomaticState(final String docUnitId, final WorkflowStateKey key) {
		...
		if (workflow == null) {
			LOG.warn(
					"Aucun workflow sur le document : impossible de rejeter automatiquement l'étape {} pour le document {}",
					key.name(), doc.getPgcnId());
		}
		else {
			final DocUnitState currentState = workflow.getCurrentStateByKey(key);
			if (currentState != null && currentState.isWaiting()) {
				...
				LOG.error("Workflow : Impossible de refuser automatiquement la tâche {} pour le document {}",
						key.name(), doc.getPgcnId());
			}
			handleStatus(doc, workflow);
		}
	...
	/**
	 * Gestion des implications de l'avancement du workflow sur le statut des entités
	 * liées (lot, projet)
	 * @param doc
	 * @param workflow
	 */
	private void handleStatus(final DocUnit doc, final DocUnitWorkflow workflow) {
		...
		final DocUnitState state = workflow.getByKey(WorkflowStateKey.CLOTURE_DOCUMENT)
			.stream()
			.collect(NumahopCollectors.singletonCollector());
		...
		if (state.isDone()) {
			...
			final DocUnitState validationState = workflow.getByKey(WorkflowStateKey.VALIDATION_DOCUMENT)
				.stream()
				.collect(NumahopCollectors.singletonCollector());
			if (WorkflowStateStatus.CANCELED == validationState.getStatus()) {
				// Validation document annulée, on est en administration / terminer
				// workflow
			...
			// Gestion du lot
			// LOT : cloturé uniquement si tout OK (tout terminé et surtout sans erreur:
			// tous les docs doivent avoir 1 worflow terminé)
			...
			LOG.trace(
					"handleStatus - Boucle sur les docUnits du lot {} - total docs: {} - docs avec workflow en cours ou non demarré: {}",
					lot.getLabel(), lot.getDocUnits().size(), uncompletedWorkflows);
	...
	/**
	 * Vérification du droit de l'utilisateur de réaliser la tâche avec la
	 * {@link WorkflowStateKey} fournie
	 * @param userId
	 * @param docUnitId
	 * @param key
	 * @return
	...
	public boolean canUserProcessState(final String userId, final String docUnitId, final WorkflowStateKey key) {
		if (userId != null && docUnitId != null) {
			...
			final DocUnitWorkflow workflow = docUnit.getWorkflow();
			if (workflow == null) {
				LOG.debug("Aucun workflow sur le document : impossible de vérifier la faisabilité de l'étape {}",
						key.name());
			}
			else {
				final DocUnitState currentState = workflow.getCurrentStateByKey(key);
	...
	public boolean canStateBeProcessed(final DocUnit doc, final DocUnitState state) {
		switch (state.getKey()) {
			...
			case VALIDATION_CONSTAT_ETAT:
				...
				return conditionReportService.isConditionReportDetailPresentInDocUnit(doc.getIdentifier(),
						Type.LIBRARY_LEAVING);
	...
	/**
	 * Vérifie si la state est ignoree ou en cours. Valable uniquement pour les states non
	 * répétables
	 * @param docUnitId
	 * @param key
	 * @return
	...
	/**
	 * Vérifie si la state est finie et validée. Valable uniquement pour les states non
	 * répétables
	 * @param docUnitId
	 * @param key
	 * @return
	...
	/**
	 * Retourne les {@link DocUnitState} liés à un {@link WorkflowGroup} en cours ou à
	 * venir
	 * @param group
	 * @return
	 */
	...
	public void processState(final String docUnitId, final WorkflowStateKey key, final String userId) {
		processState(docUnitService.findOneWithAllDependenciesForWorkflow(docUnitId), key,
				userService.findByIdentifier(userId));
	}
	...
	public void rejectState(final String docUnitId, final WorkflowStateKey key, final String userId) {
		rejectState(docUnitService.findOneWithAllDependenciesForWorkflow(docUnitId), key,
				userService.findByIdentifier(userId));
	}

	/**
	 * Permet de déterminer si un workflow est en cours sur le document (impact sur les
	 * actions)
	 * @param docUnitId
	 * @return
	 */
	...
	public boolean isWorkflowRunning(final String docUnitId) {
		final List<DocUnitWorkflow> workflow = docUnitWorkflowRepository
			.findByDocUnitIdentifierIn(Collections.singletonList(docUnitId));
	...
	public boolean isCheckInProgress(final String docUnitId) {
		final List<DocUnitWorkflow> workflows = docUnitWorkflowRepository
			.findByDocUnitIdentifierIn(Collections.singletonList(docUnitId));
		if (workflows != null && !workflows.isEmpty()) {
			final DocUnitWorkflow workflow = workflows.get(0);
			final ArrayList<WorkflowStateKey> keys = new ArrayList<>(
					Arrays.asList(WorkflowStateKey.CONTROLE_QUALITE_EN_COURS, WorkflowStateKey.PREVALIDATION_DOCUMENT,
							WorkflowStateKey.PREREJET_DOCUMENT, WorkflowStateKey.VALIDATION_DOCUMENT));
	...
	public void forceReinitState(final String id, final DocUnitState state) {
		...
		// et retour arriere pour les states suivantes qui peuvent etre en cours ou en
		// attente
