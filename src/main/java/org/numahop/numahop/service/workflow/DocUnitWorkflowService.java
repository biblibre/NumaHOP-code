...
import static org.numahop.numahop.domain.workflow.WorkflowStateStatus.*;
...
import org.numahop.numahop.domain.workflow.state.ArchivageDocumentState;
import org.numahop.numahop.domain.workflow.state.ClotureDocumentState;
import org.numahop.numahop.domain.workflow.state.ConstatEtatApresNumerisationState;
import org.numahop.numahop.domain.workflow.state.ConstatEtatAvantNumerisationState;
import org.numahop.numahop.domain.workflow.state.ControlesAutomatiquesEnCoursState;
import org.numahop.numahop.domain.workflow.state.ControlesQualiteState;
import org.numahop.numahop.domain.workflow.state.DiffusionDocumentDigitalLibraryState;
import org.numahop.numahop.domain.workflow.state.DiffusionDocumentLocaleState;
import org.numahop.numahop.domain.workflow.state.DiffusionDocumentOmekaState;
import org.numahop.numahop.domain.workflow.state.DiffusionDocumentState;
import org.numahop.numahop.domain.workflow.state.GenerationBordereauState;
import org.numahop.numahop.domain.workflow.state.InitialisationDocumentState;
import org.numahop.numahop.domain.workflow.state.LivraisonDocumentEnCoursState;
import org.numahop.numahop.domain.workflow.state.NumerisationEnAttenteState;
import org.numahop.numahop.domain.workflow.state.PreRejetState;
import org.numahop.numahop.domain.workflow.state.PreValidationState;
import org.numahop.numahop.domain.workflow.state.RapportsControleState;
import org.numahop.numahop.domain.workflow.state.ReLivraisonDocumentEnCoursState;
import org.numahop.numahop.domain.workflow.state.ValidationBordereauConstatEtatState;
import org.numahop.numahop.domain.workflow.state.ValidationConstatEtatState;
import org.numahop.numahop.domain.workflow.state.ValidationDocumentState;
import org.numahop.numahop.domain.workflow.state.ValidationNoticesState;
import org.numahop.numahop.exception.PgcnValidationException;
import org.numahop.numahop.repository.workflow.DocUnitStateRepository;
 ...
 *
 * @author jbrunet Créé le 10 oct. 2017
 */
@Service
public class DocUnitWorkflowService {
	...
	@Autowired
	public DocUnitWorkflowService(final DocUnitWorkflowRepository docUnitWorkflowRepository,
			final DocUnitStateRepository docUnitStateRepository) {
	...
	/**
	 * Gestion de la relivraison Ajoute une étape spécifique de relivraison (hors modèle)
	 * au workflow et Reinitialise l'étape de contrôle automatique (modèle)
	 * @param instance
	 */
	@Transactional
	...
	public void resetDeliverStatesForDetachedDoc(final DocUnit docUnit) {
		...
		if (docUnit.getWorkflow() != null) {
			...
			if (workflow.getCurrentStateByKey(WorkflowStateKey.RELIVRAISON_DOCUMENT_EN_COURS) == null
					|| !workflow.getCurrentStateByKey(WorkflowStateKey.RELIVRAISON_DOCUMENT_EN_COURS)
						.isCurrentState()) {
				final DocUnitState state = new ReLivraisonDocumentEnCoursState();
	...
	/**
	 * Detricote le workflow pour revenir à l'état antérieur. !! ADMIN ONLY
	 * @param workflow
	 * @param key
	 */
	@Transactional
	...
	/**
	 * Retour au statut précédent pour rester cohérent... !! ADMIN ONLY
	 * @param docUnit
	 * @param statut
	 */
	 ...
	 * @param status
	 */
	private void reinitializeState(final DocUnitWorkflow workflow, final WorkflowStateKey key,
	...
	public List<DocUnitState> findAllRunningOrPendingStateForGroup(final WorkflowGroup group) {
		return docUnitStateRepository
			.findAllStateInStatusForGroupIdentifier(Arrays.asList(NOT_STARTED, PENDING, TO_SKIP, TO_WAIT), group);
	}
	 ...
	 */
	@Transactional(readOnly = true)
	public List<DocUnitWorkflow> findAll(final List<String> libraries, final List<String> projects,
			final List<String> lots, final List<String> deliveries, final List<WorkflowStateKey> pendingStates) {
		return docUnitWorkflowRepository.findPendingDocUnitWorkflows(libraries, projects, lots, deliveries,
				pendingStates, null);
	}
