package org.numahop.numahop.service.lot.ui;

import org.numahop.numahop.domain.document.DocUnit;
import org.numahop.numahop.domain.dto.audit.AuditLotRevisionDTO;
...
import org.numahop.numahop.service.lot.mapper.UILotMapper;
...
public class UILotService {
	...
	@Autowired
	public UILotService(final LotService lotService, final UILotMapper uiLotMapper, final AccessHelper accessHelper,
	...
	/**
	 * Vérifie que le lot est dispo pour une livraison. - lots physiques en cours (déjà
	 * filtrés par la requete) - lots digitaux créés - lots digitaux en cours avec un doc
	 * workflow en attente de relivraison
	 * @param lot
	 * @return
	 */
	...
	private List<Lot> filterLots(final List<Lot> lotsIn) {
		...
		if (currentUser != null && User.Category.PROVIDER.equals(currentUser.getCategory())) {
			return lotsIn.stream()
				.filter(lot -> lot.getProvider() != null
						&& StringUtils.equals(lot.getProvider().getIdentifier(), currentUser.getIdentifier()))
	...
	@Transactional(readOnly = true)
	public Page<SimpleLotDTO> search(final String search, final List<String> libraries, final List<String> projects,
			final boolean active, final List<LotStatus> lotStatuses, final Integer docNumber,
			final List<String> fileFormats, final List<String> identifiers, final Integer page, final Integer size,
			final List<String> sorts) {
		final Page<Lot> lots = lotService.search(search, libraries, projects, active, lotStatuses, null, docNumber,
				fileFormats, identifiers, page, size, sorts);
		return lots.map(SimpleLotMapper.INSTANCE::lotToSimpleLotDTO);
	}

	@Transactional(readOnly = true)
	public List<AuditLotRevisionDTO> getLotsForWidget(final LocalDate fromDate, final List<String> libraries,
