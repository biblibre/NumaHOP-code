...
public final class GZipResponseUtil {
	...
	/**
	 * Gzipping an empty file or stream always results in a 20 byte output This is in java
	 * or elsewhere.
	 * <p/>
	 * On a unix system to reproduce do <code>gzip -n empty_file</code>. -n tells gzip to
	 * not include the file name. The resulting file size is 20 bytes.
	 * <p/>
	 * Therefore 20 bytes can be used indicate that the gzip byte[] will be empty when
	 * ungzipped.
	 */
	...
	/**
	 * Checks whether a gzipped body is actually empty and should just be zero. When the
	 * compressedBytes is {@link #EMPTY_GZIPPED_CONTENT_SIZE} it should be zero.
	...
	/**
	 * Performs a number of checks to ensure response saneness according to the rules of
	 * RFC2616:
	 * <ol>
	 * <li>If the response code is
	 * {@link jakarta.servlet.http.HttpServletResponse#SC_NO_CONTENT} then it is illegal
	 * for the body to contain anything. See
	 * http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html#sec10.2.5
	 * <li>If the response code is
	 * {@link jakarta.servlet.http.HttpServletResponse#SC_NOT_MODIFIED} then it is illegal
	 * for the body to contain anything. See
	 ...
	 * decompress it.
	 * </p>
	 * @param response the response which will have a header added to it. I.e this method
	 * changes its parameter
	 * @throws GzipResponseHeadersNotModifiableException Either the response is committed
	 * or we were called using the include method from a
