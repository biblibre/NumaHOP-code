...
/**
 * Regroupement des vérifications de traitement lié au workflow (déconnecté de
 * l'utilisateur)
 */
@Component
public class WorkflowAccessHelper {
	...
	@Autowired
	public WorkflowAccessHelper(final LotService lotService, final WorkflowService workflowService,
	...
	/**
	 * Permet de déterminer si le document est livrable (étape du workflow à livraison ou
	 * pas de workflow)
	 * @param docUnitId
	 * @return
	 */
	...
	public boolean canDocUnitBeDelivered(final String docUnitId) {
		return !workflowService.isWorkflowRunning(docUnitId)
				|| workflowService.isStateRunning(docUnitId, WorkflowStateKey.LIVRAISON_DOCUMENT_EN_COURS)
	...
	public boolean canLotBeValidated(final String lotId) {
		...
		if (lot.getDocUnits() == null || lot.getDocUnits().isEmpty()) {
			...
			throw new PgcnBusinessException(
					builder.reinit().setCode(PgcnErrorCode.WORKFLOW_LOT_DOCUNIT_MISSING).build());
		}
		return Lot.LotStatus.CREATED.equals(lot.getStatus());
	}

	/**
	 * Autorisation de suppression d'un constat d'état si le workflow n'est pas avancé
	 * dessus
	 * @param docUnitId
	 * @return
	 */
	...
	public boolean canConstatBeDeleted(final String docUnitId) {
		...
		return workflowService.isStateSkippedOrRunning(docUnitId, WorkflowStateKey.VALIDATION_CONSTAT_ETAT)
				&& workflowService.isStateSkippedOrRunning(docUnitId, WorkflowStateKey.CONSTAT_ETAT_AVANT_NUMERISATION)
				&& workflowService.isStateSkippedOrRunning(docUnitId, WorkflowStateKey.CONSTAT_ETAT_APRES_NUMERISATION);
	}

	/**
	 * Vérifie si un élément lié à une étape peut être supprimée, ie l'étape n'est pas
	 * terminée
	 * @param docUnitId
	 * @param key
	 * @return
	...
	public boolean canCheckBePerformed(final String docUnitId) {
		return workflowService.isStateRunning(docUnitId, WorkflowStateKey.CONTROLE_QUALITE_EN_COURS)
				|| workflowService.isStateRunning(docUnitId, WorkflowStateKey.VALIDATION_DOCUMENT);
	}
	...
	/**
	 * Permet de savoir si on peut toujours affecter la docUnit liée au constat à un train
	 * (1 constat et 1 workflow non encore demarré ou n'ayant pas depassé l'etape 'En
	 * attente de numerisation)
	 * @param identifier
	 * @return
	 */
	@Transactional(readOnly = true)
	public StateIsDoneDTO canChangeTrain(final String identifier) {
		...
		result.setDone((!workflowService.isWorkflowRunning(identifier))
				|| (workflowService.isWorkflowRunning(identifier) && workflowService.areStatesRunning(identifier,
						WorkflowStateKey.GENERATION_BORDEREAU, WorkflowStateKey.VALIDATION_CONSTAT_ETAT,
