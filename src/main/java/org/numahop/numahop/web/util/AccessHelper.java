...
import static org.numahop.numahop.service.user.UserService.*;
...
/**
 * Outils permettant de vérifier que l'utilisateur courant ait bien accès aux ressources
 * qu'il demande
 * <p>
...
public class AccessHelper {
	...
	public AccessHelper(final BibliographicRecordService bibliographicRecordService,
			final DeliveryService deliveryService, final DigitalDocumentService digitalDocumentService,
			final DocUnitService docUnitService, final LotService lotService, final ProjectService projectService,
			final TrainService trainService, final UserService userService,
	...
	public Collection<Train> filterTrains(final Iterable<String> identifiers) {
		...
		return checkCurrentUser(currentUser).map(check -> check ? trains : Collections.<Train>emptyList())
			.orElseGet(
					() -> trains.stream().filter(train -> checkTrain(train, currentUser)).collect(Collectors.toList()));
	}
	...
	public Collection<BibliographicRecord> filterBibliographicRecords(final List<String> identifiers) {
		...
		return checkCurrentUser(currentUser)
			.map(check -> check ? records : Collections.<BibliographicRecord>emptyList())
	...
	public boolean checkConditionReportDetail(final String identifier) {
		final CustomUserDetails currentUser = SecurityUtils.getCurrentUser();
		return checkCurrentUser(currentUser).orElseGet(() -> {
			final ConditionReportDetail conditionReportDetail = conditionReportDetailService
	...
	public List<ConditionReportDetail> filterConditionReportDetails(final List<String> identifiers) {
		final CustomUserDetails currentUser = SecurityUtils.getCurrentUser();
		final List<ConditionReportDetail> conditionReportDetails = conditionReportDetailService
			.findByIdentifierIn(identifiers);

		return checkCurrentUser(currentUser)
			.map(check -> check ? conditionReportDetails : Collections.<ConditionReportDetail>emptyList())
	...
	public Collection<DigitalDocument> filterDigitalDocuments(final Iterable<String> identifiers) {
		...
		return checkCurrentUser(currentUser)
			.map(check -> check ? digitalDocs : Collections.<DigitalDocument>emptyList())
	 ...
	 * @param currentUser
	 * @return false si on n'a pas affaire à un user connecté, true si le user est super
	 * user, empty si une vérification plus poussée des droits est nécessaire
	 */
	@Transactional(readOnly = true)
	...
	/**
	 * Droits d'accès au projet: - même bib + presta - bib associée - intervenant
	 * @param project
	 * @param currentUser
	 * @return
	 */
	private boolean checkProject(final Project project, final CustomUserDetails currentUser) {
		...
		(checkCurrentLibrary(project.getLibrary(), currentUser) && checkProjectForProvider(project, currentUser))
				...
				|| (checkAssociatedLibraries(project.getAssociatedLibraries(), currentUser)
						&& checkAssociatedUsers(project.getAssociatedUsers(), currentUser)));
	}

	/**
	 * Droits d'accès au projet: - même bib (sans vérif presta) - bib associée -
	 * intervenant
	 * @param project
	 * @param currentUser
	 * @return
	...
	/**
	 * Droits d'accès au lot: - même bib + presta - bib associée - intervenant
	 * @param lot
	 * @param currentUser
	 * @return
	 */
	...
	/**
	 * Droits d'accès au lot: - même bib (sans vérif presta) - bib associée - intervenant
	 * @param lot
	 * @param currentUser
	 * @return
	 */
	...
	private boolean checkConditionReportDetail(final ConditionReportDetail detail,
	 ...
	 * @param project
	 * @param currentUser
	 * @return true si currentUser est un presta et vérifie les droits presta sur le
	 * projet
	 */
		...
		if (currentUser.getCategory() != null) {
			switch (currentUser.getCategory()) {
				case PROVIDER:
					...
					return project.getProvider() != null
							&& StringUtils.equals(currentUser.getIdentifier(), project.getProvider().getIdentifier())
	 ...
	 * @param lot
	 * @param currentUser
	 * @return true si currentUser n'est pas un presta, ou vérifie les droits presta sur
	 * le lot
	 */
			...
			switch (currentUser.getCategory()) {
				case PROVIDER:
					return checkUser(lot.getProvider(), currentUser) || (lot.getProvider() == null
							&& lot.getProject() != null && checkUser(lot.getProject().getProvider(), currentUser));
				case OTHER:
					break;
			}
		}
	...
	/**
	 * Vérification des bibliothèques associées L'accès est restreint à la liste fournie
	 * @param allowedLibraries
	 * @param currentUser
	 * @return
	 */
	private boolean checkAssociatedLibraries(final Collection<Library> allowedLibraries,
			final CustomUserDetails currentUser) {
		return CollectionUtils.isNotEmpty(allowedLibraries) && allowedLibraries.stream()
			.anyMatch(lib -> StringUtils.equals(lib.getIdentifier(), currentUser.getLibraryId()));
	}
	...
	/**
	 * Vérification des intervenants Si la liste d'intervenants est vide, l'accès est
	 * autorisé Sinon l'accès est restreint à la liste fournie
	 * @param allowedUsers
	 * @param currentUser
	 * @return
	 */
	private boolean checkAssociatedUsers(final Collection<User> allowedUsers, final CustomUserDetails currentUser) {
		return CollectionUtils.isEmpty(allowedUsers) || allowedUsers.stream()
			.anyMatch(u -> StringUtils.equals(u.getIdentifier(), currentUser.getIdentifier()));
	}
