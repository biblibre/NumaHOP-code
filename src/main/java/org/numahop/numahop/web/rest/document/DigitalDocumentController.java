...
import org.numahop.numahop.web.util.WorkflowUserAccessHelper;
...
public class DigitalDocumentController extends AbstractRestController {
	...
	public DigitalDocumentController(final UIDigitalDocumentService uiDigitalDocumentService,
			final DigitalDocumentService digitalDocumentService, final ViewerService viewerService,
			final AccessHelper accessHelper, final LibraryAccesssHelper libraryAccesssHelper,
			final WorkflowAccessHelper workflowAccessHelper, final WorkflowUserAccessHelper workflowUserAccessHelper,
	...
	/**
	 * Retourne le thumbnail correspondant à l'identifiant de doc numérique et à la page
	 * donnée
	 */
	@RequestMapping(value = "/{identifier}", params = { "thumbnail" }, method = RequestMethod.GET,
			produces = MediaType.IMAGE_JPEG_VALUE)
	@Timed
	public ResponseEntity<?> getThumbnailById(final HttpServletRequest request, final HttpServletResponse response,
			@PathVariable final String identifier, @RequestParam final int pageNumber) throws PgcnTechnicalException {
	...
	/**
	 * Retourne le format VIEW correspondant à l'identifiant de doc numérique et à la page
	 * donnée
	 */
	@RequestMapping(value = "/{identifier}", params = { "view" }, method = RequestMethod.GET,
			produces = MediaType.IMAGE_JPEG_VALUE)
	@Timed
	@RolesAllowed(CHECK_HAB3)
	public ResponseEntity<?> getViewById(final HttpServletRequest request, final HttpServletResponse response,
			@PathVariable final String identifier, @RequestParam final int pageNumber) throws PgcnTechnicalException {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	public ResponseEntity<DigitalDocumentDTO> getByIdWithPageCount(final HttpServletRequest request,
	...
	@Timed
	public ResponseEntity<CheckConfigurationDTO> getConfigurationCheck(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	@Timed
	public ResponseEntity<Map<String, String>> getDeliveryNotes(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	@Timed
	public ResponseEntity<String> getFilename(final HttpServletRequest request, @PathVariable final String identifier,
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	public ResponseEntity<String[]> getMasterPdfInfos(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	@Timed
	public ResponseEntity<List<String>> getFilenamesWithErrors(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	@Timed
	public ResponseEntity<List<Integer>> getFileNumbersWithErrors(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	public ResponseEntity<?> endChecks(final HttpServletRequest request, @PathVariable final String identifier,
			@RequestParam final boolean checksOK, @RequestParam final boolean sampling) {
	...
	public ResponseEntity<DigitalDocumentDTO> update(final HttpServletRequest request,
			@RequestBody final DigitalDocumentDTO dto) {
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	public ResponseEntity<Collection<SimpleDigitalDocumentDTO>> getAllDocumentsToCheck(
			final HttpServletRequest request) {
		final Collection<SimpleDigitalDocumentDTO> digitalDocuments = uiDigitalDocumentService.getAllDocumentsToCheck();
		final List<SimpleDigitalDocumentDTO> filteredDocs = filterDocsDTOs(digitalDocuments,
				SimpleDigitalDocumentDTO::getIdentifier);
	...
	public ResponseEntity<SimpleDocPageDTO> getPage(final HttpServletRequest request,
			@PathVariable final String identifier, @RequestParam final int pageNumber) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	public ResponseEntity<Page<SimpleListDigitalDocumentDTO>> search(final HttpServletRequest request,
		...
		return createResponseEntity(uiDigitalDocumentService.search(search, status, filteredLibraries, projects, lots,
				trains, deliveries, dateFrom, dateTo, dateLimitFrom, dateLimitTo, searchPgcnId, searchTitre,
				searchRadical, searchFileFormats, searchMaxAngles, searchPageFrom, searchPageTo, searchPageCheckFrom,
				searchPageCheckTo, searchMinSize, searchMaxSize, validated, page, size, sorts));
	}
	...
	private <T> List<T> filterDocsDTOs(final Collection<T> docs, final Function<T, String> getIdentifierFn) {
		return accessHelper.filterDigitalDocuments(docs.stream().map(getIdentifierFn).collect(Collectors.toList()))
			...
			.map(doc -> docs.stream()
				.filter(d -> StringUtils.equals(getIdentifierFn.apply(d), doc.getIdentifier()))
				.findAny())
	...
	@Timed
	public ResponseEntity<Map<String, String>> getPiecesNumber(final HttpServletRequest request,
			@PathVariable final String identifier) {
		if (!accessHelper.checkDigitalDocument(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
