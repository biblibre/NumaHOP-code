package org.numahop.numahop.web.rest.delivery;

import static org.numahop.numahop.domain.delivery.Delivery.DeliveryPayment.PAID;
...
import org.numahop.numahop.domain.dto.delivery.*;
...
public class DeliveryController extends AbstractRestController {
	...
	@Autowired
	public DeliveryController(final UIDeliveryService uiDeliveryService, final DeliveryService deliveryService,
			final DeliveryReportingService deliveryReportingService, final EsDeliveryService esDeliveryService,
	...
	public ResponseEntity<Page<SimpleDeliveryDTO>> search(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiDeliveryService.search(search, filteredLibraries, filteredProjects, lots,
				providers, status, dateFrom, dateTo, page, size), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<AuditDeliveryRevisionDTO>> getDeliveriesForWidget(final HttpServletRequest request,
		...
		// Chargement
		final List<AuditDeliveryRevisionDTO> revisions = uiDeliveryService.getDeliveriesForWidget(fromDate,
				filteredLibraries, filteredProjects, lots, status, false);
		// Réponse
		return new ResponseEntity<>(revisions, HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "sampledWidget", "from" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DEL_HAB0)
	public ResponseEntity<List<AuditDeliveryRevisionDTO>> getSampledDeliveriesForWidget(
			final HttpServletRequest request,
			@DateTimeFormat(pattern = "yyyy-MM-dd") @RequestParam(name = "from") final LocalDate fromDate,
			@RequestParam(value = "library", required = false) final List<String> libraries,
			@RequestParam(value = "project", required = false) final List<String> projects,
			@RequestParam(value = "lot", required = false) final List<String> lots, @RequestParam(value = "status",
					defaultValue = "TO_BE_CONTROLLED") final List<Delivery.DeliveryStatus> status) {
		...
		// Chargement
		final List<AuditDeliveryRevisionDTO> revisions = uiDeliveryService.getDeliveriesForWidget(fromDate,
				filteredLibraries, filteredProjects, lots, status, true);
		// Réponse
		return new ResponseEntity<>(revisions, HttpStatus.OK);
	}
	 ...
	 * @param lockedDocs
	 * @param metaDatas
	 * @param createDocs si true, création des UD associées aux metaDatas
	 * @return
	 */
	...
	@Timed
	@RolesAllowed({ DEL_HAB5, DEL_HAB5_2 })
	public ResponseEntity<PreDeliveryDTO> predeliver(final HttpServletRequest request, @PathVariable final String id,
			@RequestParam(name = "create_docs", required = false, defaultValue = "false") final boolean createDocs) {
		if (!accessHelper.checkDelivery(id)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	...
	@Timed
	@RolesAllowed(DEL_HAB0)
	public void generateSlipCSV(final HttpServletResponse response, @PathVariable final String id,
	...
	@Timed
	@RolesAllowed(DEL_HAB0)
	public void generateSlipLotCSV(final HttpServletRequest request, final HttpServletResponse response,
	...
	@Timed
	@RolesAllowed(DEL_HAB0)
	public void generateSlipLotPDF(final HttpServletRequest request, final HttpServletResponse response,
			@PathVariable final String id) throws PgcnTechnicalException {
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "deliverySample" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DEL_HAB0)
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "checkConfig" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "deliveryReport" },
			produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
	@Timed
	@RolesAllowed(DEL_HAB0)
	public ResponseEntity<?> getDeliveryReport(final HttpServletResponse response, @PathVariable final String id)
			throws PgcnTechnicalException {
	...
	@Timed
	public ResponseEntity<DeliveryDTO> create(final HttpServletRequest request,
			@RequestBody final ManualDeliveryDTO delivery) throws PgcnException {
		...
		esDeliveryService.indexAsync(savedDelivery.getIdentifier()); // Moteur de
																		// recherche
	...
	@Timed
	public ResponseEntity<DeliveryDTO> update(final HttpServletRequest request,
			@RequestBody final ManualDeliveryDTO delivery) throws PgcnException {
		// Droits d'accès au lot
		...
		// mode de livraison, le lot de rattachement (#874)
		if (!request.isUserInRole(DEL_HAB8)) {
	...
	private boolean hasProtectedChanges(final ManualDeliveryDTO newDelivery, final Delivery oldDelivery) {
		...
		return !StringUtils.equals(newDelivery.getPayment(), oldDelivery.getPayment().name())
				...
				// lot
				|| !Objects.equals(newDelivery.getLot() != null ? newDelivery.getLot().getIdentifier() : null,
						oldDelivery.getLot() != null ? oldDelivery.getLot().getIdentifier() : null);
	...
	public ResponseEntity<Collection<SimpleDeliveryDTO>> findAllSimpleDTO() {
		final Collection<Delivery> delivs = accessHelper.filterDeliveries(null); // charge
																					// +
																					// filtre
																					// toutes
																					// les
																					// livraisons
		final Collection<SimpleDeliveryDTO> delivsDTO = delivs.stream()
			.map(DeliveryMapper.INSTANCE::deliveryToSimpleDeliveryDTO)
			.collect(Collectors.toList());
		return createResponseEntity(delivsDTO);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "previouscheckslip" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DEL_HAB0)
	public ResponseEntity<Collection<SimpleDeliveryDTO>> getPreviousCheckSlips(
			@RequestParam(value = "digitalDocIdentifier") final String digitalDocIdentifier) {
		if (!accessHelper.checkDigitalDocument(digitalDocIdentifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
		return new ResponseEntity<>(uiDeliveryService.findPreviousDeliveriesForCheckSlips(digitalDocIdentifier),
				HttpStatus.OK);
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "duplicate" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DEL_HAB2, DEL_HAB8 })
	...
	@RequestMapping(method = RequestMethod.GET, params = { "filterByProjectsLots" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Collection<SimpleDeliveryDTO>> getSimpleByProjectsLots(
			@RequestParam(value = "filteredProjects", required = false) final List<String> projects,
		...
		deliveries = accessHelper
			.filterDeliveries(deliveries.stream().map(Delivery::getIdentifier).collect(Collectors.toList()));
		return createResponseEntity(deliveries.stream()
			.map(DeliveryMapper.INSTANCE::deliveryToSimpleDeliveryDTO)
			.collect(Collectors.toList()));
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "forViewer" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DEL_HAB0 })
	...
	@Timed
	@RolesAllowed({ DEL_HAB0 })
	public ResponseEntity<List<DeliveryDTO>> findAllForProject(
	...
	@RequestMapping(method = RequestMethod.GET, params = { "docUnit", "latest" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DEL_HAB0 })
	...
	@Timed
	@RolesAllowed({ DEL_HAB0 })
	public ResponseEntity<Map<String, Object>> getDeliveryStatus(final HttpServletRequest request,
			@PathVariable final String id) {
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "detachDoc" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<DeliveryDTO> detachDigitalDoc(@PathVariable final String id,
			@RequestBody final DigitalDocumentDTO docToDetach) throws PgcnException {
		// Droits d'accès
		if (!accessHelper.checkDelivery(id)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "delivprogress" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DEL_HAB0 })
	public ResponseEntity<List<String>> getDeliveryProgress(@PathVariable final String id,
			@RequestParam(value = "digitalId", required = false) final String digitalId) {
		String prog = "0";
		if (StringUtils.isNotBlank(id) && DeliveryController.PROGRESS_VALUES.containsKey(id)) {
	 ...
	 */
	@RolesAllowed({ DEL_HAB0 })
	@RequestMapping(method = RequestMethod.GET, params = { "diskspace", "widget" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Map<String, Long>> getDiskInfos(final HttpServletRequest request,
			@RequestParam(value = "libraries", required = true) final List<String> libraries) {
