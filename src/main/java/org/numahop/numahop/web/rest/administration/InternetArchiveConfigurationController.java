...
import org.numahop.numahop.service.administration.InternetArchiveConfigurationService;
...
public class InternetArchiveConfigurationController extends AbstractRestController {
	...
	public InternetArchiveConfigurationController(final InternetArchiveConfigurationService iaConfigurationService,
			final LibraryAccesssHelper libraryAccesssHelper, final InternetArchiveCollectionService iaCollectionService,
	...
	@RequestMapping(method = RequestMethod.GET, params = { "configurations", "library" },
	...
	public ResponseEntity<Page<InternetArchiveConfigurationDTO>> search(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		// Recherche
		final Page<InternetArchiveConfigurationDTO> results = iaConfigurationService.search(search, filteredLibraries,
				page, size);
	...
	@Timed
	@RolesAllowed({ CONF_INTERNET_ARCHIVE_HAB0 })
	public ResponseEntity<InternetArchiveConfiguration> getById(final HttpServletRequest request,
	...
	public ResponseEntity<InternetArchiveConfiguration> udpate(final HttpServletRequest request,
		...
		// Vérification des droits d'accès par rapport à la bibliothèque de
		// l'utilisateur, pour le conf existant
