...
public class UserController extends AbstractRestController {
	...
	@Autowired
	public UserController(final UserService userService, final UIUserService uiUserService,
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "change_password" },
	...
	public ResponseEntity<UserDTO> create(final HttpServletRequest request, @RequestBody final UserCreationDTO user)
	...
	public ResponseEntity<Page<SimpleUserDTO>> search(final HttpServletRequest request,
		...
		final boolean filterProviders = currentUser == null
				|| Objects.equals(currentUser.getCategory(), User.Category.PROVIDER);

		return new ResponseEntity<>(uiUserService.search(search, initiale, active, filterProviders, filteredLibraries,
				categories, roles, page, size), HttpStatus.OK);
	}
	...
	public ResponseEntity<UserDTO> update(final HttpServletRequest request, @RequestBody final UserDTO user)
			throws PgcnException {
	 ...
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "signature" },
			produces = MediaType.IMAGE_PNG_VALUE)
	@Timed
	@RolesAllowed(USER_HAB0)
	public void downloadSignature(final HttpServletResponse response, @PathVariable("id") final String userId)
			throws PgcnTechnicalException {
	...
	@Timed
	@RolesAllowed(USER_HAB0)
	public void downloadThumbnail(final HttpServletResponse response, @PathVariable("id") final String userId)
			throws PgcnTechnicalException {
		final User user = userService.getOne(userId);
		if (user == null) {
			response.setStatus(HttpServletResponse.SC_NOT_FOUND);
	 ...
	 * @throws PgcnTechnicalException
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "signexists" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	 ...
	 * @return
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE, params = { "signature" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ USER_HAB2, USER_HAB6 })
	public void deleteSignature(final HttpServletRequest request, final HttpServletResponse response,
	 ...
	 * @return
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "signature" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ USER_HAB2, USER_HAB6 })
	public void uploadSignature(final HttpServletRequest request, final HttpServletResponse response,
			@PathVariable("id") final String userId, @RequestParam(name = "file") final List<MultipartFile> files) {
		// Droits d'accès: autres profils
		if (request.isUserInRole(USER_HAB2)) {
			if (!accessHelper.checkUser(userId)) {
				response.setStatus(HttpServletResponse.SC_FORBIDDEN);
