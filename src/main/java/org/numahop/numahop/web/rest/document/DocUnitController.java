...
import org.numahop.numahop.service.document.ui.UIDocUnitService;
...
public class DocUnitController extends AbstractRestController {
	...
	@Autowired
	public DocUnitController(final DocUnitService docUnitService, final UIDocUnitService uiDocUnitService,
			final EsDocUnitService esDocUnitService, final AccessHelper accessHelper,
			final LibraryAccesssHelper libraryAccesssHelper, final LockService lockService,
	...
	public ResponseEntity<DocUnitDTO> create(final HttpServletRequest request, @RequestBody final DocUnitDTO doc)
			throws PgcnException {
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB3)
	public ResponseEntity<Collection<DocUnitDeletedReportDTO>> deleteDocUnitsProject(
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB3)
	public ResponseEntity<Collection<DocUnitDeletedReportDTO>> unlink(final HttpServletResponse response,
			@RequestBody final List<String> ids) {
	...
	public ResponseEntity<Page<SimpleDocUnitDTO>> search(final HttpServletRequest request,
			...
			@RequestParam(value = "hasDigitalDocuments", defaultValue = "false",
					required = false) final boolean hasDigitalDocuments,
			...
			@RequestParam(value = "nonArchivable", defaultValue = "false",
					required = false) final boolean nonArchivable,
			@RequestParam(value = "distributed", defaultValue = "false", required = false) final boolean distributed,
			@RequestParam(value = "nonDistributed", defaultValue = "false",
					required = false) final boolean nonDistributed,
			@RequestParam(value = "distributable", defaultValue = "false",
					required = false) final boolean distributable,
			@RequestParam(value = "nonDistributable", defaultValue = "false",
					required = false) final boolean nonDistributable,
		...
		return new ResponseEntity<>(uiDocUnitService.search(search, hasDigitalDocuments, active, archived, nonArchived,
				archivable, nonArchivable, distributed, nonDistributed, distributable, nonDistributable,
				filteredLibraries, projects, lots, statuses, lastModifiedDateFrom, lastModifiedDateTo, createdDateFrom,
				createdDateTo, page, size, sorts), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "searchAllForProject", "project" },
	...
	@RequestMapping(method = RequestMethod.POST, params = { "searchAsList" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DOC_UNIT_HAB0 })
	public ResponseEntity<Page<SimpleListDocUnitDTO>> searchAsList(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiDocUnitService.searchAsList(requestParams.getSearch(),
				requestParams.isHasDigitalDocuments(), requestParams.isActive(), requestParams.isArchived(),
				requestParams.isNonArchived(), requestParams.isArchivable(), requestParams.isNonArchivable(),
				requestParams.isDistributed(), requestParams.isNonDistributed(), requestParams.isDistributable(),
				requestParams.isNonDistributable(), filteredLibraries, requestParams.getProjects(),
				requestParams.getLots(), requestParams.getTrains(), requestParams.getStatuses(),
				requestParams.getLastModifiedDateFrom(), requestParams.getLastModifiedDateTo(),
				requestParams.getCreatedDateFrom(), requestParams.getCreatedDateTo(), requestParams.getFilter(), page,
				size, sorts), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.POST, params = { "searchAsMinList" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DOC_UNIT_HAB0 })
	public ResponseEntity<List<MinimalListDocUnitDTO>> searchAsMinList(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiDocUnitService.searchAsMinList(requestParams.getSearch(),
				requestParams.getLibraries(), requestParams.getProjects(), requestParams.getLots(),
				requestParams.getTrains(), requestParams.getStatuses()), HttpStatus.OK);
	}
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public ResponseEntity<DocUnitDTO> update(@RequestBody final DocUnitDTO doc)
			throws PgcnException, PgcnLockException {
		// Droits d'accès
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	public ResponseEntity<List<SummaryDocUnitWithLotDTO>> findAllForProject(
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	public ResponseEntity<List<DocUnitDTO>> getChildrenByParentId(
			@RequestParam(name = "parent") final String parentId) {
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public ResponseEntity<DocUnitDTO> addChildren(@RequestParam(name = "parent") final String parentId,
			@RequestBody final List<String> children) {
		if (!accessHelper.checkDocUnit(parentId)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	 ...
	 * @return
	 */
	@RequestMapping(method = RequestMethod.POST, params = { "removechild" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public ResponseEntity<DocUnitDTO> addSibling(@RequestParam(name = "siblingid") final String id,
			@RequestBody final List<String> siblingIds) {
		if (!accessHelper.checkDocUnit(id)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
	 ...
	 * @return
	 */
	@RequestMapping(method = RequestMethod.POST, params = { "removesibling" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	...
	/**
	 * Télécharge une archive ZIP contenant les fichiers demandés pour les unités
	...
	public void massExport(final HttpServletResponse response, @RequestParam(name = "docs") final List<String> docs,
		...
		try {
			writeResponseHeaderForDownload(response, "application/zip", null,
					DateUtils.formatDateToString(LocalDateTime.now(), "yyyy-MM-dd HH-mm-ss")
							+ (StringUtils.isNotBlank(pgcnId) ? "_" + pgcnId : "") + "_export.zip");
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public void lock(final HttpServletResponse response, @PathVariable final String identifier)
	...
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public void unlock(final HttpServletResponse response, @PathVariable final String identifier)
			throws PgcnLockException {
		final DocUnit docUnit = docUnitService.findOne(identifier);
		// pas trouvé
		if (docUnit == null) {
	...
	@RequestMapping(value = "/{identifier}", method = RequestMethod.POST, params = { "inactiveDoc" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	@Timed
	@RolesAllowed(DOC_UNIT_HAB2)
	public ResponseEntity<DocUnitDTO> inactive(@PathVariable final String identifier,
			@RequestBody final DocUnitDTO doc) {
		if (!accessHelper.checkDocUnit(identifier)) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
