...
import org.numahop.numahop.service.statistics.StatisticsDocumentService;
...
public class StatisticsDocUnitController {
	...
	public StatisticsDocUnitController(final LibraryAccesssHelper libraryAccesssHelper,
			final UICinesReportService uiCinesReportService, final UIDocUnitService uiDocUnitService,
			final UIInternetArchiveReportService uiInternetArchiveReportService, final StatisticsService uiStatService,
			final StatisticsDocumentService statisticsDocumentService, final AccessHelper accessHelper) {
	...
	public ResponseEntity<Page<StatisticsDocUnitCountDTO>> getDocUnitList(final HttpServletRequest request,
		...
		final Page<StatisticsDocUnitCountDTO> docUnits = uiStatService.getDocUnits(filteredLibraries, projects, lots,
				page, size);
	...
	public ResponseEntity<StatisticsDocUnitStatusRatioDTO> getDocUnitStatusRatio(final HttpServletRequest request,
		...
		final StatisticsDocUnitStatusRatioDTO dto = uiStatService.getDocUnitStatusRatio(filteredLibraries, project, lot,
				state);
	...
	public ResponseEntity<List<StatisticsDocUnitAverageDTO>> getDocUnitAverages(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		final List<StatisticsDocUnitAverageDTO> docUnits = uiStatService.getDocUnitAverages(filteredLibraries, projects,
				lots, deliveries, fromDate, toDate, groupBy);
		return new ResponseEntity<>(docUnits, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<StatisticsProcessedDocUnitDTO>> getExportedDocUnitList(final HttpServletRequest request,
		...
		// Droits d'accès
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(uiInternetArchiveReportService.findAll(filteredLibraries, fromDate, failures),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getCheckDelayStatisitics(final HttpServletRequest request,
		...
		return new ResponseEntity<>(
				uiStatService.getCheckDelayStatisitics(filteredLibraries, projects, lots, deliveries, groupBy),
				HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "doc_published" },
	...
	public ResponseEntity<Page<StatisticsDocPublishedDTO>> getDocPublishedStat(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(statisticsDocumentService.getDocPublishedStat(filteredLibraries, projects, lots,
				fromDate, toDate, types, collections, page, size), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "doc_rejected" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Page<StatisticsDocRejectedDTO>> getDocRejectedStat(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(statisticsDocumentService.getDocRejectedStats(filteredLibraries, projects,
				providers, fromDate, toDate, page, size), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "doc_types" }, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Page<String>> getDocUnitTypes(
