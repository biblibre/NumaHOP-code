...
import org.numahop.numahop.service.statistics.StatisticsProgressService;
import org.numahop.numahop.service.statistics.StatisticsService;
import org.numahop.numahop.service.statistics.StatisticsWorkflowService;
import org.numahop.numahop.service.train.ui.UITrainService;
...
public class StatisticsController extends AbstractRestController {
	...
	@Autowired
	public StatisticsController(final AccessHelper accessHelper, final LibraryAccesssHelper libraryAccesssHelper,
			final UIDeliveryService uiDeliveryService, final UITrainService uiTrainService,
			final StatisticsService statisticsService, final StatisticsProgressService statisticsProgressService,
	...
	public ResponseEntity<Page<StatisticsProjectDTO>> projectList(final HttpServletRequest request,
		...
		return new ResponseEntity<>(statisticsService.searchProject(search, libraries,
				projects.stream().map(AbstractDomainObject::getIdentifier).collect(Collectors.toList()), fromDate,
				toDate, page, size, sorts), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "projectGroupByStatus" },
	...
	@RequestMapping(method = RequestMethod.GET, params = { "lotGroupByStatus" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	@RequestMapping(method = RequestMethod.GET, params = { "projectProgress" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Page<StatisticsProgressDTO>> getProjectProgress(final HttpServletRequest request,
		...
		return new ResponseEntity<>(
				statisticsProgressService.getProjectProgress(filteredLibraries, projects, fromDate, toDate, page, size),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<Page<StatisticsProgressDTO>> getLotProgress(final HttpServletRequest request,
		...
		return new ResponseEntity<>(
				statisticsProgressService.getLotProgress(filteredLibraries, projects, fromDate, toDate, page, size),
				HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "trainGroupByStatus" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<List<Map<String, Object>>> getTrainGroupByStatus(final HttpServletRequest request,
		...
		// Droits d'accès
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(statisticsService.getTrainGroupByStatus(filteredLibraries, projects),
				HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "docunitGroupByStatus" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<List<Map<String, Object>>> getDocUnitsGroupByStatus(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(
				workflowProgressReportService.getDocUnitsGroupByStatus(filteredLibraries, projects, lots),
				HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "userGroupByLibrary" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	@RequestMapping(method = RequestMethod.GET, params = { "provider_train" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<List<StatisticsProviderTrainDTO>> getProviderTrainStats(final HttpServletRequest request,
		...
		// Droits d'accès
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(uiTrainService.getProviderTrainStats(filteredLibraries, projects, trains, status,
				returnFrom, returnTo, sendFrom, sendTo, insuranceFrom, insuranceTo), HttpStatus.OK);
	}
