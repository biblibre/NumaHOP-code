...
import static org.numahop.numahop.web.rest.document.security.AuthorizationConstants.COND_REPORT_HAB0;
import static org.numahop.numahop.web.rest.document.security.AuthorizationConstants.COND_REPORT_HAB6;
...
public class DescriptionPropertyController extends AbstractRestController {
	...
	public DescriptionPropertyController(final DescriptionPropertyService descriptionPropertyService,
			UiPropertyConfigurationService propertyConfigurationService) {
	...
	public ResponseEntity<DescriptionProperty> create(@RequestBody final DescriptionProperty property)
	...
	@Timed
	@RolesAllowed({ COND_REPORT_HAB0, COND_REPORT_HAB6 })
	public List<DescriptionProperty> getAllWithFakesAndShowCreationByLibrary(final HttpServletRequest request,
		...
		return propertiesList.stream()
			.filter(prop -> !propertiesConfDto.contains(prop.getIdentifier()))
			.collect(Collectors.toList());
	}

	private void addFakePropertiesInList(List<DescriptionProperty> properties) {
		/**
		 * This list of fakes properties have to be the same of the mappinfSrvc.js AND
		 * condreportSrvc.js
		 */
	...
	@Timed
	@RolesAllowed(COND_REPORT_HAB6)
	public ResponseEntity<DescriptionProperty> update(@RequestBody final DescriptionProperty value)
			throws PgcnException {
