...
public class DocUnit extends AbstractDomainObject {
	...
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "lot")
	@Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED) // l'id du lot (champ
																	// de docunit) est
																	// audité, mais
																	// pas le lot lui-même
	private Lot lot;
	...
	public InternetArchiveCollection getCollectionIA() {
		...
		// TODO: gérer l'initialisation au niveau du service, ou au moins pas dans le
		// getter
		try {
	...
	public CinesPAC getPlanClassementPAC() {
		...
		catch (final LazyInitializationException e) { // NOSONAR pour pas pourrir les logs
														// car c'est qd mm souvent que
														// ....
			LOG.warn("Problème d'initialisation: {}", e.getMessage());
			return null;
		}
	...
	public enum State {
		...
		AVAILABLE,
		// l'unité documentaire n'est pas disponible dans l'application (import en
		// cours, ...)
		NOT_AVAILABLE,
		// l'unité documentaire a été supprimée (utilisé pour les recherches d'ud
		// importées)
		DELETED,
