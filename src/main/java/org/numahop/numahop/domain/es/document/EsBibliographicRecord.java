...
public class EsBibliographicRecord {
	...
	public static EsBibliographicRecord from(final BibliographicRecord record) {
		...
		esRecord.setLastModifiedDate(Stream
			.concat(Stream.of(record.getLastModifiedDate()),
					record.getProperties().stream().map(AbstractDomainObject::getLastModifiedDate))
