...
public class EsConditionReportDetail {
	...
	public static EsConditionReportDetail from(final ConditionReportDetail detail) {
		...
		esDetail
			.setDescriptions(detail.getDescriptions().stream().map(EsDescription::from).collect(Collectors.toSet()));
		...
		esDetail.setSortedType(detail.getType() != null ? detail.getType().ordinal() : Integer.MAX_VALUE);
		return esDetail;
	}
