...
/**
 * Bean vérifiant si une URL est joignable. Sert à attendre qu'un service web soit
 * disponible avant de lancer l'application, à faire dépendre d'un autre bean via une
 * relation depends-on dans spring.
 */
public class HttpServiceStartupValidator implements InitializingBean {
	...
	/**
	 * Définit l'interval entre 2 essais (en secondes) Par défaut
	...
	/**
	 * Définit la durée après laquelle une exception fatale sera levée (en secondes). Par
	 * défaut {@value #DEFAULT_TIMEOUT}.
	 */
	...
	/**
	 * Vérifie que l'url est accessible. Réessaie pendant 'timeout' secondes, toutes les
	 * 'interval' secondes
	 */
	@Override
		...
		try {
			...
			if (LOG.isInfoEnabled()) {
				...
				if (rest > this.interval) {
					LOG.info("URL {} not reachable - retry in {} seconds (fail in {} seconds) : {}", url, this.interval,
							rest, ex.getLocalizedMessage());
				}
			}
			return false;
		}
