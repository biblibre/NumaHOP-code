...
import org.numahop.numahop.domain.document.QPhysicalDocument;
import org.numahop.numahop.domain.library.QLibrary;
import org.numahop.numahop.domain.lot.QLot;
import org.numahop.numahop.domain.project.QProject;
...
import org.numahop.numahop.domain.workflow.QWorkflowGroup;
import org.numahop.numahop.domain.workflow.QWorkflowModelState;
...
public class DocUnitWorkflowRepositoryImpl implements DocUnitWorkflowRepositoryCustom {
	...
	@Override
	public Page<DocUnitWorkflow> findDocUnitProgressStats(final List<String> libraries, final List<String> projects,
			final boolean projetActive, final List<String> lots, final List<String> trains, final String pgcnId,
			final List<WorkflowStateKey> states, final List<WorkflowStateStatus> status, final List<String> users,
			final LocalDate fromDate, final LocalDate toDate, final Pageable pageable) {

		final QDocUnitWorkflow qDocUnitWorkflow = QDocUnitWorkflow.docUnitWorkflow;

		final JPAQuery<DocUnitWorkflow> baseQuery = createQueryToFindDocUnitWorkFlows(qDocUnitWorkflow, libraries,
				projects, projetActive, lots, trains, pgcnId, states, status, users, fromDate, toDate);
	...
	private JPAQuery<DocUnitWorkflow> createQueryToFindDocUnitWorkFlows(final QDocUnitWorkflow qDocUnitWorkflow,
			final List<String> libraries, final List<String> projects, final boolean projetActive,
			final List<String> lots, final List<String> trains, final String pgcnId,
			final List<WorkflowStateKey> states, final List<WorkflowStateStatus> status, final List<String> users,
		...
		// provider, library
		QueryDSLBuilderUtils.addAccessFilters(builder, qLibrary, qLot, qProject, qAssociatedLibrary, qAssociatedUser,
				libraries, null);
	...
	public List<DocUnitWorkflow> findDocUnitWorkflows(final DocUnitWorkflowSearchBuilder searchBuilder) {
		...
		return DocUnitWorkflowHelper
			.getFindDocUnitWorkflowQuery(query, searchBuilder, qDocUnit, qDocUnitWorkflow, qDocUnitState)
			.fetch();
	}

	@Override
	public List<DocUnitWorkflow> findPendingDocUnitWorkflows(final List<String> libraries, final List<String> projects,
		...
		if (CollectionUtils.isNotEmpty(deliveries)) {
			...
			final BooleanBuilder subBuilder = new BooleanBuilder()
				.and(qDeliveredDocument.delivery.identifier.in(deliveries))
	...
	public List<DocUnitWorkflow> findDocUnitWorkflowsInControl(final List<String> libraries,
		...
		// Étape de livraison / relivraison terminée, non suivie d'une étape de
		...
		/*
		 * Workflow d 'UD ayant une (Re)Livraison terminée, non suivie d' une validation
		 * terminée La double sous-requête donne: select * from doc_workflow w inner join
		 * doc_workflow_state s on s.workflow = w.identifier where s.start_date is not
		 * null and s.end_date is not null and s.`key` in ('LIVRAISON_DOCUMENT_EN_COURS',
		 * 'RELIVRAISON_DOCUMENT_EN_COURS') and not exists( select s2.identifier from
		 * doc_workflow w2 inner join doc_workflow_state s2 on s2.workflow = w2.identifier
		 * where s2.start_date is not null and s2.end_date is not null and s2.`key` in
		 * ('VALIDATION_DOCUMENT') and s2.start_date >= s.end_date and w2.identifier =
		 * w.identifier )
		 */
	...
	@Override
	public List<Object[]> getDocUnitsGroupByStatus(final List<String> libraries, final List<String> projects,
	...
	public List<DocUnitWorkflow> findDocUnitWorkflowsForDigitalLibraryExport(final String library) {
		...
		builder.and(qDocUnit.library.identifier.eq(library));
		builder.and(qDocUnit.digLibExportStatus.ne(DocUnit.ExportStatus.IN_PROGRESS)
			.or(qDocUnit.digLibExportStatus.isNull()));
