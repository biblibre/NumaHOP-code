...
public class SearchResultPage<T> extends PageImpl<T> {
	...
	public SearchResultPage(final SearchHits<T> page, final Pageable pageable) {
		super(page.getSearchHits().stream().map(SearchHit::getContent).collect(Collectors.toList()), pageable,
				page.getTotalHits());
		this.highlights.addAll(getHighlights(page));
		this.aggregations.putAll(getAggregations(page));
	}

	protected SearchResultPage(final List<T> content, final Pageable pageable, final long total,
			final List<Highlight> highlights, final Map<String, List<Map<String, ?>>> aggregations) {
	...
	public <U> Page<U> map(final Function<? super T, ? extends U> converter) {
		return new SearchResultPage<>(this.getConvertedContent(converter), this.getPageable(), this.getTotalElements(),
				this.highlights, this.aggregations);
	}
		...
		for (final SearchHit<T> hit : response.getSearchHits()) {
			if (hit != null) {
				hit.getHighlightFields()
					.entrySet()
					.stream()
					.flatMap(e -> e.getValue().stream().map(frag -> new Highlight(hit.getId(), e.getKey(), frag)))
	...
	private List<Map<String, ?>> getAggregation(final Aggregate aggregate) {
		if (aggregate.isLterms()) {
			return aggregate.lterms()
				...
				.map(bucket -> Map.of(RESPONSE_FACET_TITLE,
						...
						RESPONSE_FACET_COUNT, bucket.docCount()))
		...
		if (aggregate.isSterms()) {
			return aggregate.sterms()
				...
				.array()
				.stream()
				.map(bucket -> Map.of(RESPONSE_FACET_TITLE, bucket.key()._toJsonString(), RESPONSE_FACET_COUNT,
						bucket.docCount()))
				.collect(Collectors.toList());
		}
			...
			return aggregate.nested()
				...
				.values()
				.stream()
				.flatMap(a -> getAggregation(a).stream())
				.collect(Collectors.toList());
		}
			...
			return aggregate.dateHistogram()
				...
				.array()
				.stream()
				.map(bucket -> Map.of(RESPONSE_FACET_TITLE, bucket.keyAsString(), RESPONSE_FACET_COUNT,
						bucket.docCount()))
				.collect(Collectors.toList());
		}
		else {
