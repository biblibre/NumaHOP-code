...
public final class QueryDSLBuilderUtils {
	 ...
	 */
	public static void addFilterForInitiale(final BooleanBuilder builder, final String initiale,
		...
		if (StringUtils.isNotBlank(initiale)) {
			...
			if (StringUtils.equals(initiale, INITIAL_OTHER)) {
				IntStream.range('a', 'z')
					.mapToObj(ch -> path.startsWithIgnoreCase(String.valueOf(ch)))
					.forEach(builder::andNot);
			}
			else {
	 ...
	 */
	public static void addAccessFilters(final BooleanBuilder builder, final QLibrary qLibrary, final QLot qLot,
			final QProject qProject, final QLibrary qAssociatedLibrary, final QUser qAssociatedUser,
			final List<String> libraries, final List<String> providers) {
		final CustomUserDetails currentUser = SecurityUtils.getCurrentUser();

		// Prestataire: voit les projets / lots sur lesquels il est prestataires + les
		// données de sa bibliothèque
	 ...
	 * Filtrage des prestataires (si l'utilisateur courant est un prestataire)
	 */
	public static void addAccessFilters(final BooleanBuilder builder, final QLibrary qLibrary, final QProject qProject,
			final List<String> libraries, final List<String> providers) {
		final CustomUserDetails currentUser = SecurityUtils.getCurrentUser();

		// Prestataire: voit les projets sur lesquels il est prestataires + les données
		// de sa bibliothèque
		if (currentUser != null && currentUser.getCategory() == User.Category.PROVIDER) {
			builder.and(qProject.provider.identifier.eq(currentUser.getIdentifier())
				.and(qLibrary.identifier.eq(currentUser.getLibraryId())));
		}
		// Sinon on applique les filtres demandés
		else {
