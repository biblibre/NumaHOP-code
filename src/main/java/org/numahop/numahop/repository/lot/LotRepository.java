...
public interface LotRepository extends JpaRepository<Lot, String>, LotRepositoryCustom {
	...
	@Query("""
			select l.identifier from Lot l
			""")
	List<String> findAllIdentifiers();

	@Query("""
			select l.identifier from Lot l
			where lower(l.label) like %?1%
			""")
	...
	@Query("""
			select lo from Lot lo
			left join fetch lo.project p
			left join fetch p.library l
			where lo.status = 'CLOSED' and lo.filesArchived = false and l.identifier = ?1 and lo.realEndDate < ?2
			""")
	...
	@Query("""
			select lo from Lot lo
			where (lo.type = 'PHYSICAL' and lo.status = 'ONGOING') or  (lo.type = 'DIGITAL' and lo.status != 'CLOSED') and lo.active = ?1
			""")
