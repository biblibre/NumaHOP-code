...
import org.numahop.numahop.domain.dto.lot.QSimpleLotDTO;
...
public class LotRepositoryImpl implements LotRepositoryCustom {
	...
	public List<SimpleLotDTO> findAllIdentifiersInProjectIds(final Collection<String> projectIds) {
		...
		return queryFactory.select(new QSimpleLotDTO(lot.identifier, lot.label))
			.from(lot)
			.innerJoin(lot.project, project)
			.where(project.identifier.in(projectIds))
			.fetch();
	}
	...
	public Page<Lot> search(final LotSearchBuilder searchBuilder, final Pageable pageable) {
		...
		if (searchBuilder.getLastDlvFrom().isPresent() || searchBuilder.getLastDlvTo().isPresent()) {
			...
			searchBuilder.getLastDlvFrom()
				.ifPresent(fromDate -> subFilter.and(qDeliveredDocument.deliveryDate.goe(fromDate)));
			searchBuilder.getLastDlvTo()
				.ifPresent(toDate -> subFilter.and(qDeliveredDocument.deliveryDate.loe(toDate)));
	...
	public List<Lot> findLotsForWidget(final LocalDate fromDate, final List<String> libraries,
			final List<String> projects, final List<Lot.LotStatus> statuses) {
		...
		return queryFactory.select(qLot)
			.from(qLot)
			.leftJoin(qLot.project, qProject)
	 ...
	 * @return JPQLQuery
	 */
	protected JPAQuery<Lot> applySorting(final Sort sort, final JPAQuery<Lot> query, final QLot lot,
