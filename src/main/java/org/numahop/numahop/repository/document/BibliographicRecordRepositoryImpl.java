...
public class BibliographicRecordRepositoryImpl implements BibliographicRecordRepositoryCustom {
	...
	public Page<BibliographicRecord> search(final String search, final List<String> libraries,
			...
			final LocalDate createdDateFrom, final LocalDate createdDateTo, final Boolean orphan,
		...
		if (StringUtils.isNotBlank(search)) {
			final BooleanExpression nameFilter = qRecord.title.containsIgnoreCase(search)
				.or(qDocUnit.pgcnId.containsIgnoreCase(search));
		...
		if (CollectionUtils.isNotEmpty(statuses)) {
			...
			if (statuses.contains(WorkflowStateKey.CLOTURE_DOCUMENT.name())) {
				...
				statusBuilder.and(qState.discriminator.eq(WorkflowStateKey.CLOTURE_DOCUMENT))
					.and(qState.status.in(WorkflowStateStatus.FINISHED));
				if (CollectionUtils.isNotEmpty(statuses)) {
					wkfStates.remove(WorkflowStateKey.CLOTURE_DOCUMENT);
					statusBuilder.or(qState.discriminator.in(wkfStates)
						.and(qState.startDate.isNotNull().and(qState.endDate.isNull())));
				}
			}
			else {
		...
		if (lastModifiedDateFrom != null) {
			final BooleanExpression lastModifiedDateFromFilter = qRecord.lastModifiedDate
				.after(lastModifiedDateFrom.atStartOfDay());
			builder.and(lastModifiedDateFromFilter);
		}

		if (lastModifiedDateTo != null) {
			final BooleanExpression lastModifiedDateToFilter = qRecord.lastModifiedDate
		...
		if (createdDateTo != null) {
			final BooleanExpression createdDateToFilter = qRecord.createdDate
				.before(createdDateTo.atTime(LocalTime.MAX));
	 ...
	 * @return
	 */
	protected JPAQuery<BibliographicRecord> applySorting(final Sort sort, final JPAQuery<BibliographicRecord> query,
			final QBibliographicRecord bib, final QDocUnit doc, final QProject project, final QLot lot) {
