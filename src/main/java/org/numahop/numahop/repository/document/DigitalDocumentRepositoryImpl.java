...
public class DigitalDocumentRepositoryImpl implements DigitalDocumentRepositoryCustom {
	...
	@Override
	public Page<DigitalDocument> search(final String search, final List<DigitalDocument.DigitalDocumentStatus> status,
			final List<String> libraries, final List<String> projects, final List<String> lots,
			final List<String> trains, final List<String> deliveries, final LocalDate dateFrom, final LocalDate dateTo,
			...
			final Double searchMinSize, final Double searchMaxSize, final boolean validated, final Pageable pageable) {
		...
		// provider
		QueryDSLBuilderUtils.addAccessFilters(builder, library, lot, project, qAssociatedLibrary, qAssociatedUser,
				libraries, null);
		...
		if (relivraison) {
			final BooleanExpression stateFilter = docUnitState.discriminator
				.eq(WorkflowStateKey.RELIVRAISON_DOCUMENT_EN_COURS);
	...
	protected JPAQuery<DigitalDocument> applySorting(final Sort sort, final JPAQuery<DigitalDocument> query,
			final QDigitalDocument doc, final QDocUnit docUnit) {
