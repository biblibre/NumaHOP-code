...
import org.numahop.numahop.domain.administration.QSftpConfiguration;
...
public class SftpConfigurationRepositoryImpl implements SftpConfigurationRepositoryCustom {
	...
	public Page<SftpConfiguration> search(final String search, final List<String> libraries, final Pageable pageable) {
		...
		final JPAQuery<SftpConfiguration> baseQuery = queryFactory.selectDistinct(configuration)
			.from(configuration)
			.where(builder);
		...
		final List<SftpConfiguration> result = baseQuery.leftJoin(configuration.library)
			.fetchJoin()
