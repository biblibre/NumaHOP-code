 ...
 * <ul>
 * <li><a href=
 * "http://jaspan.com/improved_persistent_login_cookie_best_practice">Improved Persistent
 * Login Cookie Best Practice</a></li>
...
public class CustomPersistentRememberMeServices extends AbstractRememberMeServices {
	...
	public CustomPersistentRememberMeServices(final Environment env,
			...
			final UserRepository userRepository, final PersistentTokenRepository persistentTokenRepository) {
	...
	private PersistentToken getPersistentToken(final String[] cookieTokens) {
		...
		if (DateUtils.convertToLocalDate(token.getTokenDate())
	...
	private void addCookie(final PersistentToken token, final HttpServletRequest request,
			final HttpServletResponse response) {
		setCookie(new String[] { token.getSeries(), token.getTokenValue() }, TOKEN_VALIDITY_DAYS * 86400, request,
				response);
	}
