...
public class ControlesQualiteState extends DocUnitState {
	...
	public void process(final User user) {
		...
		// Initialisation de la prochaine étape si applicable (aucune étape en cours de la
		// branche)
		final List<DocUnitState> currentStates = getWorkflow().getCurrentStates();
		if (currentStates.isEmpty() || isWaitingStateForWorkflow(currentStates)) {
			...
			if (preVal.isSkippedOrCanceled()) {
				final DocUnitState valState = getWorkflow()
					.getFutureOrRunningByKey(WorkflowStateKey.VALIDATION_DOCUMENT);
				if (valState != null) {
