...
public class Check extends AbstractDomainObject {
	...
	public enum ErrorLabel {
		...
		// major
		UNREADABLE("Fichier illisible", ErrorType.MAJOR), BAD_NAME("Non-conformité du nommage", ErrorType.MAJOR),
		...
		BAD_THRESHOLD("Non-respect du seuil", ErrorType.MAJOR), WRONG_FORMAT("Format non respecté", ErrorType.MAJOR),
		...
		// minor
		BLURRED_IMAGE("Image floue", ErrorType.MINOR), SLANT_IMAGE("Image inclinée", ErrorType.MINOR),
		HALO_ON_IMAGE("Halo sur l'image", ErrorType.MINOR), SHADOW_ON_IMAGE("Ombre portée", ErrorType.MINOR),
		GEOMETRICAL_ANOMALY("Distorsions géométriques", ErrorType.MINOR),
		CHROMATIC_ANOMALY("Dérive de la chromie", ErrorType.MINOR), WRONG_FRAMING("Cadrage inadapté", ErrorType.MINOR),
		WRONG_ORDER("Ordre des vues non respecté", ErrorType.MINOR),
		BAD_OCR("Non conformité du texte ocr", ErrorType.MINOR), ANOTHER_MIN("Autre erreur mineure", ErrorType.MINOR);
