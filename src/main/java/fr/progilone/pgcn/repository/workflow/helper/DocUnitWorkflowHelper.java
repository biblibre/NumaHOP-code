...
public class DocUnitWorkflowHelper {
	...
	public static <T> JPAQuery<T> getFindDocUnitWorkflowQuery(final JPAQuery<T> query,
			final DocUnitWorkflowSearchBuilder searchBuilder, final QDocUnit qDocUnit,
			final QDocUnitWorkflow qDocUnitWorkflow, final QDocUnitState qDocUnitState) {

		final BooleanBuilder builder = new BooleanBuilder();

		// Droits d'accès
		QueryDSLBuilderUtils.addAccessFilters(builder, qDocUnit.library, qDocUnit.project,
				searchBuilder.getLibraries().orElse(null), null);
		// Projets
		...
		searchBuilder.getRoles().ifPresent(roles -> {
			...
			builder.and(JPAExpressions.select(qUser)
				.from(qUser)
				.where(new BooleanBuilder().and(qUser.role.identifier.in(roles))
					.and(qUser.login.eq(qDocUnitState.user)))
				.exists());
		});
		// Workflows
		...
		searchBuilder.getStates().ifPresent(states -> {
			if (searchBuilder.isWithFailedStatuses()) {
				builder.and(qDocUnitState.discriminator.in(states))
					.and(qDocUnitState.status.eq(WorkflowStateStatus.FINISHED)
		...
		searchBuilder.getFromDate().ifPresent(fromDate -> {
			builder.and(new BooleanBuilder().or(qDocUnitState.endDate.isNull())
				.or(qDocUnitState.endDate.after(fromDate.atStartOfDay())));
		});
		...
		searchBuilder.getCollections().ifPresent(collections -> {
			...
			builder.and(new BooleanBuilder().or(qIACollection.identifier.in(collections))
				.or(qOmekaCollection.identifier.in(collections)));
		});
		// Requête
