...
public class HelpPageRepositoryImpl implements HelpPageRepositoryCustom {
	...
	public List<HelpPageDto> search(final List<String> modules, final List<HelpPageType> types, final String search) {
		...
		final List<HelpPageDto> result = queryFactory
			.select(new QHelpPageDto(qhp.identifier, qhp.title, qhp.rank, qhp.module, qhp.type, qhp.parent.identifier))
			.from(qhp)
	...
	private void fillChildrenOf(final List<HelpPageDto> parents) {
		...
		if (!children.isEmpty()) {
			final Map<String, List<HelpPageDto>> map = children.stream()
				.collect(Collectors.groupingBy(HelpPageDto::getParent, Collectors.toList()));
			parents.forEach(p -> {
