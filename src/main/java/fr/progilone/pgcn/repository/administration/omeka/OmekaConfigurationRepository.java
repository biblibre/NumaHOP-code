...
public interface OmekaConfigurationRepository
		extends JpaRepository<OmekaConfiguration, String>, OmekaConfigurationRepositoryCustom {

	@Query("select distinct c " + "from OmekaConfiguration c " + "join fetch c.library ")
	Set<OmekaConfiguration> findAllWithDependencies();

	@Query("select distinct c " + "from OmekaConfiguration c " + "join fetch c.library " + "where c.active = ?1")
	Set<OmekaConfiguration> findByActiveWithDependencies(boolean active);

	@Query("select distinct c " + "from OmekaConfiguration c " + "join fetch c.library l " + "where l = ?1")
	...
	@Query("select c " + "from OmekaConfiguration c " + "join fetch c.library l " + "left join fetch c.omekaLists "
