 ...
 */
public interface MailboxConfigurationRepository
		extends JpaRepository<MailboxConfiguration, String>, MailboxConfigurationRepositoryCustom {

	@Query("select distinct c " + "from MailboxConfiguration c " + "join fetch c.library ")
	Set<MailboxConfiguration> findAllWithDependencies();

	@Query("select distinct c " + "from MailboxConfiguration c " + "join fetch c.library " + "where c.active = ?1")
	Set<MailboxConfiguration> findByActiveWithDependencies(boolean active);

	@Query("select distinct c " + "from MailboxConfiguration c " + "join fetch c.library l " + "where l = ?1")
	...
	@Query("select c " + "from MailboxConfiguration c " + "join fetch c.library " + "where c.identifier = ?1")
