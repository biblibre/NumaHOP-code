 ...
 */
public interface SftpConfigurationRepository
		extends JpaRepository<SftpConfiguration, String>, SftpConfigurationRepositoryCustom {

	@Query("select distinct c " + "from SftpConfiguration c " + "join fetch c.library ")
	Set<SftpConfiguration> findAllWithDependencies();

	@Query("select distinct c " + "from SftpConfiguration c " + "join fetch c.library " + "where c.active = ?1")
	Set<SftpConfiguration> findByActiveWithDependencies(boolean active);

	@Query("select distinct c " + "from SftpConfiguration c " + "join fetch c.library l " + "where l = ?1")
	...
	@Query("select c " + "from SftpConfiguration c " + "join fetch c.library " + "where c.identifier = ?1")
