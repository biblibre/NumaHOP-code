...
public interface DeliveryRepository extends JpaRepository<Delivery, String>, DeliveryRepositoryCustom {
	...
	@Query("""
		...
		left join fetch l.activeCheckConfiguration acc
		left join fetch acc.library
	...
	@Query("""
		select distinct doc from DeliveredDocument doc
		left join doc.delivery d
		join fetch doc.digitalDocument dig
		join fetch dig.docUnit du
		where d.identifier = ?1
		""")
	...
	@Query("""
		select doc from DeliveredDocument doc
		join fetch doc.digitalDocument dig
	...
	@Modifying
	@Query("""
		delete from DeliveredDocument doc
		where doc.delivery.identifier = ?1 and doc.digitalDocument.identifier = ?2
		""")
	...
	@Query("""
		select distinct dd from DeliveredDocument dd
	...
	@Query("""
		select distinct del from Delivery del
	...
	@Query("""
		select d.identifier from Delivery d
		""")
