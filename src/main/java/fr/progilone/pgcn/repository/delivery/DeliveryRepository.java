...
public interface DeliveryRepository extends JpaRepository<Delivery, String>, DeliveryRepositoryCustom {
	...
	@Query("select distinct d " + "from Delivery d " + "join fetch d.documents " + "where d.identifier = ?1")
	Delivery findOneWithDocuments(String identifier);

	@Query("select distinct d " + "from Delivery d " + "left join d.documents doc " + "left join fetch d.lot l "
			+ "left join fetch l.docUnits du " + "left join fetch l.activeCheckConfiguration acc "
			+ "left join fetch acc.library " + "left join fetch l.activeFTPConfiguration "
			+ "left join fetch du.physicalDocuments " + "where d.identifier = ?1")
	Delivery findOneWithDependencies(String id);

	@Query("select distinct doc " + "from DeliveredDocument doc " + "left join doc.delivery d "
			+ "join fetch doc.digitalDocument dig " + "join fetch dig.docUnit du " + "where d.identifier = ?1")
	Set<DeliveredDocument> findSimpleDeliveredDocumentsByDeliveryIdentifier(String id);

	@Query("select distinct doc " + "from DeliveredDocument doc " + "join fetch doc.delivery d "
			+ "join fetch doc.digitalDocument dig " + "left join fetch doc.checkSlip " + "where dig.identifier = ?1 "
			+ "order by doc.deliveryDate desc ")
	List<DeliveredDocument> findDeliveredDocumentsByDigitalDocIdentifier(String id);

	@Query("select doc from DeliveredDocument doc " + "join fetch doc.digitalDocument dig "
			+ "join fetch doc.checkSlip cs " + "join fetch cs.slipLines " + "left join fetch dig.docUnit "
	...
	@Query("select doc from DeliveredDocument doc " + "join fetch doc.digitalDocument "
			+ "where doc.delivery.identifier = ?1 " + "and doc.digitalDocument.identifier = ?2")
	...
	@Query("select distinct dd from DeliveredDocument dd " + "join fetch dd.delivery d "
			+ "join fetch dd.digitalDocument dig " + "join fetch d.lot lo "
	...
	@Query("select distinct d " + "from Delivery d " + "join fetch d.lot l " + "where l.identifier in ?1")
	...
	@Query("select distinct d " + "from Delivery d " + "left join fetch d.lot l "
			+ "left join fetch l.activeCheckConfiguration " + "left join fetch l.project p "
			+ "left join fetch p.activeCheckConfiguration " + "where d.identifier = ?1")
	Delivery getWithActiveCheckConfiguration(String identifier);

	@Query("select distinct d " + "from Delivery d " + "left join fetch d.lot l " + "left join fetch l.project p "
			+ "left join fetch p.library lib " + "left join lib.defaultRole " + "where d.identifier = ?1")
	Delivery getSimpleWithLot(String identifier);

	@Query("select distinct del " + "from Delivery del " + "join del.documents doc " + "join doc.digitalDocument dig "
			+ "join dig.docUnit u " + "where u.identifier = ?1")
	List<Delivery> findAllByDocUnitIdentifier(String docUnitId);

	@Query("select d.identifier from Delivery d")
