...
public interface LotRepository extends JpaRepository<Lot, String>, LotRepositoryCustom {
	...
	@Query("select distinct l " + "from Lot l " + "left join fetch l.project " + "left join fetch l.platforms "
	...
	@Query("from Lot l " + "left join fetch l.project " + "left join fetch l.platforms " + "left join fetch l.docUnits "
			+ "left join fetch l.activeFTPConfiguration " + "left join fetch l.activeOcrLanguage "
			+ "where l.identifier = ?1")
	Lot findOneWithDependencies(String identifier);

	@Query("from Lot l " + "join fetch l.project p " + "join fetch p.library lib "
			+ "left join fetch l.activeFTPConfiguration " + "left join fetch p.activeFTPConfiguration "
			+ "left join fetch lib.activeFTPConfiguration " + "where l.identifier = ?1")
	Lot findOneWithActiveFtpConfig(String identifier);

	@Query("from Lot l " + "left join fetch l.activeCheckConfiguration "
			+ "left join fetch l.activeFormatConfiguration " + "left join fetch l.activeOcrLanguage "
			+ "left join fetch l.project p " + "left join fetch p.activeCheckConfiguration "
			+ "left join fetch p.activeFormatConfiguration " + "left join fetch p.library lib "
			+ "left join fetch lib.activeCheckConfiguration " + "left join fetch lib.activeFormatConfiguration "
			+ "left join fetch lib.activeOcrLangConfiguration " + "where l.identifier = ?1")
	Lot findOneWithActiveCheckConfiguration(String identifier);

	@Query("from Lot l " + "left join fetch l.project p " + "left join fetch l.platforms "
			+ "left join fetch l.docUnits " + "left join fetch l.activeFTPConfiguration "
			+ "left join fetch l.activeCheckConfiguration " + "left join fetch p.library " + "where l.identifier = ?1")
	Lot findOneWithMoreDependencies(String identifier);

	@Query("select distinct l " + "from Lot l " + "left join fetch l.project " + "left join fetch l.platforms "
			+ "left join fetch l.activeFTPConfiguration " + "left join fetch l.activeOcrLanguage " + "where l.id in ?1")
	List<Lot> findAllWithDependencies(final Collection<String> ids);

	@Query("from Lot lo " + "left join fetch lo.project p " + "left join fetch p.library l "
			+ "where lo.status = 'CLOSED' " + "and lo.filesArchived = false " + "and l.identifier = ?1 "
	...
	@Query("from Lot lo " + "where (lo.type = 'PHYSICAL' and lo.status = 'ONGOING') "
			+ "or  (lo.type = 'DIGITAL' and lo.status != 'CLOSED') " + "and lo.active = ?1")
	List<Lot> findAllByActiveForDelivery(boolean active);

	@Query("from Lot lo " + "left join fetch lo.project " + "left join fetch lo.provider "
			+ "where lo.type = 'PHYSICAL' and lo.status = 'ONGOING' " + "and lo.active = ?1")
	List<Lot> findAllActiveForMultiLotsDelivery(boolean active);

	List<Lot> findAllByProjectIdentifier(String id);

	List<Lot> findAllByActiveAndTypeAndStatusAndProjectIdentifier(boolean active, Lot.Type type, Lot.LotStatus status,
			String id);
	...
	@Query("from Lot lo " + "left join fetch lo.docUnits du " + "left join fetch du.workflow "
	...
	@Query("from Lot lo " + "left join fetch lo.workflowModel wf " + "left join fetch wf.modelStates "
			+ "left join fetch lo.project  " + "where lo.identifier = ?1 ")
