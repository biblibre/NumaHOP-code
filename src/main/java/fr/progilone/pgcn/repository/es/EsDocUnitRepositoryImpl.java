...
public class EsDocUnitRepositoryImpl extends AbstractEsRepository<EsDocUnit> implements EsDocUnitRepositoryCustom {
	...
	protected HighlightQuery getHighlightField() {
		return new HighlightQuery(new Highlight(List.of("label", "pgcnId").stream().map(HighlightField::new).toList()),
				null);
	}
	...
	protected Query getFilterQueryBuilder(final String searchField, final List<String> values) {
		...
		switch (type) {
			case "RECORD_PROPERTY":
				...
				return QueryBuilders.nested(b -> b.path("records.properties")
					.query(searchQry.build()._toQuery())
	...
	protected Map<String, Aggregation> getAggregationBuilders() {
		return Stream
			.of("DOCUNIT:type", "DOCUNIT:archivable", "DOCUNIT:distributable", "RECORD_PROPERTY:contributor",
					"RECORD_PROPERTY:coverage", "RECORD_PROPERTY:creator", "RECORD_PROPERTY:language",
					"RECORD_PROPERTY:publisher", "RECORD_PROPERTY:relation", "RECORD_PROPERTY:rights",
					"RECORD_PROPERTY:subject", "RECORD_PROPERTY:type")
	...
	private void addDocUnitSearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
		switch (field) {
			case "pgcnId":
				builder.should(getFullTextQueryBuilder(DocUnit_.pgcnId.getName(), search, false)); // pas
																									// de
																									// recherche
																									// fuzzy
																									// sur
			...
			case "type":
				...
				break;
			case "collectionIA":
				builder.should(getExactQueryBuilder(path(DocUnit_.collectionIA, InternetArchiveCollection_.identifier),
						search));
				break;
			...
			case "digitalId":
				builder.should(
						getExactQueryBuilder(path(DocUnit_.physicalDocuments, PhysicalDocument_.digitalId), search));
				break;
			case "library":
	...
	private void addRecordsearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
		switch (field) {
			case "title":
				builder.should(QueryBuilders.nested()
	...
	private void addPropertySearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
	...
	private void addPhysicalDocSearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
		switch (field) {
			case "totalPage":
				builder.should(
						getRangeQueryBuilder(path(DocUnit_.physicalDocuments, PhysicalDocument_.totalPage), search));
				break;
		}
	}

	private void addCinesSearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
		switch (field) {
			case "dateSent":
				builder.should(QueryBuilders.nested()
	...
	private void addIaSearch(final EsQueryBuilder builder, final String search, final String field,
			final boolean fuzzy) {
		switch (field) {
			case "dateSent":
				builder.should(QueryBuilders.nested()
					.path("iaReports")
					.query(getRangeQueryBuilder("iaReports.dateSent", search))
					.scoreMode(ChildScoreMode.Sum)
					.build()
					._toQuery());
				break;
			case "status":
				builder.should(QueryBuilders.nested()
					.path("iaReports")
					.query(getExactQueryBuilder("iaReports.status", search))
					.scoreMode(ChildScoreMode.Sum)
					.build()
					._toQuery());
				break;
	...
	private void addDefaultSearch(final EsQueryBuilder builder, final String search, final boolean fuzzy) {
		builder
			// Unité documentaire
			.should(getFullTextQueryBuilder(DocUnit_.pgcnId.getName(), search, false)) // pas
																						// de
																						// recherche
																						// fuzzy
																						// sur
	...
	public List<Map<String, Object>> suggest(final String text, final int size, final List<String> libraries) {
		final NativeQuery query = new NativeQueryBuilder()
			.withSuggester(Suggester.of(b -> b.suggesters(EsConstant.SUGGEST_FIELD,
					s -> s.completion(c -> c.field(EsConstant.SUGGEST_FIELD)
						.fuzzy(SuggestFuzziness.of(f -> f.fuzziness("AUTO")))
						.size(size)
						.contexts(Map.of(EsConstant.SUGGEST_CTX_LIBRARY, libraries.stream()
