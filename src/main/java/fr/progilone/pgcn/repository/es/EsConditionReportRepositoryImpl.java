...
public class EsConditionReportRepositoryImpl extends AbstractEsRepository<EsConditionReport>
		implements EsConditionReportRepositoryCustom {
		...
		if (StringUtils.isNotBlank(search)) {
			...
			// Recherche sur les champs des unités documentaires
				...
				final BoolQuery.Builder propQuery = QueryBuilders.bool()
					// type
					.must(getExactQueryBuilder(path(ConditionReport_.details, ConditionReportDetail_.descriptions,
							Description_.property, DescriptionProperty_.identifier), field))
					// valeur
					.must(getExactQueryBuilder(path(ConditionReport_.details, ConditionReportDetail_.descriptions,
							Description_.value, DescriptionValue_.identifier), search));
				// nested
	...
	protected Aggregation getAggregationBuilder(final String aggName) {
		...
		return TermsAggregation.of(b -> b.field(field).size(20).order(List.of(NamedValue.of("_count", SortOrder.Desc))))
			._toAggregation();
	}
	...
	private void addDefaultSearch(final BoolQuery.Builder builder, final String search, final boolean fuzzy) {
		...
		builder.should(getFullTextQueryBuilder(additionnalDescPath, search, fuzzy))
			...
			.should(QueryBuilders.nested()
				.path(descPath)
				.query(getFullTextQueryBuilder(valueLabel, search, fuzzy))
				.scoreMode(ChildScoreMode.Sum)
				.build()
				._toQuery())
			.boost(2F)
			.should(QueryBuilders.nested()
				.path(descPath)
				.query(getFullTextQueryBuilder(comment, search, fuzzy))
				.scoreMode(ChildScoreMode.Sum)
				.build()
				._toQuery());
	}
