...
public class EsQueryHelper {
	...
	/**
	 * Construction d'une recherche fulltext Le champ field contient tous les sous-champs
	 * nécessaires
	 */
	public static Query getFullTextQueryBuilder(final String field, final String search, final boolean fuzzySearch) {
		final BoolQuery.Builder queryBuilder = QueryBuilders.bool()
			...
			.should(QueryBuilders
				.matchPhrase(b -> b.field(path(field, EsConstant.SUBFIELD_PHRASE)).query(search).boost(10F)))
			// Recherche de search comme préfixe de la totalité du champ
			.should(QueryBuilders.matchPhrasePrefix(
					b -> b.field(path(field, SUBFIELD_PHRASE)).query(search).maxExpansions(100).boost(8F)))
	...
	public static Query getExactQueryBuilder(final String field, final Collection<String> values) {
		return QueryBuilders
			.terms(b -> b.field(field).terms(t -> t.value(values.stream().map(FieldValue::of).toList())));
	}
	...
	private static MultiMatchQuery.Builder getMultiMatchQueryBuilder(final String field, final String search,
			final boolean fuzzySearch) {
