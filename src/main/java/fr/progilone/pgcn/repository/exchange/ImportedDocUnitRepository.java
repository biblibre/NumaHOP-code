 ...
 */
public interface ImportedDocUnitRepository
		extends JpaRepository<ImportedDocUnit, String>, ImportedDocUnitRepositoryCustom {

	@Query("select i.identifier " + "from ImportedDocUnit i " + "where i.report = ?1")
	Page<String> findIdentifiersByImportReport(ImportReport report, Pageable pageable);

	@Query("select d.identifier " + "from ImportedDocUnit i " + "join i.docUnit d " + "where i.report = ?1 "
			+ "and d.state= ?2")
	Page<String> findDocUnitIdentifiersByImportReport(ImportReport report, DocUnit.State state, Pageable pageable);

	@Query("select distinct i " + "from ImportedDocUnit i " + "left join fetch i.duplicatedUnits "
			+ "left join fetch i.messages " + "join fetch i.docUnit d " + "left join fetch d.library l "
	...
	@Query("select distinct i " + "from ImportedDocUnit i " + "left join fetch i.messages "
			+ "left join fetch i.docUnit d " + "left join fetch d.library l " + "left join fetch d.records r "
			+ "left join fetch i.duplicatedUnits dup " + "left join fetch dup.records rdup "
			+ "where i.identifier in ?1 ")
	List<ImportedDocUnit> findByIdentifiersIn(List<String> identifiers, Sort sort);

	@Query("select distinct i " + "from ImportedDocUnit i " + "join i.duplicatedUnits dup "
			+ "where dup.identifier = ?1 ")
	List<ImportedDocUnit> findByDuplicatedUnits(String identifier);

	@Query("select distinct i " + "from ImportedDocUnit i " + "join fetch i.docUnit d "
