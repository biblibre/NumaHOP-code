...
public interface TemplateRepository extends JpaRepository<Template, String> {
	...
	@Query("select t " + "from Template t " + "join fetch t.library " + "where t.identifier = ?1")
	Template findByIdentifier(String identifier);

	@Query("select t " + "from Template t " + "join fetch t.library l")
	List<Template> findAll();

	@Query("select t " + "from Template t " + "join fetch t.library l " + "where l = ?1")
	List<Template> findByLibrary(Library library);

	@Query("select t " + "from Template t " + "join fetch t.library l " + "where t.name = ?1 "
			+ "and l.identifier = ?2")
