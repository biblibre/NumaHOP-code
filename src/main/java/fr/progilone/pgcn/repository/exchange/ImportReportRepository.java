...
public interface ImportReportRepository extends JpaRepository<ImportReport, String>, ImportReportRepositoryCustom {
	...
	@Query("select r " + "from ImportReport r " + "left join fetch r.mapping " + "left join fetch r.mappingChildren "
			+ "left join fetch r.csvMapping " + "left join fetch r.parentReport " + "left join fetch r.project "
			+ "left join fetch r.lot " + "join fetch r.library " + "where r.identifier = ?1")
	...
	@Query("select d.identifier " + "from ImportReport r " + "join r.docUnits i " + "join i.docUnit d "
	...
	@Query("select i.identifier " + "from ImportReport r " + "join r.docUnits i " + "where r.identifier = ?1")
