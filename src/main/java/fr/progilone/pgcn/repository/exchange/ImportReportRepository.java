...
public interface ImportReportRepository extends JpaRepository<ImportReport, String>, ImportReportRepositoryCustom {
	...
	@Query("""
		select d.identifier from ImportReport r
		join r.docUnits i
		join i.docUnit d
		where r.identifier = ?1 and d.state = ?2
		""")
	List<String> findDocUnitIdentifiersByReportAndDocUnitState(String report, DocUnit.State state);

	@Query("""
		select i.identifier from ImportReport r
		join r.docUnits i
		where r.identifier = ?1
		""")
	...
	@Modifying
	@Query("""
		update ImportReport r set r.mapping = null
		where r.mapping.identifier = ?1
		""")
	void setMappingNull(String mappingId);

	@Modifying
	@Query("""
		update ImportReport r set r.mappingChildren = null
		where r.mappingChildren.identifier = ?1
		""")
	void setMappingChildrenNull(String mappingId);

	@Modifying
	@Query("""
		update ImportReport r set r.csvMapping = null
		where r.csvMapping.identifier = ?1
		""")
	void setCsvMappingNull(String mappingId);

	@Modifying
	@Query("""
		update ImportReport r set r.lot = null
		where r.lot.identifier in ?1
		""")
	void setLotNullByLotIdIn(List<String> lotIds);

	@Modifying
	@Query("""
		update ImportReport r set r.project = null
		where r.project.identifier in ?1
		""")
	void setProjectNullByProjectIdIn(List<String> projectIds);

	@Modifying
	@Query("""
		update ImportReport r set r.parentReport = null
		where r.parentReport.identifier = ?1
		""")
