...
public interface ConditionReportRepository
	...
	@Query("select c " + "from ConditionReport c " + "join fetch c.docUnit d " + "join fetch d.library "
			+ "left join fetch c.details " + "where c.docUnit.identifier = ?1")
	ConditionReport findByDocUnit(String docUnitId);

	@Query("select c " + "from ConditionReport c " + "join fetch c.docUnit d " + "join fetch d.library "
			+ "left join fetch c.details " + "where c.identifier = ?1")
	ConditionReport findByIdentifier(String identifier);

	@Query("select distinct c " + "from ConditionReport c " + "join fetch c.docUnit d " + "join fetch d.library "
			+ "left join fetch c.details " + "where c.identifier in ?1")
	List<ConditionReport> findByIdentifierIn(List<String> identifiers);

	@Query("select distinct c " + "from ConditionReport c " + "left join fetch c.details d "
