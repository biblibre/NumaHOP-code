...
public interface ConditionReportDetailRepository extends JpaRepository<ConditionReportDetail, String> {
	...
	@Query("""
		select r from ConditionReportDetail c
		join c.report r
	...
	@Query("""
		select d from ConditionReportDetail c
		join c.report r
		join r.docUnit d
		where c.identifier = ?1
		""")
	...
	@Query("""
		select c from ConditionReportDetail c
	...
	@Query("""
		select d  from ConditionReportDetail d
		join d.report r
		join r.docUnit du
	...
	@Query("""
		select max(c.position) from ConditionReportDetail c
		where c.report.identifier = ?1
		""")
