...
public interface DocUnitRepository extends JpaRepository<DocUnit, String>, DocUnitRepositoryCustom {
	...
	@Query("""
		select distinct d from DocUnit d
		left join fetch d.library
		left join fetch d.physicalDocuments pd
		left join fetch d.digitalDocuments dd
		left join fetch d.project
	...
	@Query("""
		select distinct sd from DocUnit d
		join d.sibling s
		join s.docUnits sd
		where d.identifier = ?1
		""")
	...
	@Query("""
		select d.identifier from DocUnit d
		where d.state = ?1
		""")
	...
	@Modifying
	@Query("""
		update DocUnit u set u.parent = null
		where u.parent.identifier in ?1
		""")
	...
	@Query("""
		select distinct d.type from DocUnit d
