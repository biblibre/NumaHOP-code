...
public interface DocUnitRepository extends JpaRepository<DocUnit, String>, DocUnitRepositoryCustom {
	...
	@Query("select d " + "from DocUnit d " + "left join fetch d.library " + "where d.identifier = ?1")
	DocUnit findOneWithLibrary(String identifier);

	@Query("select d " + "from DocUnit d " + "left join fetch d.library " + "left join fetch d.physicalDocuments pd "
			+ "left join fetch d.project " + "left join fetch d.records r " + "left join fetch r.properties p "
			+ "left join fetch p.type " + "where d.identifier = ?1")
	DocUnit findOneWithDependencies(String identifier);

	@Query("select d " + "from DocUnit d " + "left join fetch d.activeOcrLanguage " + "left join fetch d.lot l "
			+ "left join fetch l.activeOcrLanguage " + "where d.identifier = ?1")
	DocUnit findOneWithOcrLanguage(String identifier);

	@Query("select d " + "from DocUnit d " + "left join fetch d.library " + "left join fetch d.workflow w "
			+ "where d.identifier = ?1")
	DocUnit findOneWithAllDependenciesForWorkflow(String identifier);

	@Query("select d " + "from DocUnit d " + "left join fetch d.library lib " + "left join fetch d.project p "
			+ "left join fetch d.lot l " + "left join fetch d.physicalDocuments pd " + "left join fetch d.workflow w "
			+ "left join fetch d.digitalDocuments dd " + "left join fetch dd.deliveries del "
			+ "where d.identifier = ?1")
	DocUnit findOneWithAllDependenciesForHistory(String identifier);

	@Query("select d " + "from DocUnit d " + "left join fetch d.library " + "left join fetch d.project p "
			+ "left join fetch p.provider " + "left join fetch d.lot l " + "left join fetch l.provider "
			+ "where d.identifier = ?1")
	...
	@Query("select d " + "from DocUnit d " + "left join fetch d.library " + "left join fetch d.physicalDocuments pd "
			+ "left join fetch d.digitalDocuments dd " + "left join fetch dd.deliveries del "
			+ "left join fetch d.project " + "left join fetch d.records r " + "left join fetch r.properties p "
			+ "left join fetch p.type " + "left join fetch d.workflow " + "where d.identifier = ?1")
	DocUnit findOneForDisplay(String identifier);

	@Query("select distinct d " + "from DocUnit d " + "left join fetch d.library "
			+ "left join fetch d.physicalDocuments pd " + "left join fetch d.digitalDocuments dd "
			+ "left join fetch d.project " + "left join fetch d.records r " + "left join fetch r.properties p "
			+ "left join fetch p.type " + "left join fetch d.automaticCheckResults " + "where d.parent.identifier = ?1")
	...
	@Query("select distinct d " + "from DocUnit d " + "left join fetch d.library "
			+ "left join fetch d.physicalDocuments pd " + "left join fetch d.digitalDocuments dd "
			+ "left join fetch dd.pages pages " + "left join fetch pages.files f " + "left join fetch d.project "
			+ "left join fetch d.records r " + "left join fetch r.properties p " + "left join fetch p.type "
	...
	@Query("select d from DocUnit d " + "left join fetch d.records r " + "left join fetch r.properties p "
			+ "left join fetch p.type " + "where d.identifier = ?1")
	DocUnit findOneByIdentifierWithRecords(String idDoc);

	@Query("select d from DocUnit d " + "left join fetch d.library " + "left join fetch d.physicalDocuments p "
			+ "left join fetch d.digitalDocuments dd " + "left join fetch d.records r " + "where d.identifier in ?1")
	Set<DocUnit> findByIdentifierInWithDocs(Iterable<String> idDocs);

	@Query("select distinct d " + "from DocUnit d " + "left join fetch d.library " + "left join fetch d.project "
			+ "left join fetch d.lot " + "where d.identifier in ?1")
	List<DocUnit> findByIdentifierInWithProj(Iterable<String> idDocs);

	@Query("select distinct d " + "from DocUnit d " + "left join fetch d.physicalDocuments p "
			+ "left join fetch p.train t " + "where d.lot.identifier = ?1 and p.digitalId = ?2")
	...
	@Query("select d " + "from DocUnit d " + "left join fetch d.exportData ed " + "left join fetch ed.properties "
			+ "left join fetch d.archiveItem ai " + "left join fetch ai.collections " + "left join fetch ai.subjects "
			+ "left join fetch ai.headers " + "where d.identifier = ?1")
	...
	@Query("select distinct d from DocUnit d " + "left join fetch d.library lib " + "left join fetch d.records r "
			+ "left join fetch r.properties p " + "left join fetch p.type " + "left join fetch d.archiveItem ai "
			+ "left join fetch ai.collections " + "left join fetch ai.subjects " + "left join fetch ai.headers "
			+ "where d.identifier in ?1")
