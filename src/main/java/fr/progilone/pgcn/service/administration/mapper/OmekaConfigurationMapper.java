...
public abstract class OmekaConfigurationMapper {
	...
	@Mappings({ @Mapping(target = "omekaCollections", ignore = true), @Mapping(target = "omekaItems", ignore = true) })
	...
	protected void updateObj(final OmekaConfigurationDTO dto, @MappingTarget final OmekaConfiguration conf) {
		// Chargement depuis un dto : il nous manque l'objet confOmeka sur chaque
		// OmekaList
		if (conf.getOmekaLists() != null) {
			conf.getOmekaLists().stream().filter(ol -> ol.getName() != null).forEach(ol -> ol.setConfOmeka(conf));
		}
	...
	protected void updateDto(final OmekaConfiguration conf, @MappingTarget final OmekaConfigurationDTO dto) {
		...
		final List<OmekaListDTO> collections = conf.getOmekaLists()
			.stream()
		...
		final List<OmekaListDTO> items = conf.getOmekaLists()
			.stream()
