...
public class EadMappingEvaluationService {
	...
	@Autowired
	public EadMappingEvaluationService(@Qualifier("groovy") final ScriptEngine engine,
	 ...
	 * @param rule
	 * @param bindings
	 * @return un boolean résultat de l'évaluation de la condition, true si pas de
	 * condition définie, false si pb de compilation ou d'évaluation
	...
	public boolean evalCondition(CompiledMappingRule rule, Map<String, Object> bindings) {
		try {
			...
			LOG.error(
					"Une erreur s'est produite lors de l'évaluation de la condition.\n- bindings définis: {}\n- condition: {}",
					StringUtils.join(bindings.keySet(), ", "), rule.getRule().getCondition());
	 ...
	 * @param rule
	 * @param bindings
	 * @return un objet résultat de l'évaluation de l'expression, ou null si pas
	 * d'expression ou si une erreur s'est produite
	 * @throws ScriptException
	 */
	public Object evalExpression(CompiledMappingRule rule, Map<String, Object> bindings) {
		try {
			...
			LOG.error(
					"Une erreur s'est produite lors de l'évaluation de l'expression.\n- bindings définis: {}\n- expression: {}",
					StringUtils.join(bindings.keySet(), ", "), rule.getRule().getExpression());
			LOG.error(e.getMessage(), e);
			return null;
		}
	}
	...
	private Optional<CompiledScript> compileStatement(final CompiledStatement compiledStatement) {
		...
		for (final CustomScript customScript : compiledStatement.getCustomScripts()) {
			if (compiledStatement.getConfigScript() != null) {
				final String cfgScript = StringUtils.joinWith("\n", customScript.getConfigScript(),
						compiledStatement.getConfigScript());
				eval(cfgScript, bindings);
			}
		}

		// Génération du script à partir de la règle de mapping saisie par
		// l'utilisateur:
		...
		// Script d'initialisation éxécuté avant tout appel du script saisi par
		// l'utilisateur
	 ...
	 * @param script
	 * @return Optional vide si une exception se produit au cours de la compilation du
	 * script
	 */
	...
	private Map<String, CustomScript> getScriptBinding(final CompiledStatement compiledStatement) {
		return compiledStatement.getCustomScripts()
			.stream()
			.collect(Collectors.toMap(CustomScript::getCode, Function.identity(), (a, b) -> a));
	}
