...
public class SubfieldsFormatter extends CustomScript {
	...
	public String format(final DataField field) {
		...
		// pas de config: on concatène tous les sous-champs en conservant leur ordre
		// d'apparition
	 ...
	 * @param groupPrefix
	 * @param groupSuffix
	 * @param subfldCfg liste de préfixes et de codes
	 */
	...
	public String getConfigScript() {
		return "def " + SCRIPT_NAME + "Add = {\n"
				+ "      String code, String prefix = null, String suffix = null -> script." + getCode()
				+ ".add((char)code, prefix, suffix)\n" + "}\n" + "def " + SCRIPT_NAME + "AddGroup = {\n"
				+ "      String prefix, String suffix, String... config -> script." + getCode()
				+ ".addGroup(prefix, suffix, config)\n" + "}\n" + "def " + SCRIPT_NAME + "Filter = {\n"
				+ "      String... tags -> script." + getCode() + ".setFilterCodes((char[])tags)\n" + "}\n" + "def "
				+ SCRIPT_NAME + "Separator = {\n" + "      String separator -> script." + getCode()
	...
	public String getInitScript() {
		return "def " + SCRIPT_NAME + " = {\n" + "      DataField field -> script." + getCode() + ".format(field)\n"
				...
				+ "  script." + getCode() + ".setTransliterate(" + MarcMappingEvaluationService.BINDING_FN_TRANSLITERATE
				+ ".&getValue)\n" + "}\n";
	}
	...
	private interface Config {
		...
		/**
		 * Application de cette configuration de formattage, et ajout du résultat dans
		 * value
		 * @param value
		 * @param subfields
		 */
					...
					.ifPresent(cfg -> {
						...
						if (cfg instanceof SubfieldGroupConfig) {
							if (!runOnce.contains(cfg)) {
								...
								final List<Subfield> matchingSubfields = subfields.stream()
									.filter(subfield2 -> cfg.match(subfield2.getCode()))
									.collect(Collectors.toList());
								// Application des sous-config sur la liste des
								// sous-champs correspodants
