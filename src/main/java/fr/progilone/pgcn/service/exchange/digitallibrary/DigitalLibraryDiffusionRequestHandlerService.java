...
public class DigitalLibraryDiffusionRequestHandlerService {
	...
	@Autowired
	public DigitalLibraryDiffusionRequestHandlerService(
			final DigitalLibraryDiffusionService digitalLibraryDiffusionService,
	...
	public void automaticDigitalLibraryExport() {
		...
		final List<DocUnit> docsToExport = digitalLibraryDiffusionService
			.findDocUnitsReadyForDigitalLibraryExportByLibrary();
		int i = 0;
		...
		for (final DocUnit doc : docsToExport) {
			...
			if (digitalLibraryDiffusionService.exportDocToDigitalLibrary(doc, true, i == 1, lastDoc)) {
				if (workflowService.isStateRunning(doc.getIdentifier(),
						WorkflowStateKey.DIFFUSION_DOCUMENT_DIGITAL_LIBRARY)) {
	 ...
	 */
	@Async
	public void exportDocToDigitalLibrary(final DocUnit doc, final String userId, final boolean multiple,
			final boolean firstDoc, final boolean lastDoc) {
		if (digitalLibraryDiffusionService.exportDocToDigitalLibrary(doc, multiple, firstDoc, lastDoc)) {
			if (workflowService.isStateRunning(doc.getIdentifier(),
					WorkflowStateKey.DIFFUSION_DOCUMENT_DIGITAL_LIBRARY)) {
				workflowService.processState(doc.getIdentifier(), WorkflowStateKey.DIFFUSION_DOCUMENT_DIGITAL_LIBRARY,
						userId);
			}
		}
		else {
			if (workflowService.isStateRunning(doc.getIdentifier(),
					WorkflowStateKey.DIFFUSION_DOCUMENT_DIGITAL_LIBRARY)) {
				workflowService.rejectState(doc.getIdentifier(), WorkflowStateKey.DIFFUSION_DOCUMENT_DIGITAL_LIBRARY,
						userId);
			}
		}
