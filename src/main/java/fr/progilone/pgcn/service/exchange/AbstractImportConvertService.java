...
public abstract class AbstractImportConvertService {
	...
	// Taille maximale des chaînes de caratères mappées sur des champs de DocUnit et
	// BibliographicRecord
	protected static final int COL_MAX_WIDTH = 255;

	// Taille maximale des chaînes de caratères indexées, mappées sur des champs de
	// DocUnit et BibliographicRecord
	...
	protected void setDefaultValues(final DocUnit docUnit) {
		if (!docUnit.getRecords().isEmpty()) {
			...
			// Copie des droits
			final DocUnit.RightsEnum rights = bibRecord.getProperties()
				.stream()
				.filter(p -> StringUtils.equals("rights", p.getType().getIdentifier()))
				.map(p -> {
					try {
	...
	/**
	 * Création des {@link DocProperty} de bibRecord en fonction de la règle de mapping,
	 * et des valeurs récupérées
	 * @param bibRecord
	 * @param typeId
	 * @param propertyTypes
	 * @param values
	 * @param baseRank
	 */
	protected void setDocProperty(final BibliographicRecord bibRecord, final String typeId,
			final Map<String, DocPropertyType> propertyTypes, final List<String> values, final Integer baseRank) {
		// Type de propriété
	...
	protected void setDocProperty(final BibliographicRecord bibRecord, final DocPropertyType propertyType,
			final List<String> values, final Integer baseRank) {
		int rank = 0;
		if (bibRecord.getPropertyOrder() == PropertyOrder.BY_PROPERTY_TYPE) {
			if (baseRank != null) {
				rank = baseRank * 10000;
			}
			rank += bibRecord.getProperties()
				.stream()
	...
	/**
	 * Alimentation des champs d'un objet en fonction de la règle de mapping, et des
	 * valeurs récupérées
	 * @param object
	 * @param fieldName
	 * @param values
	...
	protected void setObjectField(final Object object, final String fieldName, final List<String> values) {
		...
		try {
			// Vérification de la valeur actuelle (si la valeur est un type primitif, elle
			// n'est jamais null...)
			final Object current = PropertyUtils.getSimpleProperty(object, fieldName);
			if (current != null
					&& !PropertyUtils.getPropertyDescriptor(object, fieldName).getPropertyType().isPrimitive()) {
				LOG.debug("Le champ {} est déjà défini avec la valeur {} pour l'objet {}", fieldName,
						String.valueOf(current), object.getClass().getName());
				return;
			}
			...
			if (values.size() > 1) {
				LOG.debug("Il y a plus d'une valeur trouvée pour le champ {} ({}); seule la première sera récupérée.",
						fieldName, StringUtils.join(values, ", "));
			}
			...
			// Non géré
				...
				LOG.warn("Le type {} de la propriété {} n'est pas géré dans le mapping", fieldType.getName(),
						fieldName);
			}
	...
	protected boolean hasDocProperty(final BibliographicRecord bibRecord, final String typeId) {
		return bibRecord.getProperties()
			.stream()
			.anyMatch(p -> StringUtils.equals(p.getType().getIdentifier(), typeId));
	}
