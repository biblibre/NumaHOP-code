...
public class ImportMarcService extends AbstractImportService {
	...
	public ImportMarcService(final DeduplicationService deduplicationService,
			...
			final ImportReportService importReportService, final MarcMappingEvaluationService mappingEvaluationService,
			final MarcToDocUnitConvertService convertService, final MappingService mappingService,
	 ...
	 * @param importFiles Fichier de notices à importer
	 * @param fileFormat Format du fichier (MARC, MARCJSON, MARCXML)
	 * @param dataEncoding Encodage du fichier
	 * @param mappingId Identifiant du mapping
	 * @param mappingChildId Identifiant du mapping pour l'import des exemplaires de
	 * périodiques
	 ...
	 */
	@Async
	public void importMarcAsync(final List<File> importFiles, final FileFormat fileFormat,
			final DataEncoding dataEncoding, final String mappingId, final String mappingChildId,
		...
		try {
			...
			for (final File importFile : importFiles) {
				report = preimportFile(importFile, fileFormat, dataEncoding, mappingId, mappingChildId, parentKeyExpr,
						report, archivable, distributable);
			}
			/*
			...
			LOG.info("Les fichiers {} {} sont traités avec le statut {}", fileFormat, report.getFilesAsString(),
					report.getStatus());
	 ...
	 */
	@Async
	public void importMarcXmlAsync(final List<File> importFiles, final String mappingId, final ImportReport report,
			final boolean stepValidation, final boolean stepDeduplication,
			final ImportedDocUnit.Process defaultDedupProcess, final boolean archivable, final boolean distributable) {
		importMarcAsync(importFiles, FileFormat.MARCXML, DataEncoding.UTF_8, mappingId, null, null, null, report,
				stepValidation, stepDeduplication, defaultDedupProcess, archivable, distributable);
	}
	 ...
	 */
	private ImportReport preimportFile(final File importFile, final FileFormat fileFormat,
			final DataEncoding dataEncoding, final String mappingId, final String mappingChildId,
			final String parentKeyExpr, final ImportReport report, final boolean archivable,
		...
		if (fileFormat == FileFormat.MARC) {
			final String encoding = dataEncoding == DataEncoding.UTF_8 ? StandardCharsets.UTF_8.name()
					: dataEncoding == DataEncoding.ISO_8859_1 ? StandardCharsets.ISO_8859_1.name() : null;
			return importMarcRecords(importFile, dataEncoding, mappingId, mappingChildId, parentKeyExpr,
					in -> new MarcStreamReader(in, encoding), report, archivable, distributable);
		}
		// MARC JSON
		else if (fileFormat == FileFormat.MARCJSON) {
			return importMarcRecords(importFile, dataEncoding, mappingId, mappingChildId, parentKeyExpr,
					MarcJsonReader::new, report, archivable, distributable);
		}
		// MARC XML
		else if (fileFormat == FileFormat.MARCXML) {
			return importMarcRecords(importFile, dataEncoding, mappingId, mappingChildId, parentKeyExpr,
					MarcXmlReader::new, report, archivable, distributable);
		}
		// Non géré
		else {
			throw new PgcnTechnicalException("Le format de fichier " + fileFormat + " n'est pas supporté (fichier "
					+ importFile.getAbsolutePath() + ") " + "par le service ImportMarcService");
		}
	}
	 ...
	 * @param report
	 */
	private ImportReport importMarcRecords(final File importFile, final DataEncoding dataEncoding,
			final String mappingId, final String mappingChildId, final String parentKeyExpr,
			final Function<InputStream, MarcReader> readerBuilder, ImportReport report, final boolean archivable,
			final boolean distributable) throws PgcnTechnicalException {

		try (final InputStream fileIn = new FileInputStream(importFile);
			...
			report = importRecord(reader, charConverter, mappingId, mappingChildId, parentKeyExpr, report, archivable,
					distributable);
			return report;
	 ...
	 * @param importReport
	 */
	private ImportReport importRecord(final MarcReader reader, final CharConverter charConverter,
			final String mappingId, final String mappingChildId, final String parentKeyExpr,
			final ImportReport importReport, final boolean archivable, final boolean distributable)
			throws PgcnTechnicalException {
		...
		if (mappingChildId != null) {
			...
			if (mappingChild == null) {
				throw new PgcnTechnicalException(
						"Il n'existe pas de mapping avec l'identifiant " + mappingChildId + " (périodiques)");
			}
			// Pré-compilation du mapping
		...
		final CompiledMappingRule joinScript = StringUtils.isNotBlank(parentKeyExpr)
				? mappingEvaluationService.compileMapping(parentKeyExpr, charConverter) : null;
		...
		new TransactionalJobRunner<>(new MarcRecordIterator(reader), transactionService)
			// Configuration du job
			.setCommit(BULK_SIZE)
			// pas de parallélisation pour prévenir les transaction
			// lock au moment de importDocUnitService.create
			// Traitement principal
			.forEach((record) -> {
				try {
					// Conversion du record en unité documentaire
					final List<DocUnitWrapper> docUnits = convertService.convert(record, charConverter, compiledMapping,
							joinScript, propertyTypes);

					// #HIERARCHY_IN_SINGLE_NOTICE: Conversion des
					// exemplaires de la notice courante
					if (mappingChild != null && !docUnits.isEmpty()) {
						...
						final List<DocUnitWrapper> childrenDocUnits = convertService.convert(record, charConverter,
								compiledMappingChild, null, propertyTypes);
					...
					// Si plusieurs UD sont extraites de cette
					// notices, on renseigne le code de
					// regroupement
