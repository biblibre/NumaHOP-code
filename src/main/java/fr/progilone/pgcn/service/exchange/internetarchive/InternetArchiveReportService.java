...
public class InternetArchiveReportService {
	...
	@Autowired
	public InternetArchiveReportService(final InternetArchiveReportRepository iaReportRepository,
	...
	public void init() {
		...
		// l'application
		final List<InternetArchiveReport> interruptedImports = iaReportRepository
			.findByStatusIn(InternetArchiveReport.Status.EXPORTING, InternetArchiveReport.Status.SENDING);
		for (final InternetArchiveReport report : interruptedImports) {
			LOG.warn("L'export Internet Archive de {}, démarré le {}, a été interrompu au statut {}",
					report.getIdentifier(), report.getCreatedDate(), report.getStatus());
	 ...
	 * status SENDING. Le nombre total de fichiers à envoyer
	 * @param report
	 * @param total
	 * @return
	 */
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	public InternetArchiveReport setReportArchived(final InternetArchiveReport report, final LocalDateTime dateArchive,
