...
public class DcToDocUnitConvertService extends AbstractImportConvertService {
	 ...
	 * @return
	 */
	public <T extends ElementContainer> DocUnit convert(final T dcGroup, final Library library,
		...
		dcGroup.getAny()
			...
			.collect(Collectors.groupingBy(JAXBElement::getName,
					Collectors.mapping(JAXBElement::getValue, Collectors.toList())))
			.forEach((eltName, literals) -> {
				final List<String> values = literals.stream()
					.flatMap(c -> c.getContent().stream())
					.collect(Collectors.toList());
				// Récupération de la propriété PGCN correspondant à la propriété Dublin
				// Core
	...
	/**
	 * Recherche la propriété PGCN {@link DocPropertyType} correspondant à la propriété
	 * Dublin Core namespaceURI + localPart
	 * @param propertyTypes
	 * @param namespaceURI
	 * @param localPart
	 * @return
	 */
	private Optional<DocPropertyType> getProperty(final List<DocPropertyType> propertyTypes, final String namespaceURI,
		...
		switch (namespaceURI) {
			...
			default:
				LOG.warn(
						"L'espace de nommage {} ne correspond à aucun DocPropertyType.DocPropertySuperType; la propriété {} est ignorée",
