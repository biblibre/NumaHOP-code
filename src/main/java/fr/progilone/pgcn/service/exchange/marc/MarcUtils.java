...
public class MarcUtils {
	...
	/**
	 * Conversion d'une chaine de caractères à l'aide du {@link CharConverter} passé en
	 * paramètre
	 * @param value
	 * @param charConverter
	 * @return
	...
	/**
	 * Renvoie une instance de CharConverter correspondant générant une chaîne dans
	 * l'encodage spécifié, à partir d'une chaîne l'unicode
	 * @param dataEncoding
	 * @return
	 */
	...
	/**
	 * Renvoie une instance de CharConverter correspondant à l'encodage spécifié, et
	 * générant de l'unicode
	 * @param dataEncoding
	 * @return
	 */
	...
	public static List<Record> splitRecordByMarcKeys(final Record record, final List<MarcKey> marcKeys) {
		...
		for (final VariableField recordField : record.getVariableFields()) {
			...
			DataField copyOfDataField = null; // on garde une référence au datafield, pour
												// ne pas le dupliquer
			...
			if (matchedKeys.isEmpty()) {
				...
				if (recordField instanceof DataField) {
					...
					for (final Subfield subfield : f.getSubfields()) {
						if (matchedKeys.stream()
							.noneMatch(key -> key.getCode() != null && key.getCode() == subfield.getCode())) {
							continue;
						}
						// Si on retrouve une nouvelle fois le sous-champ key.code, on
						// recréé un nouveau datafield
						if (copyOfDataField == null || copyOfDataField.getSubfield(subfield.getCode()) != null) {
							copyOfDataField = marcFactory.newDataField(f.getTag(), f.getIndicator1(),
									f.getIndicator2());
						...
						f.getSubfields()
							.stream()
							.filter(sub -> matchedKeys.stream()
								.noneMatch(oth -> Character.compare(sub.getCode(), oth.getCode()) == 0))
							.forEach(sub -> fCopyOfDataField
								.addSubfield(marcFactory.newSubfield(sub.getCode(), sub.getData())));
					}
				}
			}
		}
		...
		return itemFieldCombos.stream().map(fieldCombo -> {
			...
			Stream.concat(fieldCombo.values().stream(), otherFields.stream()).map(f -> {
				if (f instanceof DataField) {
					...
					// champ
					final DataField copyOfField = marcFactory.newDataField(f.getTag(), dataField.getIndicator1(),
							dataField.getIndicator2());
					// sous-champs
					dataField.getSubfields()
						.forEach(subfield -> copyOfField
							.addSubfield(marcFactory.newSubfield(subfield.getCode(), subfield.getData())));
					return copyOfField;
