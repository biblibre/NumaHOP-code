...
public class ImportReportService {
	...
	@Autowired
	public ImportReportService(final DocUnitRepository docUnitRepository, final FileStorageManager fm,
			final ImportReportRepository importReportRepository,
			final ImportedDocUnitRepository importedDocUnitRepository, final TransactionService transactionService,
			final WebsocketService websocketService, final ConditionReportRepository conditionReportRepository,
			final ImageMetadataValuesRepository imageMetadataValuesRepository, final DocUnitService docUnitService) {
	...
	public void init() {
		...
		final List<ImportReport> interruptedImports = importReportRepository.findByStatusIn(ImportReport.Status.PENDING,
				ImportReport.Status.PRE_IMPORTING, ImportReport.Status.DEDUPLICATING, ImportReport.Status.IMPORTING);
		for (final ImportReport report : interruptedImports) {
			LOG.warn("L'import des fichiers {}, démarré le {}, a été interrompu au statut {}",
					report.getFilesAsString(), report.getStart(), report.getStatus());
	 ...
	 */
	@Transactional
	public ImportReport createSimpleImportReport(final String fileName, final Long fileSize,
			final FileFormat fileFormat, final DataEncoding dataEncoding, final String libraryId,
			final String projectId, final String lotId, final String mappingId) {
	...
	@Transactional
	public ImportReport createImportReport(final List<MultipartFile> files, final ImportReport.Type type,
			final FileFormat fileFormat, final DataEncoding dataEncoding, final String libraryId,
			final String projectId, final String lotId, final String mappingId, final String mappingChildrenId,
			final String parentReportId, final String join) {
	 ...
	 */
	@Transactional
	public ImportReport createCSVImportReport(final List<MultipartFile> files, final ImportReport.Type type,
			final FileFormat fileFormat, final DataEncoding dataEncoding, final String libraryId,
			final String projectId, final String lotId, final String mappingId, final String parentReportId,
			final String join) {
		final ImportReport report = new ImportReport();
		report.setType(type);
		report.setFileFormat(fileFormat);
	...
	public ImportReport createDCImportReport(final List<MultipartFile> files, final ImportReport.Type type,
			final FileFormat fileFormat, final DataEncoding dataEncoding, final String libraryId,
			final String projectId, final String lotId, final String mapping, final String join) {
		final Library library = new Library();
		library.setIdentifier(libraryId);
	...
	public ImportReport failReport(ImportReport importReport, final String message) {
		...
		importReport.setEnd(now);
		importReport.setMessage(
				"Arrêt imprévu du traitement au statut " + importReport.getStatus() + " avec l'erreur: " + message);
	...
	/**
	 * Construction d'une {@link Map} contenant les informations sur le statut du rapport
	 * d'import
	 * @param report
	 * @return
	 */
	...
	public Page<ImportReport> search(final String search, final List<String> users,
			final List<ImportReport.Status> status, final List<String> libraries, final Integer page,
			final Integer size) {
	...
	public void delete(final String identifier) {
		...
		final List<String> docUnitIds = importReportRepository.findDocUnitIdentifiersByReportAndDocUnitState(identifier,
				DocUnit.State.NOT_AVAILABLE);

		LOG.debug("Suppression des références aux unités documentaires dans le rapport d'import {}", identifier);
		new TransactionalJobRunner<>(importedIds, transactionService).setCommit(BATCH_SIZE)
			.forEachGroup(BATCH_SIZE, ids -> {
		...
		new TransactionalJobRunner<>(docUnitIds, transactionService).setCommit(BATCH_SIZE)
			.forEachGroup(BATCH_SIZE, id -> {
	...
	public File saveImportFile(final InputStream source, final ImportReport importReport, final String fileName) {
		...
		return fm.copyInputStreamToFileWithOtherDirs(source, root.toFile(), Arrays.asList(importReport.getIdentifier()),
				destName, true, true);
	}
