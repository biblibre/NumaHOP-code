...
public class OmekaRequestHandlerService {
	...
	public OmekaRequestHandlerService(final OmekaService omekaService, final WorkflowService workflowService,
			final OmekaConfigurationService omekaConfigurationService, final LibraryService libraryService,
			final TransactionService transactionService, final DocUnitRepository docUnitRepository) {
				...
				job.setCommit(1).setReadOnly(false).setElementName("Doc Unit Exporting Omeka").forEach(docId -> {
					...
					if (omekaService.exportDocToOmeka(doc, true, lastDoc, i.get() == 1)) {
						...
						if (workflowService.isStateRunning(doc.getIdentifier(),
								WorkflowStateKey.DIFFUSION_DOCUMENT_OMEKA)) {
					...
					LOG.info("Fin export vers OMEKA - DocUnit[{}] - Statut: {}", doc.getIdentifier(),
							doc.getOmekaExportStatus().name());
					return true;
				}).process();
			});
	}
	 ...
	 */
	@Async
	public void exportDocToOmeka(final DocUnit docUnit, final String userId, final boolean multiple,
			final boolean lastDoc, final boolean firstDoc) {
		if (omekaService.exportDocToOmeka(docUnit, multiple, lastDoc, firstDoc)) {
			if (workflowService.isStateRunning(docUnit.getIdentifier(), WorkflowStateKey.DIFFUSION_DOCUMENT_OMEKA)) {
				workflowService.processState(docUnit.getIdentifier(), WorkflowStateKey.DIFFUSION_DOCUMENT_OMEKA,
						userId);
			}
		}
		else {
