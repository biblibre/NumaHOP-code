...
public class DocUnitToCSVService {
	...
	@Autowired
	public DocUnitToCSVService(final DocPropertyTypeService docPropertyTypeService, final DocUnitService docUnitService,
	...
	/**
	 * Convertit l'unité documentaire en CSV, et retourne le résultat sous la forme d'une
	...
	public void convertFromList(final OutputStream out, final Collection<String> docUnitIds, final List<String> fields,
			final List<String> docUnitFields, final List<String> bibFields, final List<String> physFields,
	...
	/**
	 * Convertit les UD du lot en CSV, et écrit le résultat dans l'OutputStream passé en
	 * paramètre
	 * @param out
	 * @param lotId
	 * @param fields
	 ...
	 */
	@Transactional(readOnly = true)
	public void convertLotFromList(ServletOutputStream out, String lotId, List<String> fields,
			final List<String> docUnitFields, final List<String> bibFields, final List<String> physFields,
			final String encoding, final char separator) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void convert(final OutputStream out, final Collection<DocUnit> docUnits,
			List<DocPropertyType> propertyTypes, final List<String> docUnitFields, final List<String> bibFields,
			final List<String> physFields, final String encoding, final char separator) throws IOException {
		...
		try (Writer writer = new OutputStreamWriter(out, encoding);
				CSVWriter csvWriter = new CSVWriter(writer, separator, DEFAULT_QUOTE_CHARACTER,
			...
			for (final DocUnit docUnit : docUnits) {
				...
				final List<String> physValues = getCollectionOfObjectsValues(docUnit.getPhysicalDocuments(),
						physFields);
	 ...
	 * @param physFields
	 */
	private void writeHeader(final CSVWriter csvWriter, final List<DocPropertyType> propertyTypes,
			final List<String> docUnitFields, final List<String> bibFields, final List<String> physFields) {
	 ...
	 * @param physValues
	 */
	private void writeBody(final CSVWriter csvWriter, final List<DocPropertyType> propertyTypes,
			final Collection<BibliographicRecord> records, final List<String> docUnitValues,
			final List<String> bibFields, final List<String> physValues) {
		...
		for (final BibliographicRecord record : records) {
			...
			final Map<DocPropertyType, List<DocProperty>> propByType = record.getProperties()
				.stream()
				.collect(Collectors.groupingBy(DocProperty::getType));
