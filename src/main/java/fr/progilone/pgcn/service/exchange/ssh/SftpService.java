...
public class SftpService {
	...
	/**
	 * Fichier contenant les RSA key fingerprint des serveurs sur lesquels se
	 * connecter<br/>
	 * <ul>
	 * <li>RSA key fingerprint:
	 * <code> ssh -oHostKeyAlgorithms='ssh-rsa' user@server</code></li>
	 * <li>Noms des serveurs non hachés: fichier <code> .ssh/config</code>, propriété
	 * <code> HashKnownHosts no</code></li>
	 * </ul>
	 *
	 ...
	 * - yes (conseillé): les connexions aux serveurs absents du fichier knownHosts sont
	 * rejetées - no: toutes les connexions sont autorisées; si un fichier knownHosts est
	 * renseignés, il est alimenté automatiquement avec les nouvelles connexions
	 */
	...
	public void sftpPut(final SftpConfiguration conf, final Path localSource) throws PgcnTechnicalException {
		...
		if (localSource != null) {
			try {
				...
				LOG.debug("Envoi de {} vers {}@{}:{}:{}", localSource.toAbsolutePath().toString(),
						session.getUserName(), session.getHost(), session.getPort(), conf.getTargetDir());
				...
				if (pathExists) {
					...
					if (fileName != null) {
						...
						LOG.warn("Le répertoire distant {} existe déjà, le transfert est annulé",
				...
				LOG.debug("Transfert de {} terminé; fermeture de la connexion SFTP vers {}@{}:{}",
						localSource.toAbsolutePath().toString(), conf.getUsername(), conf.getHost(), conf.getPort());
				if (channelSftp != null) {
					channelSftp.exit();
				}
				if (session != null) {
	...
	private static void goToTargetDir(final String targetDir, final ChannelSftp channelSftp) throws SftpException {
		...
		if (!targetExists) {
			...
			String[] path = targetDirPath.toString()
	...
	private static boolean checkPath(final Path localSource, final ChannelSftp.LsEntry e) {
		// Si le localSource ne retourne pas de fileName, considérer que l'on est sur la
		// root
	...
	/**
	 * Vérifie l'existence du fichier / répertoire local dans le répertoire distant
	 * courant
	 * @param localSource
	 * @param channelSftp
	 * @return
	 ...
	 * @param localSource fichier ou répertoire à transférer
	 * @param channelSftp connexion SFTP établie
	 */
	 ...
	 */
	private Session openSession(final SftpConfiguration conf, final boolean disableKeyCheck)
			throws JSchException, PgcnTechnicalException {
		LOG.debug("Ouverture d'une session distante sur {}@{}:{}, StrictHostKeyChecking = {}", conf.getUsername(),
				conf.getHost(), conf.getPort(), !disableKeyCheck);
	...
	private ChannelSftp getChannelSftp(final Session session) throws JSchException {
		LOG.debug("Initialisation de la connexion SFTP pour {}@{}:{}", session.getUserName(), session.getHost(),
				session.getPort());
