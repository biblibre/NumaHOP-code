...
public class UIOcrLangConfigMapper {
	...
	public void mapDtoToConfig(final OcrLangConfigurationDTO dto, final OcrLangConfiguration updated) {
		...
		dto.getOcrLanguages().stream().map(OcrLanguageMapper.INSTANCE::dtoToObject).forEach(lng -> {
	...
	public OcrLangConfigurationDTO mapConfigToDto(final OcrLangConfiguration config,
			final OcrLangConfigurationDTO dto) {
		...
		final List<OcrLanguageDTO> languages = config.getActivatedOcrLanguages()
			.stream()
