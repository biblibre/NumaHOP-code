...
public class ConditionReportExportService {
	...
	/**
	 * Génération du fichier qui servira de modèle d'import des constats d'état des unités
	 * documentaires passées en paramètres
	 * @param out
	 * @param docUnitIds
	 * @param format
	 * @throws IOException
	...
	public void writeReportTemplate(final OutputStream out, final List<String> docUnitIds, final WorkbookFormat format)
			throws IOException {
		...
		final TemplateBuilder templateBuilder = new TemplateBuilder(format).setDescProperties(descProperties)
			.setDescValues(descValues);
		templateBuilder.initWorkbook();

		// Unités documentaires
		final List<DocUnit> docUnits = docUnitService.findAllById(docUnitIds)
			.stream()
			.sorted((f1, f2) -> f1.getPgcnId().compareTo(f2.getPgcnId()))
			.collect(Collectors.toList());

		for (final DocUnit docUnit : docUnits) {
			final List<PropertyConfiguration> configurations = propertyConfigurationService
				.findByLibrary(docUnit.getLibrary());
	...
	/**
	 * Classe regroupant toutes les opérations de génération du modèle d'import de constat
	 * d'état
	 */
	private final class TemplateBuilder {
		...
		private void buildConfSheet() {
			...
			for (final DescriptionProperty property : descProperties) {
				...
				final DataValidationConstraint constraint = format == XLS
						? ((HSSFDataValidationHelper) validationHelper)
							.createFormulaListConstraint(getNameName(property))
		...
		public void buildSheet(final DocUnit docUnit, final List<PropertyConfiguration> configurations) {
			...
			rowNb += 2;
			// Type de document
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.type"),
					DescriptionProperty.Type.TYPE, descProperties, configurations);
			// Descriptions
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.description"),
					DescriptionProperty.Type.DESCRIPTION, descProperties, configurations);
			// Etat du document
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.state"),
					DescriptionProperty.Type.STATE, descProperties, configurations);
			...
			// État de la reliure
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.binding"),
					DescriptionProperty.Type.BINDING, descProperties, configurations);
			rowNb = buildInfoBlock(sheet, rowNb, PropertyConfiguration.InternalProperty.BINDING_DESC, configurations);
			// Numérotation
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.numbering"),
					DescriptionProperty.Type.NUMBERING, descProperties, configurations);
			// Estimation du nombre de vues
			rowNb = buildViewBlock(sheet, ++rowNb);
			rowNb = buildInfoBlock(sheet, ++rowNb, PROP_DESC_OTHER);
			// Points de vigilance
			rowNb = buildDetailBlock(sheet, ++rowNb, messageService.getMessage("condreport", "title.vigilance"),
					DescriptionProperty.Type.VIGILANCE, descProperties, configurations);
		...
		private short buildDocumentRows(final Sheet sheet, short rowNb, final DocUnit docUnit) {
			...
			row.createCell(0).setCellValue(docUnit.getIdentifier()); // identifiant
																		// interne
			createStyledCell(row, 2, messageService.getMessage("condreport", "title.docunit.import"), Style.BOLD_COLOR);
			createStyledCell(row, 3, messageService.getMessage("condreport", "validation.boolean.true"),
					Style.BOLD_COLOR);
		 ...
		 * @return
		 */
		private short buildDetailBlock(final Sheet sheet, short rowNb, final String title,
				final DescriptionProperty.Type type, final List<DescriptionProperty> bProperties,
			...
			for (final DescriptionProperty property : bProperties) {
				...
				final Optional<PropertyConfiguration> confOpt = configurations.stream()
					.filter(c -> c.getDescProperty() != null
							&& StringUtils.equals(c.getDescProperty().getIdentifier(), property.getIdentifier()))
					.findAny();
		...
		private short buildInsuranceBlock(final Sheet sheet, short rowNb) {
			...
			addIntegerValidationConstraint(sheet, new CellRangeAddressList(rowNb, rowNb, 3, 3),
					messageService.getMessage("condreport", "INSURANCE"));
		 ...
		 * @return
		 */
		private short buildDimensionBlock(final Sheet sheet, short rowNb,
			...
			final Optional<PropertyConfiguration> confOpt = configurations.stream()
				.filter(c -> c.getInternalProperty() == property)
				.findAny();
			...
			addIntegerValidationConstraint(sheet, new CellRangeAddressList(rowNb, rowNb, 3, 5),
					messageService.getMessage("condreport", "DIMENSION"));
		 ...
		 * @return
		 */
		private short buildInfoBlock(final Sheet sheet, short rowNb, final PropertyConfiguration.InternalProperty field,
				final List<PropertyConfiguration> configurations) {
			final Row row = sheet.createRow(rowNb);
			row.createCell(0).setCellValue(TYPE_DETAIL);
			...
			final Optional<PropertyConfiguration> confOpt = configurations.stream()
				.filter(c -> c.getInternalProperty() == field)
				.findAny();
		...
		private short buildViewBlock(final Sheet sheet, short rowNb) {
			...
			addIntegerValidationConstraint(sheet, new CellRangeAddressList(rowNb, rowNb + 2, 3, 3),
		...
		/**
		 * Initialisation du builder: définition des {@link DescriptionValue} disponibles
		 * pour les états de reliure
		 * @param descValues
		 * @return
		 */
		...
		private void createBorderedCell(final Sheet sheet, final short rowNb, final int colNb,
				final BorderStyle borderStyle, final short borderColor) {
		...
		private void addBooleanValidationConstraint(final Sheet sheet, final CellRangeAddressList addressList) {
			...
			// pb de longueur de formule avec le format xls; si bloquant, baser la formule
			// sur des plages excel
			catch (final IllegalArgumentException e) {
				LOG.error("Une erreur s'est produite lors de l'ajout de la liste booléenne pour la plage {}: {}",
						addressList.getCellRangeAddresses()[0].formatAsString(), e.getMessage());
			}
		}
		 ...
		 * @param field
		 */
		private void addIntegerValidationConstraint(final Sheet sheet, final CellRangeAddressList addressList,
				final String field) {
			final DataValidationConstraint dvConstraint = this.format == XLS
					? ((HSSFDataValidationHelper) validationHelper).createIntegerConstraint(GREATER_OR_EQUAL, "0", null)
					: ((XSSFDataValidationHelper) validationHelper).createIntegerConstraint(GREATER_OR_EQUAL, "0",
							null);
			final DataValidation dataValidation = this.format == XLS
					? ((HSSFDataValidationHelper) validationHelper).createValidation(dvConstraint, addressList)
					: ((XSSFDataValidationHelper) validationHelper).createValidation(dvConstraint, addressList);
			...
			dataValidation.createPromptBox(field,
					messageService.getMessage("condreport", "validation.integer.promptText"));
		...
		private void addListValidationConstraint(final Sheet sheet, final short rowNb, final String property) {
			...
			if (descriptionsConstraints.containsKey(property)) {
				...
				// pb de longueur de formule avec le format xls; si bloquant, baser la
				// formule sur des plages excel
				catch (final IllegalArgumentException e) {
					LOG.error(
							"Une erreur s'est produite lors de l'ajout de la liste de validation pour la propriété {}: {}",
