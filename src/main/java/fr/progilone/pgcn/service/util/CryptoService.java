...
public class CryptoService {
	...
	/**
	 * Encrypte le texte text à l'aide du password et du salt fournis dans la
	...
	/**
	 * Décrypte des données cryptées et encodées en base64 à l'aide du password et du salt
	 * fournis dans la configuration de l'application
	 * @param text
	 * @return
	 * @throws PgcnTechnicalException
	 */
	 ...
	 * @param salt salt pour le cryptage
	...
	private String encrypt(final String text, final String password, final String salt) throws PgcnTechnicalException {
		...
		try {
			...
			// Ajout du vecteur d'initialisation en tête du résultat (pour permettre le
			// décryptage)
		...
		catch (NoSuchPaddingException | NoSuchAlgorithmException | IOException | InvalidAlgorithmParameterException
	 ...
	 * @param data données cryptées et encodées en base64
	 * @param password mot de passe pour le décryptage
	 * @param salt salt pour le décryptage
	 * @return
	 * @throws PgcnTechnicalException
	 */
	private String decrypt(final String data, final String password, final String salt) throws PgcnTechnicalException {
		...
		catch (NoSuchAlgorithmException | InvalidKeyException | InvalidAlgorithmParameterException | IOException
	 ...
	 * @param password Mot de passe permettant de générer la clé de cryptage / décryptage
	 * @param salt tableau d'octets encodés en Base64
	 * @return
	 * @throws PgcnTechnicalException
	 */
	private SecretKey getSecretKey(final String password, final String salt) throws PgcnTechnicalException {
		try {
			...
			final byte[] _salt = StringUtils.isEmpty(salt) ? Base64.getDecoder().decode(KEY_DEFAULT_SALT)
					: Base64.getDecoder().decode(salt);

			final SecretKeyFactory factory = SecretKeyFactory.getInstance(KEY_ALGORITHM);
			final KeySpec spec = new PBEKeySpec(_password, _salt, KEY_ITERATION_COUNT, KEY_LENGTH); // password,
																									// salt,
																									// iterationCount,
																									// keyLength
	 ...
	 * @param size taille du vecteur d'initialisation
	 * @return
	 */
	...
	/**
	 * @param in flux à décoder, préfixé par le vecteur d'initialisation
	 * @param size taille du vecteur d'initialisation
	 * @return
	 * @throws IOException
	 */
	private IvParameterSpec readIv(final InputStream in, final int size) throws IOException {
		...
		while (offset < size) {
			...
			if (read == -1) {
				throw new IOException("Le flux à décrypter est trop court pour contenir un vecteur d'initialisation de "
						+ size + " octets");
			}
			offset += read;
		}
