 ...
 * <p/>
 * Définir les relations à l'aide de
 * AuthorizationManager.setRequirements(mon_autorisation, mon_prérequis_1,
 * mon_prérequis_2, ...)
 * <p/>
...
public class AuthorizationManager {
	...
	/**
	 * Clé: autorisation Valeurs: liste des autorisations nécessaires pour bénéficier de
	 * l'autorisation clé
	 */
	private static Map<String, List<String>> requirements = Collections.synchronizedMap(new HashMap<>());

	// Initialisation des constantes (définition des liens de dépendances entre les
	// droits)
	static {
		try {
	...
	public static List<String> getDependencies(String authorization) {
		return AuthorizationManager.requirements.entrySet()
			.stream()
