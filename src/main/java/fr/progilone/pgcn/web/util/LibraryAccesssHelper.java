...
public class LibraryAccesssHelper {
	...
	/**
	 * Vérifie que l'utilisateur courant ait bien accès à object par rapport à sa
	 * bibliothèque
	 * @param request
	 * @param object
	 * @param getLibraryFn
	 ...
	 */
	@Transactional(readOnly = true)
	public <T> boolean checkLibrary(final HttpServletRequest request, final T object,
			final Function<T, Library> getLibraryFn, final String... bypassRoles) {
		// On ne contrôle pas la bibliothèque pour les utilisateurs ayant un rôle dans
		// bypassRoles
	...
	/**
	 * Renvoie les bibliothèques à inclure dans la recherche La liste renvoyée dépend des
	 * droits de l'utilisateur courant Si l'utilisateur est superadmin, on ne filtre pas
	 * la liste demandée Sinon il n'a droit d'accéder qu'à sa bibliothèque
	 * @param request
	 * @param libraryFilter
	 * @return
	 */
	@Transactional(readOnly = true)
	public List<String> getLibraryFilter(final HttpServletRequest request, final List<String> libraryFilter,
	 ...
	 */
	@Transactional(readOnly = true)
	public <T> Collection<T> filterObjectsByLibrary(final HttpServletRequest request, final Collection<T> objects,
			final Function<T, String> getLibraryIdFn, final String... bypassRoles) {
		// On ne contrôle pas la bibliothèque pour les utilisateurs ayant un rôle dans
		// bypassRoles
		final String[] roles = Arrays.copyOf(bypassRoles, bypassRoles.length + 2);
		roles[roles.length - 1] = AuthorizationConstants.SUPER_ADMIN;
