...
public class ExportController extends AbstractRestController {
	...
	@Autowired
	public ExportController(final DocUnitService docUnitService, final DocUnitToCSVService docUnitToCSVService,
			final DocUnitToJenaService docUnitToJenaService, final ExportEadService exportEadService,
	...
	public void exportDocUnitToCsv(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Export du CSV
			...
			try {
				...
				docUnitToCSVService.convertFromList(response.getOutputStream(),
						docUnits.stream().map(DocUnit::getIdentifier).collect(Collectors.toList()), fields,
						docUnitFields, bibFields, physFields, encoding, separator);
	...
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public void exportLotToCsv(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Export
			...
			try {
				...
				docUnitToCSVService.convertLotFromList(response.getOutputStream(), lotId, fields, docUnitFields,
						bibFields, physFields, encoding, separator);
	...
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public void exportDocUnitToRdfxml(final HttpServletRequest request, final HttpServletResponse response,
			@RequestParam(value = "type", required = false,
					defaultValue = "DC") final DocPropertyType.DocPropertySuperType superType,
		...
		// Export
			...
			try {
				writeResponseHeaderForDownload(response, MediaType.TEXT_XML_VALUE, null,
						getExportName(docUnit, superType.name()));
	...
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public void getEad(final HttpServletResponse response, @RequestParam(value = "docUnit") final String docUnitId)
