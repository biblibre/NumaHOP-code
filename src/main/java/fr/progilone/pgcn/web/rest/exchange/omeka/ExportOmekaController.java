...
public class ExportOmekaController {
	...
	@Autowired
	public ExportOmekaController(final DocUnitService docUnitService, final LibraryAccesssHelper libraryAccesssHelper,
			final OmekaConfigurationService omekaConfigurationService,
			final OmekaRequestHandlerService omekaRequestHandlerService, final AccessHelper accessHelper) {
	...
	/**
	 * Export Omeka unitaire. Génération du répertoire d'export et upload sur le serveur
	 * SFTP pour OMEKA (avec les métadonnées Dublin Core).
	 * @param request
	 * @param docUnitId
	 * @param conf
	 * @return
	...
	public ResponseEntity<?> exportDocUnitToOmekaWithDc(final HttpServletRequest request,
		...
		final Set<OmekaConfiguration> confs = omekaConfigurationService.findByLibraryAndActive(docUnit.getLibrary(),
				true);
	...
	/**
	 * Export Omeka multiple. Déclenche l'export OMEKA de la liste de documents spécifiés.
	 ...
	 * @param firstDoc
	 */
	private void exportOmeka(final HttpServletRequest request, final DocUnit docUnit, final boolean lastDoc,
