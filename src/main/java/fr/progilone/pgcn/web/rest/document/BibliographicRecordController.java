...
public class BibliographicRecordController extends AbstractRestController {
	...
	public BibliographicRecordController(final AccessHelper accessHelper,
			...
			final UIBibliographicRecordService uiBibliographicRecordService, final EsDocUnitService esDocUnitService,
			final LockService lockService, final WorkflowAccessHelper workflowAccessHelper) {
	...
	public ResponseEntity<BibliographicRecordDTO> create(@RequestBody final BibliographicRecordDTO record)
	...
	public void delete(@RequestBody final List<String> ids) {
		...
		uiBibliographicRecordService
			.delete(filteredByWorkflow.stream().map(BibliographicRecord::getIdentifier).collect(Collectors.toList()));
	}
	...
	public ResponseEntity<Page<SimpleBibliographicRecordDTO>> search(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiBibliographicRecordService.search(search, filteredLibraries, projects, lots,
				statuses, lastModifiedDateFrom, lastModifiedDateTo, createdDateFrom, createdDateTo, orphan, page, size,
				sorts), HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "searchAsList" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ DOC_UNIT_HAB0 })
	public ResponseEntity<Page<SimpleListBibliographicRecordDTO>> searchAsList(final HttpServletRequest request,
		...
		// Droits d'accès
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(uiBibliographicRecordService.searchAsList(search, filteredLibraries, projects, lots,
				statuses, trains, lastModifiedDateFrom, lastModifiedDateTo, createdDateFrom, createdDateTo, orphan,
				page, size, sorts), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<SimpleBibliographicRecordDTO>> findAll() {
		...
		// Droits d'accès
		final List<SimpleBibliographicRecordDTO> filteredDtos = filterRecordsDTOs(dtos,
				SimpleBibliographicRecordDTO::getIdentifier);
		return new ResponseEntity<>(filteredDtos, HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "all_operations" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	public ResponseEntity<List<SimpleBibliographicRecordDTO>> findAllOperations(
			@RequestParam("identifier") final String docUnitId) {
	...
	@RequestMapping(value = "/{identifier}", method = RequestMethod.GET, params = { "dc" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(DOC_UNIT_HAB0)
	...
	public ResponseEntity<BibliographicRecordDTO> update(final HttpServletRequest request,
			@RequestBody final BibliographicRecordDTO recordDTO) throws PgcnException, PgcnLockException {
		// Droits d'accès
	...
	public void update(@RequestBody final BibliographicRecordMassUpdateDTO updates) throws PgcnException {
		...
		final Collection<BibliographicRecord> filteredRecords = accessHelper
			.filterBibliographicRecords(updates.getRecordIds());
		// #1864 : pas de controle de droit / workflow.
		final Collection<BibliographicRecord> filteredByWorkflow = new ArrayList<>(filteredRecords);

		updates.setRecordIds(
				filteredByWorkflow.stream().map(BibliographicRecord::getIdentifier).collect(Collectors.toList()));
		final List<BibliographicRecord> savedRecords = bibliographicRecordService.update(updates);
		esDocUnitService.indexAsync(savedRecords.stream()
			.map(BibliographicRecord::getDocUnit)
	...
	private <T> List<T> filterRecordsDTOs(final Collection<T> records, final Function<T, String> getIdentifierFn) {
		return accessHelper
			.filterBibliographicRecords(records.stream().map(getIdentifierFn).collect(Collectors.toList()))
			.stream()
			// Correspondance record autorisé => recordDTO
			.map(doc -> records.stream()
