...
public class CheckConfigurationController extends AbstractRestController {
	...
	public ResponseEntity<CheckConfigurationDTO> create(final HttpServletRequest request,
			@RequestBody final CheckConfigurationDTO checkConfiguration) throws PgcnException {
	...
	public ResponseEntity<CheckConfigurationDTO> delete(final HttpServletRequest request,
	...
	public ResponseEntity<Page<SimpleCheckConfigurationDTO>> search(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiCheckConfigurationService.search(search, filteredLibraries, page, size),
				HttpStatus.OK);
	}
	...
	@Timed
	@RolesAllowed({ CHECK_HAB0 })
	public ResponseEntity<CheckConfigurationDTO> getById(final HttpServletRequest request,
			@PathVariable final String id) {
		final CheckConfigurationDTO configuration = uiCheckConfigurationService.getOne(id);
		return createResponseEntity(configuration);
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "edition" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ CHECK_HAB0 })
	public ResponseEntity<CheckConfigurationDTO> getByIdForEdition(final HttpServletRequest request,
			@PathVariable final String id) {
	...
	@Timed
	@RolesAllowed({ CHECK_HAB0 })
	public ResponseEntity<List<SimpleCheckConfigurationDTO>> getByProjectId(final HttpServletRequest request,
			@RequestParam(value = "project") final String projectId) {
		final List<SimpleCheckConfigurationDTO> configuration = uiCheckConfigurationService
	...
	public ResponseEntity<CheckConfigurationDTO> duplicateCheckConfiguration(final HttpServletRequest request,
			@PathVariable final String id) {
		return new ResponseEntity<>(uiCheckConfigurationService.duplicateCheckConfiguration(id), HttpStatus.OK);
	}

	@RequestMapping(value = "/{idDocUnit}", method = RequestMethod.GET, params = { "docUnit" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ USER_HAB0 })
	public ResponseEntity<CheckConfigurationDTO> getByDocUnit(final HttpServletRequest request,
			@PathVariable final String idDocUnit) {
