...
public class ProjectController extends AbstractRestController {
	...
	@Autowired
	public ProjectController(final ProjectService projectService, final UIProjectService uiProjectService,
			final AccessHelper accessHelper, final EsProjectService esProjectService,
	...
	public ResponseEntity<ProjectDTO> create(final HttpServletRequest request, @RequestBody final ProjectDTO project)
	...
	public void delete(final HttpServletResponse response, @RequestBody final List<ProjectDTO> projects) {
		// Droits d'accès
		final Collection<Project> filteredProjects = accessHelper
			.filterProjects(projects.stream().map(ProjectDTO::getIdentifier).collect(Collectors.toList()));
		uiProjectService.delete(filteredProjects);
		response.setStatus(HttpServletResponse.SC_OK);
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "idDocs" },
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "idLibraries" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	public ResponseEntity<Page<SimpleProjectDTO>> search(final HttpServletRequest request,
		...
		return new ResponseEntity<>(
				uiProjectService.search(search, initiale, active, filteredLibraries, status, providers, page, size),
				HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.GET, params = { "searchProject" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ PROJ_HAB7 })
	public ResponseEntity<List<SimpleProjectDTO>> searchProject(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		return new ResponseEntity<>(
				uiProjectService.loadCreatedProjects(searchProject, initiale, filteredLibraries, statuses, active),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<List<AuditProjectRevisionDTO>> getProjectsForWidget(final HttpServletRequest request,
		...
		final List<String> filteredLibraries = libraryAccesssHelper.getLibraryFilter(request, libraries);
		// Chargement
		final List<AuditProjectRevisionDTO> revisions = uiProjectService.getProjectsForWidget(fromDate,
				filteredLibraries, statuses);
		// Réponse
	...
	public ResponseEntity<Collection<SimpleProjectDTO>> findAllActiveDTO(final HttpServletRequest request) {
		...
		// Droits d'accès
		// simpleProjectDTOs = libraryAccesssHelper.filterObjectsByLibrary(request,
		// simpleProjectDTOs, dto -> dto.getLibrary().getIdentifier());
	...
	@Timed
	public ResponseEntity<Collection<SimpleProjectDTO>> findAllActiveDTO(final HttpServletRequest request,
			@RequestParam final List<String> libraries) {
		Collection<SimpleProjectDTO> simpleProjectDTOs = uiProjectService.findAllActiveByLibraryIn(libraries);
		simpleProjectDTOs = libraryAccesssHelper.filterObjectsByLibrary(request, simpleProjectDTOs,
	...
	@RequestMapping(method = RequestMethod.GET, params = { "dto2", "libraries" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Collection<SimpleProjectDTO>> findAllDTO(final HttpServletRequest request,
			@RequestParam final List<String> libraries) {
	...
	public ResponseEntity<ProjectDTO> update(final HttpServletRequest request, @RequestBody final ProjectDTO projectDTO)
			throws PgcnException {
		// Droits d'accès
		...
		if (!canDisableProj || !canCancelProj) {
			...
			if (!canDisableProj && (project.isActive() && !projectDTO.isActive()
					|| (project.getStatus() != CLOSED && StringUtils.equals(projectDTO.getStatus(), "CLOSED")))) {
				return new ResponseEntity<>(HttpStatus.FORBIDDEN);
			}
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "cancelProj" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ PROJ_HAB6 })
	public ResponseEntity<ProjectDTO> cancelProject(@PathVariable final String id,
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "suspendProj" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ PROJ_HAB5 })
	public ResponseEntity<ProjectDTO> suspendProject(@PathVariable final String id,
			@RequestBody final ProjectDTO projectDTO) {
		// Droits d'accès
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.POST, params = { "reactivProj" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed({ PROJ_HAB5 })
	public ResponseEntity<ProjectDTO> reactivateProject(@PathVariable final String id,
			@RequestBody final ProjectDTO projectDTO) {
		// Droits d'accès
		if (!accessHelper.checkProject(id) || !StringUtils.equals(projectDTO.getStatus(), "PENDING")) {
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
