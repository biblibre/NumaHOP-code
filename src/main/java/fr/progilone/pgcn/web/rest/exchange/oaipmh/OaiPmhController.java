...
public class OaiPmhController extends AbstractRestController {
	...
	@RequestMapping(method = RequestMethod.GET, params = { "listIdentifiers" },
	...
	public ResponseEntity<Map<String, String>> checkQuery(@RequestParam(value = "baseUrl") final String baseUrl,
		...
		final Optional<Map<String, String>> response = oaiPmhService
			.listIdentifiers(baseUrl, "oai_dc", from, to, set, null)
			.map(oai -> {
				final Map<String, String> data = new HashMap<>();
				if (oai.getListIdentifiers().getResumptionToken() != null) {
					data.put("completeListSize", String
						.valueOf(oai.getListIdentifiers().getResumptionToken().getCompleteListSize().toString()));
				}
				else {
	...
	public ResponseEntity<ImportReport> importOaiPmh(final HttpServletRequest request,
		...
		final ImportReport importReport = importReportService.createSimpleImportReport(
				getReportName(baseUrl, from, to, set), null, FileFormat.OAIPMH, DataEncoding.UTF_8, libraryId,
				projectId, lotId, null);

		// Import asynchrone
		try {
			importOaiPmhService.importOaiPmhAsync(libraryId, importReport, new OaiPmhRequest(baseUrl, from, to, set),
