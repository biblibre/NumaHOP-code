 ...
 *
 * @author jbrunet Créé le 24 avr. 2017
 */
@RestController
...
public class ExportInternetArchiveController extends AbstractRestController {
	...
	@Autowired
	public ExportInternetArchiveController(final InternetArchiveService iaService, final DocUnitService docUnitService,
			final EsDocUnitService esDocUnitService, final InternetArchiveServiceAsync iaServiceAsync,
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET, params = { "prepare_item" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	@RolesAllowed(EXPORT_INTERNET_ARCHIVE_HAB0)
	public ResponseEntity<InternetArchiveItemDTO> prepare(final HttpServletRequest request,
			@PathVariable("id") final String identifier) throws PgcnTechnicalException {
		...
		// l'utilisateur, pour le conf à importer
	...
	@Timed
	@RolesAllowed(EXPORT_INTERNET_ARCHIVE_HAB0)
	public ResponseEntity<?> create(final HttpServletRequest request, @PathVariable("id") final String identifier,
	...
	@Timed
	@RolesAllowed(EXPORT_INTERNET_ARCHIVE_HAB0)
	public ResponseEntity<?> save(final HttpServletRequest request, @PathVariable("id") final String identifier,
			@RequestBody final InternetArchiveItemDTO item) throws PgcnTechnicalException {
	...
	@Timed
	@RolesAllowed(EXPORT_INTERNET_ARCHIVE_HAB0)
	public void massExport(final HttpServletRequest request, @RequestParam(name = "docs") final List<String> docs)
