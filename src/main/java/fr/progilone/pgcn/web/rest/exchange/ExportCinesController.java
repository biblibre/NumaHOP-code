...
public class ExportCinesController extends AbstractRestController {
	...
	public ExportCinesController(final CinesRequestHandlerService cinesRequestHandlerService,
			final ExportCinesService exportCinesService, final MailboxConfigurationService mailboxConfigurationService,
			final LibraryAccesssHelper libraryAccesssHelper, final AccessHelper accessHelper) {
	 ...
	 */
	@RequestMapping(value = "/{identifier}", method = RequestMethod.GET, params = { "export" },
	...
	public ResponseEntity<?> saveExportData(@PathVariable("id") final String identifier,
	...
	/**
	 * Génération du répertoire d'export et upload sur le serveur CINES (avec les
	 ...
	 * métadonnées EAD).
	 */
	@RequestMapping(method = RequestMethod.POST, params = { "send", "ead" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@RolesAllowed({ DOC_UNIT_HAB4 })
	...
	/**
	 * Vérification des boites mail, à la recherche d'accusés réception, de notifications
	 * de rejet ou de certificats d'archivage
	 */
	@RequestMapping(method = RequestMethod.GET, params = { "check_mailbox" },
			produces = MediaType.APPLICATION_JSON_VALUE)
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public ResponseEntity<?> checkMailbox(final HttpServletRequest request,
		...
		// Vérification des boites mail auxquelles a accès l'utilisateur
			...
			// Sinon on ne vérifie que les boites mails liées à la bibliothèque de
			// l'utilisateur
	...
	@RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_XML_VALUE,
			params = { "aip", "docUnit" })
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public ResponseEntity<?> getAip(final HttpServletRequest request, final HttpServletResponse response,
	...
	@RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_XML_VALUE,
			params = { "sip", "docUnit" })
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public ResponseEntity<?> getSip(final HttpServletRequest request, final HttpServletResponse response,
			@RequestParam(value = "docUnit") final String docUnitId,
	...
	@RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_XML_VALUE,
			params = { "mets", "docUnit" })
	@RolesAllowed({ DOC_UNIT_HAB4 })
	public ResponseEntity<?> getMets(final HttpServletRequest request, final HttpServletResponse response,
			@RequestParam(value = "docUnit") final String docUnitId,
			@RequestParam(value = "cinesStatus") final String status) throws PgcnTechnicalException {
