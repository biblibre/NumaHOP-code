...
public class Z3950Controller extends AbstractRestController {
	...
	@Autowired
	public Z3950Controller(final LibraryAccesssHelper libraryAccesssHelper, final ImportMarcService importMarcService,
			final ImportReportService importReportService, final Z3950Service z3950Service) {
	 ...
	 * @param z3950ResultDTO résultat de recherche Z39.50 à importer
	 * @param libraryId identifiant de la bibliothèque pour laquelle s'effectue l'import
	 * @param mappingId identifiant du mapping à utiliser pour importer le fichier
	...
	public ResponseEntity<ImportReport> importFile(final HttpServletRequest request,
			@RequestBody final Z3950RecordDTO z3950ResultDTO, @RequestParam(value = "library") final String libraryId,
		...
		if (marcXml != null && !marcXml.isEmpty()) {
			LOG.info("Import depuis le serveur Z39.50 {} (format {}, encodage {}, taille {})", z3950Name,
					importReport.getFileFormat(), importReport.getDataEncoding(),
			...
			try {
				...
				try (InputStream in = new ByteArrayInputStream(marcXml.getBytes(StandardCharsets.UTF_8))) {
					importFile = importReportService.saveImportFile(in, importReport,
							importReport.getFiles().get(0).getOriginalFilename());
					LOG.debug("Copie depuis le serveur Z39.50 {} vers le fichier {}", z3950Name,
							importFile.getAbsolutePath());
				}
				importMarcService.importMarcXmlAsync(Collections.singletonList(importFile), mappingId, importReport,
						stepValidation, stepDeduplication, stepDeduplication ? defaultDedupProcess : null, false,
						false);
			...
			LOG.info("L'import depuis le serveur Z39.50 {} (format {}, encodage {}) est annulé car il est vide",
					z3950Name, FileFormat.MARCXML, importReport.getDataEncoding());
	...
	private ImportReport createImportReport(final Z3950RecordDTO z3950RecordDTO, final String libraryId,
		...
		return importReportService.createSimpleImportReport(fileName, fileSize, FileFormat.MARCXML, DataEncoding.UTF_8,
