...
public class SearchController extends AbstractRestController {
	...
	@Autowired
	public SearchController(final EsDeliveryService esDeliveryService, final EsDocUnitService esDocUnitService,
			final EsConditionReportService esConditionReportService, final EsLotService esLotService,
			final EsProjectService esProjectService, final EsTrainService esTrainService,
			final IndexManagerService indexManagerService, final LibraryAccesssHelper libraryAccesssHelper) {
	 ...
	 * Recherche générale
	 * @param request
	 * @param searches termes recherchés
	 * @param filters filtres (post-filters), liés à la sélection d'une facette
	 * @param page n° de la page
	 * @param size taille de la page
	 * @param searchOn type de donnée recherche
	 * @param fuzzy recherche approchée / exacte
	 * @param sorts tris
	 * @param facet inclure les facettes dans le résultat de la recherche
	 * @return
	...
	public ResponseEntity<Map<SearchEntity, Page<?>>> search(final HttpServletRequest request,
			...
			@RequestParam(name = "facet", required = false, defaultValue = "false") final boolean facet)
			throws PgcnException {

		final List<String> libraries = libraryAccesssHelper.getLibraryFilter(request, null);

		final Map<SearchEntity, Page<?>> result = searchOn.stream()
			.collect(Collectors.toMap(Function.identity(), searchEntity -> {
				switch (searchEntity) {
					case CONDREPORT:
						return esConditionReportService.search(searches, filters, libraries, fuzzy, page, size, sorts,
								facet);
					case DELIVERY:
	...
	public ResponseEntity<List<Map<String, Object>>> suggest(final HttpServletRequest request,
		...
		final List<String> libraries = libraryAccesssHelper.getLibraryFilter(request,
				Collections.singletonList(SUGGEST_CTX_GLOBAL));
