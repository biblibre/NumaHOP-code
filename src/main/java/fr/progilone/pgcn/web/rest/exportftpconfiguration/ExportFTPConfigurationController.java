...
public class ExportFTPConfigurationController extends AbstractRestController {
	...
	public ResponseEntity<ExportFTPConfigurationDTO> create(final HttpServletRequest request,
			@RequestBody final ExportFTPConfigurationDTO confDto) throws PgcnException, PgcnTechnicalException {
		// Vérification des droits d'accès par rapport à la bibliothèque de l'utilisateur,
		// pour la conf à importer
		if (confDto.getLibrary() == null
				|| !libraryAccesssHelper.checkLibrary(request, confDto.getLibrary().getIdentifier())) {
	...
	public ResponseEntity<ExportFTPConfigurationDTO> delete(final HttpServletRequest request,
	...
	public ResponseEntity<Page<SimpleExportFTPConfDTO>> search(final HttpServletRequest request,
		...
		return new ResponseEntity<>(uiExportFTPConfigurationService.search(search, filteredLibraries, page, size),
				HttpStatus.OK);
	}
	...
	@Timed
	@RolesAllowed({ EXP_FTP_HAB0 })
	public ResponseEntity<ExportFTPConfigurationDTO> getById(final HttpServletRequest request,
			@PathVariable final String id) {
		// Chargement
	...
	@RequestMapping(method = RequestMethod.GET, params = { "project", "fullConfig" },
	...
	public ResponseEntity<List<? extends AbstractDTO>> getByProjectId(final HttpServletRequest request,
		...
		final List<? extends AbstractDTO> configuration = uiExportFTPConfigurationService.getAllByProjectId(project,
				libraries, fullConfig);
	...
	public ResponseEntity<ExportFTPConfigurationDTO> update(final HttpServletRequest request,
		...
		final ExportFTPConfiguration dbConfiguration = exportFtpConfigurationService
			.getOne(configuration.getIdentifier());
		// Non trouvé
		if (dbConfiguration == null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
		// Vérification des droits d'accès par rapport à la bibliothèque de l'utilisateur,
		// pour la conf existant
