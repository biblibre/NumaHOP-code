...
public class ConditionReportDetailControllerTest {
	...
	public void setUp() {
		final ConditionReportDetailController controller = new ConditionReportDetailController(accessHelper,
				workflowAccessHelper, conditionReportService, conditionReportDetailService, esConditionReportService);
	...
	public void testCreate() throws Exception {
		...
		when(conditionReportDetailService.findParentByIdentifier(detail.getIdentifier()))
			.thenReturn(detail.getReport());

		// 403
		this.restMockMvc
			.perform(post("/api/rest/condreport_detail").param("type", type.name())
	...
	public void testDelete() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(delete("/api/rest/condreport_detail/{id}", detailId).contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isForbidden());
	...
	public void testFindByIdentifier() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(get("/api/rest/condreport_detail/{identifier}", detailId).accept(MediaType.APPLICATION_JSON))
			.andExpect(status().isForbidden());
	...
	public void testFindByConditionReport() throws Exception {
		...
		when(conditionReportService.findDocUnitByIdentifier(detail.getReport().getIdentifier()))
			.thenReturn(detail.getReport().getDocUnit());
		...
		// 403
		this.restMockMvc
			.perform(get("/api/rest/condreport_detail").param("report", reportId).accept(MediaType.APPLICATION_JSON))
			.andExpect(status().isForbidden());
	...
	public void testUpdate() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(post("/api/rest/condreport_detail/{id}", detailId).contentType(MediaType.APPLICATION_JSON)
