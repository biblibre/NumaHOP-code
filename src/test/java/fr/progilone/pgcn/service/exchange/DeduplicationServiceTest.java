...
public class DeduplicationServiceTest {
	...
	public void testLookupDuplicates() {
		...
		when(docUnitRepository.getOneByPgcnIdAndState(docUnit.getPgcnId(), DocUnit.State.AVAILABLE))
			.thenReturn(duplDocUnit);
		when(docUnitRepository.searchDuplicates(eq(docUnit), any(), eq(DocUnit.State.AVAILABLE)))
