...
public class ConditionReportAttachmentServiceTest {
	...
	@BeforeEach
	public void setUp() {
		service = new ConditionReportAttachmentService(conditionReportAttachmentRepository, conditionReportRepository,
				fm);
	...
	public void testUploadAttachment() throws UnsupportedEncodingException {
		...
		final MockMultipartFile file = new MockMultipartFile(name, name, "text/plain",
		...
		when(conditionReportAttachmentRepository.save(any(ConditionReportAttachment.class)))
			.then(new SetIdAndReturnsArgumentAt(0, attachmentId));
		when(fm.copyInputStreamToFile(any(InputStream.class), anyString(), any(AbstractDomainObject.class),
				anyString()))
			.thenReturn(new File(REPORT_DIR));

		final List<ConditionReportAttachment> actual = service.uploadAttachment(Collections.singletonList(file),
				reportId);
		...
		verify(fm).copyInputStreamToFile(any(InputStream.class), eq(REPORT_DIR), any(AbstractDomainObject.class),
