...
public class EadCParserTest {
	...
	public void test01() throws ParserConfigurationException, JAXBException, SAXException, IOException {
		final String xml = "<c id=\"toplevel-001\">" + "<c id=\"sublevel-001-a\"></c>" + "<c id=\"sublevel-001-b\"></c>"
				+ "</c>";
	...
	public void test06() throws ParserConfigurationException, JAXBException, SAXException, IOException {
		final String xml = "<c id=\"toplevel-001\">" + "<did id=\"test-did\">"
				+ "<unitid type=\"division\">test top 001</unitid>" + "<physdesc>physdesc top 001</physdesc>" + "</did>"
				+ "<c id=\"sublevel-001-a\">"
	...
	public void test06b() throws ParserConfigurationException, JAXBException, SAXException, IOException {
		final String xml = "<c id=\"toplevel-001\">"
				...
				+ "<did id=\"test-did3\"><unitid type=\"division\">test sub sub a</unitid></did>" + "</c>"
				+ "<c id=\"sublevel-001-a-b\">"
				+ "<did id=\"test-did4\"><unitid type=\"division\">test sub sub a</unitid></did>" + "</c>" + "</c>"
				+ "<c id=\"sublevel-002-a\">" + "<did id=\"test-did5\"></did>" + "<c id=\"sublevel-002-a-a\">"
				+ "<did id=\"test-did6\"><unitid type=\"division\">test sub sub b</unitid></did>" + "</c>" + "</c>"
				+ "</c>";
		final C c = EadTestUtils.getCFromXml(xml);
		...
		C subC = eadCParser.getcLeaves()
			.stream()
			.filter(l -> l.getId().equals("sublevel-001-a-b"))
			.findAny()
			.orElse(null);
		assertNotNull(subC);
		...
		subC = eadCParser.getcLeaves()
			.stream()
			.filter(l -> l.getId().equals("sublevel-002-a-a"))
			.findAny()
			.orElse(null);
		assertNotNull(subC);
	...
	public void test07() throws ParserConfigurationException, JAXBException, SAXException, IOException {
		final String xml = "<c id=\"toplevel-001\">" + "  <did id=\"test-did\">" + "    <unittitle>Recueil</unittitle>"
				+ "    <physdesc>physdesc top 001</physdesc>" + "  </did>" + "  <c id=\"sublevel-001-a\">"
				+ "    <did id=\"test-did\">" + "      <unittitle>الشرح على الجرومية لأسرار العربية</unittitle>"
				+ "    </did>" + "  </c>" + "  <c id=\"sublevel-001-b\">" + "    <did id=\"test-did\">"
				+ "      <unittitle>al-Šarḥ ‘alá al-Ǧurūmiyyaẗ li-asrār al-‘arabiyyaẗ</unittitle>" + "    </did>"
				+ "  </c>" + "</c>";
		final C c = EadTestUtils.getCFromXml(xml);
