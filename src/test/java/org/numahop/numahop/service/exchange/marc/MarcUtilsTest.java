...
public class MarcUtilsTest {
	...
	/**
	 * Tester un fichier unimarc pour vérifier sa validité. Renseigner le chemin Laisser
	 * en @Disabled
	...
	public void testGetRecordInXml() {
		...
		final String expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
				+ "<marc:collection xmlns:marc=\"http://www.loc.gov/MARC21/slim\">" + "<marc:record>"
				+ "<marc:leader>00000nam a2200000 a 4500</marc:leader>"
				+ "<marc:datafield tag=\"200\" ind1=\" \" ind2=\" \">"
				+ "<marc:subfield code=\"a\">Harry Potter contre attaque</marc:subfield>" + "</marc:datafield>"
	...
	public void testSplitRecordByItems() {
		...
		record.addVariableField(
				marcFactory.newDataField("200", ' ', ' ', "a", "Dictionnaire universel fran&#xE7;ois et latin"));
		...
		assertEquals("BULAC RES MON Fol 1351",
				((DataField) actualRecord.getVariableField("995")).getSubfield('k').getData());
		...
		assertEquals("Dictionnaire universel fran&#xE7;ois et latin",
				((DataField) actualRecord.getVariableField("200")).getSubfield('a').getData());

		assertEquals(1, actualRecord.getVariableFields("995").size());
		assertEquals(1, ((DataField) actualRecord.getVariableField("995")).getSubfields('9').size());
		assertEquals(1, ((DataField) actualRecord.getVariableField("995")).getSubfields('k').size());
		assertEquals("97396", ((DataField) actualRecord.getVariableField("995")).getSubfield('9').getData());
		assertEquals("BULAC RES MON Fol 1350",
				((DataField) actualRecord.getVariableField("995")).getSubfield('k').getData());
		...
		// itemTags = 327, 945
		record = marcFactory.newRecord();
		record.addVariableField(marcFactory.newDataField("327", ' ', ' ', "a",
		...
		assertEquals("COL4°2022(1955-1960)",
				((DataField) actualRecord.getVariableField("945")).getSubfield('b').getData());
		...
		record = marcFactory.newRecord();
		record.addVariableField(marcFactory.newDataField("327", ' ', ' ', "a",
				"1958. [1] Le programme national du PCF - Février 1958 (4 p.)", "a",
				"[2] Une nouvelle lettre de M. Thorez à G. Mollet - Février 1958 (1 f.)", "b", "test 1", "b",
				"test 2"));
		record.addVariableField(marcFactory.newDataField("327", ' ', ' ', "a",
				"1960. [1]Bienvenue à N. Khrouchtchev - avril 1960 (2 p.)", "b", "test 3"));
		record.addVariableField(marcFactory.newDataField("945", ' ', ' ', "b", "COL4°2022(1955-1960)"));

		actual = MarcUtils.splitRecordByMarcKeys(record,
				Arrays.asList(new MarcKey("327", 'a'), new MarcKey("945", 'b')));
	...
	public void test170901() {
		...
		record.addVariableField(marcFactory.newDataField("995", ' ', ' ', "f", "00000001704148", "k", "FLM.12°0289",
				"r", "TR", "o", "L7"));

		final List<Record> actual = MarcUtils.splitRecordByMarcKeys(record,
				Arrays.asList(new MarcKey("966", 'a'), new MarcKey("995", 'f')));
	...
	public void test180319() {
		...
		record.addVariableField(marcFactory.newDataField("852", ' ', ' ', "h", "4 AE SUP 125 RES A1", "v",
				"ANNEE.1863-1865;Vol.N.4-5-6 A1", "h", "4 AE SUP 125 RES A2", "v", "ANNEE.1863-1865;Vol.N.4-5-6 A2"));
		record.addVariableField(marcFactory.newDataField("852", ' ', ' ', "h", "4 AE SUP 125 RES B", "v",
				"ANNEE.1860-1862;Vol.N.1-2-3"));

		final List<Record> actual = MarcUtils.splitRecordByMarcKeys(record,
				Arrays.asList(new MarcKey("930", 'a'), new MarcKey("852", 'h'), new MarcKey("852", 'v')));
