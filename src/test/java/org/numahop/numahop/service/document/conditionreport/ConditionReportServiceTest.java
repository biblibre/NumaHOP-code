...
public class ConditionReportServiceTest {
	...
	@BeforeEach
	public void setUp() {
		service = new ConditionReportService(conditionReportRepository, conditionReportDetailService,
				conditionReportAttachmentService, docUnitRepository, esConditionReportService, libraryService,
				odtEngineService, userService, jasperReportService, conditionReportSlipConfigurationService);

		final CustomUserDetails customUserDetails = new CustomUserDetails("90cdcf40-ff39-4d8d-aad5-249c29a94b3a",
				"mickey", null, null, USER_LIBRARY_ID, null, false, User.Category.OTHER);
	...
	public void testCreate() {
		...
		when(conditionReportRepository.findByDocUnit(docUnitId)).thenReturn(new ConditionReport(),
	...
	public void testSearch() {
		...
		final ConditionReportRepositoryCustom.DimensionFilter dimensions = new ConditionReportRepositoryCustom.DimensionFilter(
				ConditionReportRepositoryCustom.DimensionFilter.Operator.EQ, 1, 2, 3);
		final LocalDate from = LocalDate.now().minusMonths(1);
		final LocalDate to = LocalDate.now();
		final List<String> descriptions = Arrays.asList("a=TORTUE_NINJA", "a=COURGE_DU_NEBRASKA",
				"y=28_JOURS_PLUS_TARD");
		final Integer page = 0;
		final Integer size = 10;
		final List<String> sorts = Collections.singletonList("docUnit.pgcnId");

		final Page<String> ids = new PageImpl<>(
				Arrays.asList("3c2cffc6-8301-4433-80b4-b97a09e530c8", "af02ba13-83e5-4e12-b347-30fc655de2b6"));
		final DocUnit docUnit = new DocUnit();
		...
		final Page<ConditionReportService.SearchResult> actual = service.search(libraries, projects, lots, dimensions,
				from, to, descriptions, null, false, page, size, sorts);

		final List<ConditionReportDTO> actualReports = actual.getContent()
			.stream()
			.map(ConditionReportService.SearchResult::getReport)
			.collect(Collectors.toList());
		final List<SimpleListDocUnitDTO> actualDocs = actual.getContent()
			.stream()
			.map(ConditionReportService.SearchResult::getDocUnit)
			.collect(Collectors.toList());

		// résultat
		TestUtil.checkCollectionContainsSameElements(
				reports.stream().map(ConditionReport::getIdentifier).collect(Collectors.toList()),
		...
		TestUtil.checkCollectionContainsSameElements(Arrays.asList("TORTUE_NINJA", "COURGE_DU_NEBRASKA"),
				actualBindings.get("a"));
		TestUtil.checkCollectionContainsSameElements(Collections.singletonList("28_JOURS_PLUS_TARD"),
				actualBindings.get("y"));
	}
	...
	public void testExportDocument() throws PgcnTechnicalException {
		...
		verify(odtEngineService).generateDocumentODT(eq(Name.ConditionReport), eq(report.getDocUnit().getLibrary()),
				anyMap(), anyMap(), same(out));

		// PDF
		service.exportDocument(reportId, out, ConditionReportService.ConvertType.PDF);

		verify(odtEngineService).generateDocumentPDF(eq(Name.ConditionReport), eq(report.getDocUnit().getLibrary()),
				anyMap(), anyMap(), same(out), isNull());
	}
