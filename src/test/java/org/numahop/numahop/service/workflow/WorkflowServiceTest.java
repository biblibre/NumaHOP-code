...
/**
 * @author jbrunet Créé le 16 oct. 2017
 */
...
public class WorkflowServiceTest {
	...
	public void initializeWorkflowOptionnal() {
		...
		// Une étape étant facultative, il doit en rester 2 en attente (on a en plus
		// l'etape de validation notices dispo des le debut maintenant)
		...
		// On va donc la chercher dans les étapes déroulées, elle ne doit plus être
		// présente dans les "en cours / future")
		validateCompletedState(workflowInstance, WorkflowStateKey.GENERATION_BORDEREAU, WorkflowStateStatus.SKIPPED,
				SYSTEM_LOGIN);
	}
	...
	public void FirstStepWorkflowOptionnal() {
		...
		validateCompletedState(workflowInstance, WorkflowStateKey.VALIDATION_CONSTAT_ETAT, WorkflowStateStatus.FINISHED,
				LOGIN_USER1);

		// On vérifie que les étapes Validation bordereau et constat d'état avant
		// numérisation sont bien passées
		validateCompletedState(workflowInstance, WorkflowStateKey.VALIDATION_BORDEREAU_CONSTAT_ETAT,
				WorkflowStateStatus.SKIPPED, SYSTEM_LOGIN);
		validateCompletedState(workflowInstance, WorkflowStateKey.CONSTAT_ETAT_AVANT_NUMERISATION,
				WorkflowStateStatus.SKIPPED, SYSTEM_LOGIN);
	...
	public void DeliveryWorkflowOptionnal() {
		...
		// On vérifie que l'étape est bien terminée
		validateCompletedState(workflowInstance, WorkflowStateKey.LIVRAISON_DOCUMENT_EN_COURS,
				WorkflowStateStatus.FINISHED, LOGIN_PRESTA);
		// On vérifie que l'étape de numérisation par ricochet est bien terminée
		validateCompletedState(workflowInstance, WorkflowStateKey.NUMERISATION_EN_ATTENTE, WorkflowStateStatus.FINISHED,
				SYSTEM_LOGIN);
		...
		// On vérifie que l'étape est bien terminée
		validateCompletedState(workflowInstance, WorkflowStateKey.CONTROLES_AUTOMATIQUES_EN_COURS,
				WorkflowStateStatus.FINISHED, SYSTEM_LOGIN);
	...
	public void initializeWorkflowComplete() {
		...
		// Ce doit être l'étape VALIDATION_CONSTAT_ETAT et l'étape GENERATION_BORDEREAU
		// et VALIDATION_NOTICES
		validateStateIsPending(workflowInstance, WorkflowStateKey.VALIDATION_CONSTAT_ETAT);
		...
		// On va valider la génération du bordereau et vérifier que validation constat
		// d'état est toujours en attente
	 ...
	 * @param user
	 */
	private void validateCompletedState(final DocUnitWorkflow workflow, final WorkflowStateKey key,
			final WorkflowStateStatus status, final String user) {
	 ...
	 * @param status
	 */
	private void checkStepState(final DocUnitWorkflow workflow, final WorkflowStateKey key,
			final WorkflowStateStatus status) {
		final DocUnitState state = workflow.getCurrentStates()
			.stream()
	 ...
	 * @param status
	 */
	private void checkStepStateEndedWithStatus(final DocUnitWorkflow workflow, final WorkflowStateKey key,
			final WorkflowStateStatus status) {
	 ...
	 * @return
	 */
	private WorkflowModelState generateModelState(final WorkflowModel model, final WorkflowStateKey key,
			final boolean toSkip) {
