...
/**
 * Compilation des rapports jrxml Created by Sebastien on 21/09/2016.
 */
public class JasperCompile {
	...
	/**
	 * Cette méthode génère les fichiers jasper compilés dans le répertoire
	 * REPORT_JASPER_PATH à partir des sources jrxml présentes dans le répertoire
	 * REPORT_SRC_PATH
	 */
	@Disabled
	@Test
	public void compileAllReports() {
		...
		Stream.of(reportSources).parallel().forEach(jrxml -> {
			try {
				...
				LOG.error("La compilation du rapport {} a échoué sur l'erreur {}", jrxml.getAbsolutePath(),
						e.getMessage());
			}
		});
