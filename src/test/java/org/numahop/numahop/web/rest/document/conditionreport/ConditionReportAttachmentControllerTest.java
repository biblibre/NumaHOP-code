...
public class ConditionReportAttachmentControllerTest {
	...
	@BeforeEach
	public void setUp() {
		final ConditionReportAttachmentController controller = new ConditionReportAttachmentController(accessHelper,
				conditionReportService, conditionReportAttachmentService);
		final FormattingConversionService convService = new DefaultFormattingConversionService();
		convService.addConverter(String.class, ConditionReport.class,
				TestConverterFactory.getConverter(ConditionReport.class));
	...
	public void testFindByReport() throws Exception {
		...
		when(conditionReportAttachmentService.findByReport(report.getIdentifier()))
			.thenReturn(Collections.singletonList(attachment));

		// 403
		this.restMockMvc
			.perform(get("/api/rest/condreport_attachment").param("report", report.getIdentifier())
	...
	public void testDelete() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(delete("/api/rest/condreport_attachment/{id}", attachmentId)
	...
	public void testDownloadAttachment() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(get("/api/rest/condreport_attachment/{id}", attachmentId).param("file", "true")
	...
	public void testDownloadThumbnail() throws Exception {
		...
		// 403
		this.restMockMvc
			.perform(get("/api/rest/condreport_attachment/{id}", attachmentId).param("thumbnail", "true")
	...
	public void testUploadAttachments() throws Exception {
		...
		final MockMultipartFile file = new MockMultipartFile("file", "test.txt", "text/plain",
				"Duis ut risus finibus, blandit orci in, convallis dolor.".getBytes(StandardCharsets.UTF_8));

		when(conditionReportService.findDocUnitByIdentifier(report.getIdentifier())).thenReturn(report.getDocUnit());
		when(accessHelper.checkDocUnit(eq(docUnit.getIdentifier()))).thenReturn(false, true);
		when(conditionReportAttachmentService.uploadAttachment(any(), eq(report.getIdentifier())))
			.thenReturn(attachments);

		// 403
		this.restMockMvc
			.perform(multipart("/api/rest/condreport_attachment").file(file)
	...
	private File createTmpFile(final String id) throws IOException {
		...
		FileUtils.writeStringToFile(file, "Duis ut risus finibus, blandit orci in, convallis dolor.",
